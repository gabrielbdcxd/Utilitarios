...
abstract class _Email
{
	...
	protected static function i( $html='', $plain='' )
	{
		...
		/**
		 * PHP mail() line endings within email headers need to match the operating system default, which then passes the mail off to the local MTA which then replaces them if needed.
		 * @see	<a href='https://bugs.php.net/bug.php?id=15841'>PHP Bug 15841</a>
		 */
		$email->eol = ( \IPS\Settings::i()->mail_method === 'smtp' ) ? "\r\n" : PHP_EOL;
	...
	{
		$this->headers['From']		= ( $this->fromName ) ? ( '=?UTF-8?B?' . base64_encode( $this->fromName ) . "?= <{$this->from}>" ) : "<{$this->from}>";
		...
		foreach ( $this->headers as $k => $v )
		{
			...
			if( !in_array( \strtolower( $k ), array( 'from', 'to', 'cc', 'bcc', 'subject' ) ) )
			{
				...
				{
					$v	= \substr( wordwrap( "{$k}: {$v}", 78, "{$this->eol} " ), \strlen( $k ) + 2 );
				}
			}
		...
		return implode( "{$this->eol}", $return );
	}
	...
	public function wrapEmail()
	{
		...
		if( !empty( $this->to ) )
		{
			try
			{
				...
				/* Language may already be set from parseRecipients */
				if( $this->language === NULL )
				{
					$this->language = $member->language( TRUE );
				}
			}
	...
	public function send( $to, $cc=array(), $bcc=array() )
	{
		...
		/* Replace attachment and emoticon URLs */
		\IPS\Output::i()->parseFileObjectUrls( $this->emailBody['html'] );
		\IPS\Output::i()->parseFileObjectUrls( $this->emailBody['plain'] );
		...
		catch( \IPS\Email\Outgoing\Exception $e )
		{
			\IPS\Db::i()->insert( 'core_mail_error_logs', array(
				...
				'mlog_resend_data'	=> json_encode( array( 'headers' => array_map( function( $header ){ return (string) $header; }, $this->headers ), 'body' => $this->emailBody, 'boundary' => $boundary ) ),
			...
			/* Return the exception so it can be used */
			return $e;
		}
	...
	{
		if ( \IPS\Settings::i()->mandrill_api_key && \IPS\Settings::i()->mandrill_use_for == 2 )
		{
			$toArray = array();
			...
			/* Replace attachment and emoticon URLs */
			\IPS\Output::i()->parseFileObjectUrls( $body );
		...
		else
		{
			foreach ( $recipients as $address => $vars )
			{
				...
				/* Replace attachment and emoticon URLs */
				\IPS\Output::i()->parseFileObjectUrls( $content['html'] );
				\IPS\Output::i()->parseFileObjectUrls( $content['plain'] );
	...
	public function forceSend( $boundary )
	{
		...
		/* Smtp relies on $this->recipients being populated for the RCPT TO command, which is not done when force sending */
		if ( \IPS\Settings::i()->mail_method === 'smtp' )
		{
			$this->_parseRecipients( $this->to, TRUE );
		}
	...
	protected function _buildMessage( $boundary )
	{
		...
		{
			$this->message	.= "--{$boundary}{$this->eol}";
			$this->message	.= "Content-Type: {$contentType}; charset=UTF-8{$this->eol}";
			$this->message	.= preg_replace( "/(?<!\r)\n/", "{$this->eol}", $content );
			$this->message	.= "{$this->eol}";
		}
	...
	protected function _parseRecipients( $data, $emailOnly=FALSE )
	{
		...
		foreach ( $data as $recipient )
		{
			...
			elseif ( $recipient instanceof \IPS\Member )
			{
				...
				$this->language	= $recipient->language( TRUE );
			}
			else
			{
	 ...
	 * @note	Sending a lot of users to Mandrill can take longer than the default 5 second timeout, so we set it to 20 second timeout
	 */
	...
	{
		\IPS\Output::i()->parseFileObjectUrls( $text );
