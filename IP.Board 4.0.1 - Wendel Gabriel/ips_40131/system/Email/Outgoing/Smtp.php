...
class _SMTP extends \IPS\Email
{
	...
	public function connect()
	{
		...
		/* Check for tls:// in the hostname */
		if ( mb_strpos( \IPS\Settings::i()->smtp_host, 'tls://' ) !== FALSE )
		{
			$tls = TRUE;
			\IPS\Settings::i()->smtp_host = mb_substr( \IPS\Settings::i()->smtp_host, 6 );
		}
		...
		/* If we use ssl:// we need to strip that out */
		$host = \IPS\Settings::i()->smtp_host;

		if( mb_strpos( \IPS\Settings::i()->smtp_host, '://' ) !== FALSE )
		{
		...
		{
			$helo = 'EHLO';
			$responseCode = $this->_sendCommand( 'EHLO ' . $host, 250 );
		}
		catch ( \IPS\Email\Outgoing\Exception $e )
		{
			$helo = 'HELO';
			$responseCode = $this->_sendCommand( 'HELO ' . $host, 250 );
		}
		...
		if( $tls === TRUE )
		{
			...
			$this->_sendCommand( $helo . ' ' . $host, 250 );
		}
