...
class _Incoming
{
	...
	protected function parse( $email )
	{
		...
		{
			if( is_array( $this->mail->headers['cc'] ) )
			{
				$this->mail->headers['cc'] = implode( ",", $this->mail->headers['cc'] );
			}
	...
	protected function isAutoreply()
	{
		...
		if ( !empty( $this->mail->headers['delivered-to'] ) AND is_array( $this->mail->headers['delivered-to'] ) )
		{
			foreach( $this->mail->headers['delivered-to'] AS $deliveredTo )
			{
		...
		if( !empty($this->mail->headers['delivered-to']) AND is_string( $this->mail->headers['delivered-to'] ) AND mb_strtolower($this->mail->headers['delivered-to']) == 'autoresponder' )
		{
			return TRUE;
		}
