...
class _Output
{
	...
	public function js( $file, $app=NULL, $location=NULL )
	{
		...
		{
			/* @see http://community.---.com/4bugtrack/some-js-files-get-loaded-with-a-double-in-the-url-r3558/ */
			return array( rtrim( \IPS\Http\Url::baseUrl( \IPS\Http\Url::PROTOCOL_RELATIVE ), '/' ) . "/applications/{$app}/interface/{$file}?v=" . \IPS\SUITE_UNIQUE_KEY );
		}
		elseif ( \IPS\IN_DEV )
		{
	 ...
	 * @see		<a href='http://community.---.com/resources/bugs.html/_/4-0-0/downloads-special-characters-in-filenames-r46080'>Chrome does not need name encoded anymore</a>
	 */
	...
	public function sendOutput( $output='', $httpStatusCode=200, $contentType='text/html', $httpHeaders=array(), $cacheThisPage=TRUE, $pageIsCached=FALSE )
	{
		...
		if (
			...
			\IPS\Data\Cache::i()->storeWithExpire( 'page_' . md5( $_SERVER['REQUEST_URI'] ) . '_' . \IPS\Member::loggedIn()->language()->id . '_' . \IPS\Theme::i()->id, array(
	...
	{
		if ( mb_stristr( $output, '<fileStore.' ) )
		{
	...
	public function buildMetaTags()
	{
		...
		if( !$this->metaTagsUrl )
		{
			$this->metaTagsUrl	= \IPS\Request::i()->url()->queryPath();
