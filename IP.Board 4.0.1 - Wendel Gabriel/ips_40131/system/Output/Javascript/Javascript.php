...
class _Javascript extends \IPS\Patterns\ActiveRecord
{
	...
	public function save()
	{
		...
		if ( \IPS\IN_DEV AND $this->type == 'plugin' )
		{
			...
			$path = \IPS\ROOT_PATH . '/plugins/' . $this->plugin . '/dev/js';
			
			if ( !\is_dir( $path ) )
			{
				if ( !@\mkdir( $path, 0777, true ) or !@\chmod( $path, 0777 ) )
				{
		...
		array(
			'javascript_app', 'javascript_location', 'javascript_path', 'javascript_name'
		) );
	}
	...
	public function delete()
	{
		...
		{
			/* Write the file to disk in the correct location */
			$file = \IPS\ROOT_PATH . '/plugins/' . $this->plugin . '/dev/js/' . $this->name;
		...
		$where    = array( 'javascript_app=?', 'javascript_location=?', 'javascript_path=?', 'javascript_name=?' );
		$bindings = array( $this->app, $this->location, $this->path, $this->name );

		if ( $this->location == 'plugins' )
		{
			$where[]    = 'javascript_plugin=?';
			$bindings[] = $this->plugin;
		}
		
		\IPS\Db::i()->delete( 'core_javascript', array( implode( ' AND ', $where ), $bindings ) );
	}
	 ...
	 * @return	XML Object
	 */
	public static function createXml( $app )
	{
		...
		/* Loop */
		foreach ( \IPS\Db::i()->select( '*', 'core_javascript', ( $app === 'core' ) ? \IPS\Db::i()->in( 'javascript_app', array('core', 'global') ) : array( 'javascript_app=?', $app ), 'javascript_path, javascript_name' ) as $js )
		{
	...
	public static function exportDev( $app )
	{
		...
		{
			$xml = new \XMLReader;
			$xml->open( \IPS\ROOT_PATH . '/applications/' . $app . '/data/javascript.xml' );
	...
	protected static function _importDevDirectory( $root, $app, $location, $path='' )
	{
		...
		{
			static::$_orders[ $app ][] = array( 'path' => str_replace( \IPS\ROOT_PATH, '', $dir ), 'contents' => file_get_contents( $dir . '/order.txt' ) );
	...
	public static function compile( $app, $location=null, $file=null )
	{
		...
		else
		{
			...
			if ( $app === 'core' )
			{
				...
				{
					$obj = static::_writeJavascriptFromResultset( \IPS\Db::i()->select( '*', 'core_javascript', array( 'javascript_app=? AND javascript_location=? AND javascript_type=?', 'core', 'plugins', 'plugin' ), 'javascript_app, javascript_location, javascript_position' )->setKeyField('javascript_id'), 'plugins.js', 'core', 'plugins' );
	...
	{
		$content = '';
		$jsMap   = ( isset( \IPS\Data\Store::i()->javascript_map ) ) ? \IPS\Data\Store::i()->javascript_map : array();
...
EOF;
		...
		return \IPS\File::create( 'core_Theme', $location . '_' . $fileName, $header . "\n\n" . $content, 'javascript_' . $app, TRUE );
	}
	
	/**
	 * Get javascript language as a script
	 * 
	 * @param	int		$lang	ID of language to fetch
	 * @return string
	 */
	...
	protected static function _getMapAsScript()
	{
		...
		$json = version_compare( PHP_VERSION, '5.4.0' ) >= 0 ? json_encode( $map, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) : str_replace( '\\/', '/', json_encode( $map ) );
		
		return 'var ipsJavascriptMap = ' . $json . ';';
	}
	
	/**
	 * Minifies javascript if IPS created
	 * 
	...
	{
		require_once( \IPS\ROOT_PATH . '/system/3rd_party/Minify/lib/JSMin.php' );

		if ( mb_stristr( mb_substr( $js, 0, 1000 ), '* IPS Social Suite 4' ) )
		{
			$js = \JSMin::minify( $js );
		}
	...
	public static function inDevJs( $file, $app=NULL, $location=NULL )
	{
		...
		if ( $app === NULL AND $location === NULL )
		{
			...
			if ( mb_substr( $file, 0, 8 ) === 'js_lang_' )
			{
				return array( \IPS\Settings::i()->base_url . "/applications/core/interface/js/jslang.php?langId=" . intval( mb_substr( $file, 8, -3 ) ) );
			}
		}
		...
		{
			return array( str_replace( \IPS\ROOT_PATH, \IPS\Settings::i()->base_url, $path ) );
		}
	}
	...
	public static function inDevMapJs()
	{
					...
					foreach ( new \DirectoryIterator( $root . "{$location}/controllers" ) as $controllerDir )
					{
						...
						if ( $controllerDir->isDir() )
						{
							...
							foreach ( new \DirectoryIterator( $root . "{$location}/controllers/{$controllerDir}" ) as $file )
							{
								...
								$files[ $app ][ $location . '_' . $controllerDir ] = str_replace( \IPS\ROOT_PATH, trim( \IPS\Settings::i()->base_url, '/' ), $controllerPath ) . '/' . $file->getFileName();
							}
						}
					}
				}
		...
		$json = version_compare( PHP_VERSION, '5.4.0' ) >= 0 ? json_encode( $files, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) : str_replace( '\\/', '/', json_encode( $files ) );
		return 'var ipsJavascriptMap = ' . $json . ';';
	}
	...
	{
		$url  = ltrim( str_replace( \IPS\Settings::i()->base_url . 'applications', '', $url ), '/' );
		$bits = explode( "/", $url );
	...
	protected static function _sectionJs( $key, $location, $app )
	{
		...
		{
			$templates = array( str_replace( \IPS\ROOT_PATH, trim( \IPS\Settings::i()->base_url, '/' ), $templatesDir . '/ips.templates.' . $key . '.js' ) );
		}
