...
class _Member extends \IPS\Patterns\ActiveRecord
{
	...
	public function hideOrDeleteAllContent( $action, $extra=array() )
	{
		...
		/* Send to the queue */
		foreach ( \IPS\Content::routedClasses( FALSE, FALSE, FALSE ) as $class )
		{
			...
			try
			{
				...
				{					
					...
					if ( $row['item_app_key_1'] )
					{
						...
						{
							if( !isset( $this->markersResetTimes[ $app ][ $row['item_app_key_1'] ] ) OR !is_array( $this->markersResetTimes[ $app ][ $row['item_app_key_1'] ] ) )
							{
							...
							{
								if( !isset( $this->markersResetTimes[ $app ][ $row['item_app_key_1'] ][ $row['item_app_key_2'] ] ) OR !is_array( $this->markersResetTimes[ $app ][ $row['item_app_key_1'] ][ $row['item_app_key_2'] ] ) )
								{
	...
	{
		/* Only guests can be moderators of course */
		if ( !$this->member_id )
		{
			return FALSE;
		}
	...
	{
		if ( $member->group['gbw_cannot_be_ignored'] or !$this->member_id )
		{
			return FALSE;
		}
	...
	public function validate()
	{
		...
		foreach ( \IPS\Db::i()->select( '*', 'core_validating', array( 'member_id=?', $this->member_id ) ) as $row )
		{
			...
			if ( $row['new_reg'] )
			{
				...
				{
					/* @link http://community.---.com/4bugtrack/beta-3a-possible-problems-if-no-email-server-setup-for-new-registrations-r627/ */
	...
	public function checkPostsPerDay()
	{
		...
		if ( $count > $this->group['g_ppd_limit'] )
		{
			return FALSE;
		}
