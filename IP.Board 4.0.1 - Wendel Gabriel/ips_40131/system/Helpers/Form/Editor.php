...
class _Editor extends FormAbstract
{
		...
		foreach ( static::getAttachments( $existingAttachments, $this->postKey ) as $attachment )
		{
			try
			{
				...
				/* Reset the original filename based on the attachment record as filenames can be renamed if they contain special characters as AWS being the lowest common denominator cannot
				   handle special characters. @link https://community.---.com/4bugtrack/active-reports/408-wrong-name-for-uploaded-files-in-editor-r6519/ */
	...
	public static function getEmoticons()
	{
		...
		{
			$emoticons = iterator_to_array( \IPS\Db::i()->select( '*', 'core_emoticons', array( 'typed<>?', '' ) )->setKeyField('typed')->setValueField('image') );
			foreach( $emoticons as $key => $emoticon )
			{
				$emoticons[ $key ] = (string) \IPS\File::get( "core_Emoticons", $emoticon )->url;
			}
