...
class _Upload extends FormAbstract
{
	...
	public function __construct( $name )
	{
		...
		{
			$potentialValues[]	= (float) ini_get('upload_max_filesize');
		}

		if( (float) ini_get('post_max_size') > 0 )
		{
			$potentialValues[]	= (float) ini_get('post_max_size');
		}

		if( (float) ini_get('memory_limit') > 0 )
		{
			$potentialValues[]	= (float) ini_get('memory_limit');
		}

		$this->maxChunkSize = min( $potentialValues );
	...
	public function getValue()
	{
		...
		$return = array();
		$tempFiles = iterator_to_array( \IPS\Db::i()->select( 'id,contents', 'core_files_temp', array( 'upload_key=?', md5( $this->name . session_id() ) ) )->setKeyField('id')->setValueField('contents') );
		if ( $this->options['storageExtension'] )
		{
			...
			if ( isset( \IPS\Request::i()->$existingName ) and is_array( \IPS\Request::i()->$existingName ) )
			{
				foreach ( \IPS\Request::i()->$existingName as $id => $tempId )
				{
					if ( isset( $keep[ $id ] ) )
					{
						...
						{
							$file = \IPS\File::get( $this->options['storageExtension'], $tempFiles[ $tempId ] );
							$file->tempId = $tempId;
							$return[ $id ] = $file;
						}
						else
					...
					elseif ( !$this->options['retainDeleted'] )
					{
						if ( $tempId and isset( $tempFiles[ $tempId ] ) )
						{
							...
							try
							{
								\IPS\File::get( $this->options['storageExtension'], $tempFiles[ $tempId ] )->delete();
							}
							catch ( \Exception $e ) { }
						}
						else
