	...
	public function validate()
	{
		if ( $this->value['server'] or $this->value['un'] or $this->value['pw'] )
		{
			if ( $this->options['validate'] )
			{
				try
				{
					if ( $this->value['protocol'] == 'sftp' )
					{
						$ftp = new \IPS\Ftp\Sftp( $this->value['server'], $this->value['un'], $this->value['pw'], $this->value['port'] );
					}
					else
					{
						$ftp = new \IPS\Ftp( $this->value['server'], $this->value['un'], $this->value['pw'], $this->value['port'], ( $this->value['protocol'] == 'ssl_ftp' ) );
					}
					
					$ftp->chdir( $this->value['path'] );
				}
				...
				{
					// This means we tried an SFTP connection, but the server doesn't support it. We'll have to assume it's correct
				}
			}
		}
