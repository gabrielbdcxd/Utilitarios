...
class _Node extends FormAbstract
{
	 ...
	 * @code
	 	$defaultOptions = array(
	 		...
	 		'forceOwner'		=> \IPS\Member::load(),			// If nodes are 'owned' you can force a specific owner
	...
	public function html()
	{
		...
		if ( isset( \IPS\Request::i()->_nodeSelectName ) and  \IPS\Request::i()->_nodeSelectName === $this->name )
		{
			...
			switch ( \IPS\Request::i()->_nodeSelect )
			{
				case 'children':
					try
					{
						$node = $nodeClass::load( \IPS\Request::i()->_nodeId );
						
						$children = $node->children( NULL, NULL, $this->options['subnodes'], $disabled, $this->options['where'] );
					}
					catch ( \Exception $e )
					{
				...
				case 'parent':					
					try
					{
						...
						$children = $parent ? $parent->children( NULL, NULL, $this->options['subnodes'], $disabled, $this->options['where'] ) : $nodeClass::roots( NULL, NULL, $this->options['where'] );
					}
					catch ( \Exception $e )
					{
		...
		$children	= array();
		$noParentNodes = array();
		if( isset( $nodeClass::$ownerTypes ) and $nodeClass::$ownerTypes !== NULL )
		{
		...
		else
		{
			...
			if ( is_array( $this->value ) )
			{
				...
				{
					$title = isset( $node::$titleLangPrefix ) ? \IPS\Member::loggedIn()->language()->addToStack( $node::$titleLangPrefix . $node->_id, FALSE, array( 'json' => TRUE, 'escape' => TRUE ) ) : $node->_title;
					$selected[ !( $node instanceof $nodeClass ) ? "{$node->_id}.s" : $node->_id ] = array( 'title' => $title, 'parents' => array_values( array_map( function( $val ){ 
						return isset( $val::$titleLangPrefix ) ? \IPS\Member::loggedIn()->language()->addToStack( $val::$titleLangPrefix . $val->_id, FALSE, array( 'json' => TRUE, 'escape' => TRUE ) ) : $val->_title; 
			...
			elseif ( $this->value instanceof \IPS\Node\Model )
			{
				...
				$title = isset( $nodeClass::$titleLangPrefix ) ? \IPS\Member::loggedIn()->language()->addToStack( $class::$titleLangPrefix . $this->value->_id, FALSE, array( 'json' => TRUE, 'escape' => TRUE ) ) : $this->value->_title;
	 ...
	 * @param	array					$disabled	Disabled options
	...
	{
		if ( !$this->options['permissionCheck'] OR $node->can('view') )
		{
			if ( $noJS )
			{
