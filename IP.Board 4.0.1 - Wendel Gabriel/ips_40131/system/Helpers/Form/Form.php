...
class _Form
{
	...
	{
		$this->id = $id;
		$this->action = $action ?: \IPS\Request::i()->url();		
		$this->action = $this->action->stripQueryString('csrfKey');
		
		$this->attributes = $attributes;
		
		if( $submitLang )
		{
			$this->actionButtons[] = \IPS\Theme::i()->getTemplate( 'forms', 'core', 'global' )->button( $submitLang, 'submit', null, 'ipsButton ipsButton_primary', array( 'accesskey' => 's' ) );
		}
		...
		{
			$potentialMaxUploadValues[]	= (float) ini_get('upload_max_filesize');
		}
		if( (float) ini_get('post_max_size') > 0 )
		{
			$potentialMaxUploadValues[]	= (float) ini_get('post_max_size');
		}
		if( (float) ini_get('memory_limit') > 0 )
		{
			$potentialMaxUploadValues[]	= (float) ini_get('memory_limit');
		}
		$this->uploadField = min( $potentialMaxUploadValues ) * 1048576;
	}
