...
class _Dynamic extends \IPS\Helpers\Chart
{
	 ...
	 */
	public $availableTypes = array( 'LineChart', 'ColumnChart', 'BarChart', 'PieChart' );
		...
		try
		{
			...
			if ( $this->start )
			{
				...
				while ( $date->getTimestamp() < ( $this->end ? $this->end->getTimestamp() : time() ) )
				{
					...
					{
						case 'daily':
							$data[ $date->format( 'j n Y' ) ] = array();
	
							if( $this->groupBy )
							{
								foreach( $this->groupByKeys as $key )
								{
									$data[ $date->format( 'j n Y' ) ][ $key ]	= 0;
								}
							}
						...
						case 'weekly':
							$data[ $date->format( 'W Y' ) ] = array();
	
							if( $this->groupBy )
							{
								foreach( $this->groupByKeys as $key )
								{
									$data[ $date->format( 'W Y' ) ][ $key ]	= 0;
								}
							}
						...
						case 'monthly':
							$data[ $date->format( 'n Y' ) ] = array();
	
							if( $this->groupBy )
							{
								foreach( $this->groupByKeys as $key )
								{
									$data[ $date->format( 'n Y' ) ][ $key ]	= 0;
								}
							}
				...
				else
				{
					...
					foreach ( $data as $time => $d )
					{
						...
						switch ( $this->timescale )
						{
							...
							case 'weekly':
								$datetime->setISODate( $exploded[1], $exploded[0] );
								//$datetime = $datetime->localeDate();
								break;
