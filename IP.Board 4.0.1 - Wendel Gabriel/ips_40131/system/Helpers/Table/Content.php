...
class _Content extends Table
{
	...
	/**
	 * @brief	Honor the pinned flag for sorting. Default exclusions include vnc and activity streams
	 */
	...
	public function __construct( $class, \IPS\Http\Url $baseUrl, $where=NULL, \IPS\Node\Model $container=NULL, $includeHidden=NULL, $permCheck='view', $honorPinned=TRUE )
	{
		...
		if ( $container !== NULL )
		{
			...
			if ( $this->includeHiddenContent === NULL )
			{
			...
			/* Meta tags */
			\IPS\Output::i()->title = ( $this->page > 1 ) ? \IPS\Member::loggedIn()->language()->addToStack( 'title_with_page_number', FALSE, array( 'sprintf' => array( $container->_title, $this->page ) ) ) : $container->_title;
			\IPS\Output::i()->metaTags['title'] = $container->_title;
	...
	public function getRows( $advancedSearchValues )
	{
		...
		$this->sortBy = in_array( $this->sortBy, $this->sortOptions ) ? $this->sortBy : 
			( array_key_exists( $this->sortBy, $this->sortOptions) ? $this->sortOptions[ $this->sortBy ] : $defaultSort );
		...
		/* Get results */
		$it = $class::getItemsWithPermission( $where, $sortBy, array( ( $this->limit * ( $this->page - 1 ) ), $this->limit ), $this->permCheck, $this->includeHiddenContent, \IPS\Db::SELECT_SQL_CALC_FOUND_ROWS, NULL, $this->joinContainer, $this->joinComments, $this->joinReviews, FALSE, $this->joins );
		$this->count = $it->count( TRUE );
