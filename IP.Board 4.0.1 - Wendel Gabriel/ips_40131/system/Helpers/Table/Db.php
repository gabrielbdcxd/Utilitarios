...
class _Db extends Table
{
	...
	public function getRows( $advancedSearchValues )
	{
		...
		if ( $this->quickSearch !== NULL and \IPS\Request::i()->quicksearch )
		{
			...
			{
				$where[] = call_user_func( $this->quickSearch, \IPS\Request::i()->quicksearch );
			}
			else
			{
				...
				$where[] = array_merge( array( '(' . implode( ' OR ', $_where ) . ')' ), array_fill( 0, count( $_where ), mb_strtolower( \IPS\Request::i()->quicksearch ) ) );
			}
		}
		...
		if ( $this->group === null )
		{
			...
			$query = \IPS\Db::i()->select( '*' . ( count( $selects ) ? ', ' . implode( ', ', $selects ) : '' ), $this->table, $where, NULL, array( 0, 1 ) );
		}
		else
		{
			$query = \IPS\Db::i()->select( '*, count(DISTINCT( '.$this->table.'.' . $this->group . ')) as _count' . ( count( $selects ) ? ', ' . implode( ', ', $selects ) : '' ), $this->table, $where, NULL, array( 0, 1 ) );
		}
		...
		if ( count( $this->joins ) )
		{
			foreach( $this->joins as $join )
			{
				$query->join( $join['from'], ( isset( $join['where'] ) ? $join['where'] : null ), 'LEFT' );
			}
		}
		...
		$select = \IPS\Db::i()->select(
			...
			$where,
			$orderBy,
			array( ( $this->limit * ( $this->page - 1 ) ), $this->limit ), $this->group
		);
		...
		if ( count( $this->joins ) )
		{
			foreach( $this->joins as $join )
			{
				$select->join( $join['from'], $join['where'], 'LEFT' );
			}
		}
