...
class _Index extends \IPS\Content\Search\Index
{
	...
	public function index( \IPS\Content\Searchable $object )
	{
		...
		if ( $indexData )
		{
			...
			$data['index_permissions'] = json_encode( explode( ',', $data['index_permissions'] ) );
	...
	public function prune( \IPS\DateTime $cutoff = NULL )
	{
		if ( $cutoff === NULL )
		{
			...
			{
				/* If the index does not exist, we should not exit at this point in time */
			}
		}
		else
		{
	...
	public static function _getClassId( $class )
	{
		...
		/* Get the map (Older versions stored this in datastore, so we look there if we don't have it or use a blank array) */
		if ( !\IPS\Settings::i()->sphinx_id_map )
		{
			$map = isset( \IPS\Data\Store::i()->sphinxIdMap ) ? \IPS\Data\Store::i()->sphinxIdMap : array();
			$updateMap = TRUE;
		}
		else
		{
			$map = json_decode( \IPS\Settings::i()->sphinx_id_map, TRUE );
		}
		...
		if ( $updateMap )
		{
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => json_encode( $map ) ), array( 'conf_key=?', 'sphinx_id_map' ) );
