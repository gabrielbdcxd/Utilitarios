...
class _Lang extends \IPS\Node\Model
{
	 ...
	 * @return	void
	 */
	public static function copyCustom( $app, $key, $newKey )
	{
		$values = iterator_to_array( \IPS\Db::i()->select( 'lang_id, word_custom', 'core_sys_lang_words', array( 'word_app=? AND word_key=?', $app, $key ) )->setKeyField('lang_id')->setValueField('word_custom') );
		
		foreach( $values as $row )
		{
			static::saveCustom( $app, $newKey, $values );
		}
	}
	...
	public function get( $key )
	{
		...
		if ( $return !== NULL and is_array( $key ) and ( count( $return ) !== count( $key ) ) )
		{
			foreach( $key as $k )
			{
	...
	public function replaceWords( &$output )
	{
		...
		foreach ( $this->outputStack as $key => $values )
		{
				...
				if ( isset( $this->words[ $values[ 'key' ] ] ) )
				{
					...
					if ( isset( $values[ 'options' ][ 'sprintf' ] ) and $values[ 'options' ][ 'sprintf' ] !== null )
					{
						$sprintf = array_map(
							...
							{
								return htmlentities( trim( $val ), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', false );
							},
							( is_array(
            ...
            {
                $replacement = htmlentities( $replacement, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', false );
            }
		...
		/* We do this 3 times in case a replacement contains another replacement, etc. */
