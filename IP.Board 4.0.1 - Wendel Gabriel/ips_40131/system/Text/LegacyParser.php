...
{	
	/**
	 * Maximum number of embeds we will try.
	...
	{
		$obj = new self( $member, $allowHtml, $attachClass, $id1, $id2, $id3, $itemClass );
	...
	public function parse( $value )
	{
		...
		foreach( $matches[0] as $k => $m )
		{
			...
			if( ! mb_strpos( $matches[1][ $k ], \IPS\Settings::i()->base_url ) AND mb_strpos( $matches[0][ $k ], 'youtube.co' ) and ! mb_strpos( $matches[0][ $k ], 'nomediaparse' ) AND \IPS\Text\Parser::embeddableMedia( $matches[1][ $k ], TRUE ) )
			{
		...
		foreach( $matches[0] as $m )
		{
			...
			{
				$value = str_replace( $m, "<a href='{$m}'>{$m}</a>", $value );
			}
		}
		...
		$value = preg_replace_callback( "/\[attachment=(.+?):(.+?)\]/ims", function( $matches ) {
			...
			if( $attachment['attach_is_image'] )
			{
				...
				$return = "<a class='ipsAttachLink ipsAttachLink_image' href='{fileStore.core_Attachment}/{$attachment['attach_location']}'><img class='ipsImage ipsImage_thumbnailed' src='{fileStore.core_Attachment}/{$attachment['attach_thumb_location']}' data-fileid='" . \IPS\Settings::i()->base_url . "applications/core/interface/file/attachment.php?id={$attachment['attach_id']}'></a>";
			}
			else
			{
