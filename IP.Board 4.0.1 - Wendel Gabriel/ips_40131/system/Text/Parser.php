...
{	
	...
	public static function embeddableMedia( $url, $iframe=FALSE )
	{
		...
		if ( in_array( $domain, array( 'facebook.com', 'plus.google.com', 'gfycat.com' ) ) )
		{
			...
			/* Gfycat needs no iframe because it is just an iframe we're showing */
			if ( $domain == 'gfycat.com' )
			{
				try
				{
					...
					{
						return \IPS\Theme::i()->getTemplate( 'embed', 'core', 'global' )->gfycat( $data['gfyItem']['gfyName'], $data['gfyItem']['width'], $data['gfyItem']['height'] );
					}
				}
				catch ( \RuntimeException $e ) {}
			}
			
			/* But the others need to be in an iframe because they include <script> tags... */
			else
			{
				/* Show the iframe... */
				if ( !$iframe )
	...
	public function parse( $value )
	{
		...
		if ( !empty( $this->bbcode ) or !empty( $this->looseProfanity ) or !empty( $this->exactProfanity ) or !empty( $this->acronyms ) or $this->attachIds !== NULL )
		{
			...
			$value = str_ireplace( "[list][*]", "[list]</p><p>[*]", $value );
			$value = str_ireplace( "[list]<br>", "</p><p>[list]", $value );
		...
		$value = preg_replace( '#<([^>]+?)(href|src)=(\'|")%7B___base_url___%7D/#i', '<\1\2=\3<___base_url___>/', $value );
	...
	{		
		...
		else
		{
				...
				else
				{
					...
					try
					{
						...
						if ( !$link->isInternal )
						{
							...
							if( \IPS\Settings::i()->posts_add_nofollow and ( !\IPS\Settings::i()->posts_add_nofollow_exclude or !in_array( preg_replace( '/^www\./', '', $link->data['host'] ), json_decode( \IPS\Settings::i()->posts_add_nofollow_exclude, TRUE ) ) ) )
							{
								$rels[] = 'nofollow';
							}
						}
			...
			if ( static::$emoticons === NULL )
			{
				...
				foreach ( \IPS\Db::i()->select( 'image', 'core_emoticons' ) as $row )
				{
	...
	{
		return array(
			'ipsStyle', 'ipsStyle_underline', 'ipsStyle_center', 'ipsStyle_left', 'ipsStyle_right', 'ipsStyle_indent', 'ipsStyle_hr', 'ipsStyle_spoiler', 
			'ipsStyle_spoilerFancy', 'ipsStyle_spoilerFancy_inner', 'ipsStyle_spoilerFancy_text', 'ipsStyle_spoilerFancy_open',
			'ipsButton',
			'ipsCode', 'prettyprint', 'lang-auto', 'lang-javascript', 'lang-php', 'lang-css', 'lang-html',
			'ipsQuote','ipsQuote_citation', 'ipsQuote_closed', 'ipsQuote_open',
			'fa',
