...
class _Notification
{
	 ...
	 * Send Notification
	 *
	 * @return	array				$sentTo			Members notified
	 */
		...
		{			
			...
			if ( $this->item )
			{
				...
				if ( $item instanceof \IPS\Content\Item )
				{
					...
					if ( $member->isIgnoring( $item->author(), 'topics' ) )
					{
						$this->recipients->detach( $member );
						continue;
					}
			...
			if ( isset( $notificationPreferences[ $keyToCheck ] ) AND in_array( 'email', $notificationPreferences[ $keyToCheck ] ) )
			{
				$language = $member->language()->id;

				if ( !isset( $emails[ $language ] ) )
				{
					$emails[ $language ] = \IPS\Email::buildFromTemplate( $this->app->directory, 'notification_' . $this->key, $this->emailParams, TRUE );
				}
				...
				$emails[ $language ]->buildUnsubscribe( $this->unsubscribeType, $this->key );
				$emailRecipients[ $language ][ $member->email ] = array(
					'recipient_name'	=> $member->name,
					'recipient_photo'	=> (string) $member->photo,
					'unsubscribe_blurb'	=> $unsubscribeBlurb,
					'unfollow_link'		=> $unfollowLink
				);
				
				$sentTo[ $member->member_id ] = $member->member_id;
			}

			/* Send inline notifications, but only if this is not for report center.  Report center is treated special and the 'inline' notification
				preference actually instead controls whether the bubble should be shown on the report center icon at the top or not */
			if ( $this->key != 'report_center' AND isset( $notificationPreferences[ $keyToCheck ] ) AND in_array( 'inline', $notificationPreferences[ $keyToCheck ] ) )
			{
				if ( $this->item )
				{
					try
				...
				foreach( $this->emailParams AS $param )
				{
					if ( $param instanceof \IPS\Content )
					{
						 ...
						 * @see <a href='http://community.---.com/resources/bugs.html/_/4-0-0/reply-notifications-missing-names-r46841'>Bug Report</a>
						 */
	...
	protected function sendEmails( $emails, $emailRecipients )
	{
		foreach ( $emails as $languageId => $email )
		{
			...
			{
				$email->mergeAndSend( $emailRecipients[ $languageId ] );
			}
		}
	}
}