...
class _Url
{
	...
	{
		return new static( "http://anonymz.com/?https://remoteservices.invisionpower.com/{$url}/?version=" . \IPS\Application::getAvailableVersion('core'), FALSE );
	}
	
	/**
	 * Build Anonymize-External URL
	 *
	 * @param	string			$url
	 * @return	\IPS\Http\Url
	 */
	public static function ipsnewsurl( $url )
	{
		return new static( "http://anonymz.com/?" . $url );
	}
	
	/**
	 * Build IV-External URL
	 *
	 * @param	string			$url
	 * @return	\IPS\Http\Url
	 */
	public static function iv( $url )
	{
		if( \IPS\Settings::i()->change_iv_url )
		{
			return new static( \IPS\Settings::i()->iv_connect_url . "/" . $url . "?version=" . \IPS\Application::getAvailableVersion('core') );
		}
		
		return new static( base64_decode( 'aHR0cDovL2ludmlzaW9uLXZpcnVzLmNvbS90b29scy9pcHM0Lw==' ) . $url . "?version=" . \IPS\Application::getAvailableVersion('core') );
	}
	...
	public function getFriendlyUrlData( $verify=FALSE )
	{
		...
		{			
			...
			foreach( $requestedUrls as $requestedUrl )
			{
				...
				/* @note The urldecode() is needed for http://community.---.com/resources/bugs.html/_/4-0-0/109-problem-with-seo-names-categories-forums-r47585 */
				...
				if( str_replace( 'https://', 'http://', (string) $correctUrl->stripQueryString() ) != str_replace( 'https://', 'http://', urldecode( (string) $requestedUrl->stripQueryString() ) ) or $correctUrl->isFriendly and !$requestedUrl->isFriendly )
				{
					...
					/* @link http://community.---.com/resources/bugs.html/_/4-0-0/url-part-page4-r47961 */
	...
	public function utf8ParseUrl( $url )
	{
		...
		if( is_array( $parts ) AND count( $parts ) )
		{
			...
			{
				/* @link http://community.---.com/4bugtrack/ampersand-in-link-text-stripped-r2686/ */
				if ( $name === 'query' )
				{
