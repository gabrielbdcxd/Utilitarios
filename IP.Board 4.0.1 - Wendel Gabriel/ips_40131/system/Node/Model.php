...
abstract class _Model extends \IPS\Patterns\ActiveRecord
{
	...
	public static function loadByOwner( $member=NULL, $where=array() )
	{
		...
		/* Check the cache first */
		if( isset( static::$rootsResult[ md5( get_called_class() . $member->member_id ) ] ) )
		{
			return static::$rootsResult[ md5( get_called_class() . $member->member_id ) ];
		}
		...
		/* Set cache */
		static::$rootsResult[ md5( get_called_class(). $member->member_id ) ] = $nodes;
	...
	public static function populatePermissionMatrix( &$rows, $node, $group, $current, $level=0 )
	{
		...
		{
			$rows[ $node->_id ] = array_merge( $rows[ $node->_id ], array( static::$permissionLangPrefix . 'perm__' . $k => ( isset( $current[ $node->_id ] ) ) AND ( $current[ $node->_id ]['perm_' . $v ] === '*' OR in_array( $group, explode( ',', $current[ $node->_id ]['perm_' . $v ] ) ) ) ) );
		}
		...
		if ( $node->hasChildren( NULL ) === TRUE )
		{
			...
			{
				self::populatePermissionMatrix( $rows, $child, $group, $current, $level );
			}
			$level--;
		}
	}
	...
	{ 
		...
		else
		{
			...
			else
			{
				...
				/* This is silly, but I encountered a very strange eaccelerator bug in ticket 922529 where concatenating the data caused 
					$contentItemClass::$databaseColumnMap['author'] to evaluate as $commentClass::$databaseColumnMap['author'], resulting in DB errors */
				$contentItemColumn = $contentItemClass::$databaseTable . '.' . $contentItemClass::$databasePrefix . $contentItemClass::$databaseColumnMap['author'];
				$where[] = array( '(' . $commentClass::$databaseTable . '.' . $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['author'] . '=? OR ' . $contentItemColumn . '=?)', $member->member_id, $member->member_id );
			}
			...
			$joinClause = array( $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['item'] . '=' . $contentItemClass::$databasePrefix . $contentItemClass::$databaseColumnId );
	...
	public function deleteOrMoveForm( $showMoveToChildren=FALSE )
	{
		...
		if ( $hasContent )
		{
			...
			if ( \IPS\Request::i()->deleteNode != 1 )
			{
				...
				{
					$form->add( new \IPS\Helpers\Form\Select( 'node_move_state', 'any', FALSE, array( 'options' => array( 'open' => 'open', 'locked' => 'locked', 'any' ) ) ) );
				}
	...
	/**
	 * Form to delete content.
	 * Used when there is no content to remove
	 *
	 * @return	\IPS\Helpers\Form
	 */
	...
	public function averageRating()
	{
		...
		elseif ( isset( static::$ratingColumnMap['rating_total'] ) and isset( static::$ratingColumnMap['rating_hits'] ) )
		{
			...
			return $this->$hits ? round( $this->$total / $this->$hits ) : 0;
		}
		else
		{
			$idColumn = static::$databaseColumnId;
