...
class _Sftp extends \IPS\Ftp
{
	...
	public function __construct( $host, $username, $password, $port=22 )
	{
		...
		$this->ssh2 = ssh2_connect( $host, $port );		
		if ( $this->ssh2 === FALSE )
		{
			throw new \IPS\Ftp\Exception( 'COULD_NOT_CONNECT' );
		}
		
		if ( !@ssh2_auth_password( $this->ssh2, $username, $password ) )
		{
	...
	{
		if ( !@ssh2_scp_send( $this->ssh2, $file, $this->dir . $filename ) )
		{
	...
	public function download( $filename, $target=NULL )
	{
		...
		if ( !@ssh2_scp_recv( $this->ssh2, $this->dir . $filename, $target ) )
		{
