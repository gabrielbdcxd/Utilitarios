...
class _Theme extends \IPS\Theme
{
			...
			if ( $file === 'custom' and $app === 'core' )
			{
				foreach ( \IPS\Plugin::plugins() as $plugin )
				{
					if ( $plugin->enabled )
					{
						...
						{
							$return[] = str_replace( \IPS\ROOT_PATH, rtrim( \IPS\Settings::i()->base_url, '/' ), $file );
						}
					}
				}
			}
						...
						if ( $group->isDir() )
						{
							foreach( new \DirectoryIterator( $path . $location->getFilename() . '/' . $group->getFilename() ) as $file )
							{
								...
								if ( $id > 0 )
								{
									...
									{
										if ( md5( trim( $currentTemplates[ $app ][ $location->getFilename() ][ $group->getFilename() ][ $name ]['template_content'] ) ) == md5( trim( $html ) ) )
										{
											/* No change  */
											continue;
										}
		...
		foreach( new \DirectoryIterator( $root . $path ) as $file )
		{
			...
			else
			{
				...
				if ( $id > 0 )
				{
					...
					{
						if ( md5( trim( $currentCss[ $app ][ $location ][ $finalPath ][ $file->getFilename() ]['css_content'] ) ) == md5( trim( $css ) ) )
						{
							/* No change  */
							continue;
						}
	 ...
	 * @return	void
	 */
	protected static function _importDevResources( $app, $id, $location, $path='/' )
	{
		...
		foreach( new \DirectoryIterator( $root . $path ) as $file )
		{
			...
			else
			{
				...
				{
					\IPS\Log::i( LOG_INFO )->write( $root . $path . $file->getFilename() . " too large to import", 'designers_mode_import' );
					continue;
				}
				...
				{
					\IPS\Log::i( LOG_INFO )->write( $root . $path . $file->getFilename() . " could not be saved correctly", 'designers_mode_import' );
					continue;
				}
				...
				if ( ! isset( $master[ $file->getFilename() ] ) or $master[ $file->getFilename() ] != md5( $content ) )
				{
					$custom = 1;
				}
