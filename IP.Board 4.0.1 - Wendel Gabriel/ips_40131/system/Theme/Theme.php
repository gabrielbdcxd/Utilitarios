...
class _Theme extends \IPS\Node\Model
{
	 ...
	 */
	public static $defaultThemeSet = 0;
			...
			if ( static::$memberTheme === NULL )
			{
				...
				if ( \IPS\Dispatcher::hasInstance() and \IPS\Dispatcher::i()->controllerLocation == 'front' )
				{
					...
					if ( $setId )
					{
						...
						else if ( static::load( $setId )->canAccess() !== true )
						{
							$setId = NULL;
						}
					}
	...
	public static function themes()
	{
		if ( !static::$gotAll )
		{
			...
			foreach( $rows as $id => $theme )
			{
				...
				{
					static::$defaultThemeSet = $theme['set_id'];
				}
	...
	{
		static::themes();
		$default = static::$multitons[ static::$defaultThemeSet ];
	...
	public static function switchTheme( $themeId )
	{
		...
		try
		{
			static::$memberTheme = static::load( $themeId );
	...
	public function getTemplate( $group, $app=NULL, $location=NULL )
	{
		...
		if ( !in_array( $key, array_keys( static::$calledTemplates ) ) )
		{
			...
			if ( !isset( \IPS\Data\Store::i()->$key ) )
			{
				...
				{
					/* Rebuild in progress */
					\IPS\Log::i( LOG_WARNING )->write( "Template store key: {$key} rebuilding and requested again ({$app}, {$location}, {$group})", "template_store_building" );
			...
			/* If you want to see where the error is inside eval, uncomment below */
			#error_reporting( E_ALL );
			#$filePath = \IPS\ROOT_PATH . '/uploads/' . mt_rand() . '.php';
	...
	{		
		if( $this->author_name )
		{
			return \IPS\Theme::i()->getTemplate( 'customization', 'core' )->themeDescription( $this->author_name, $this->author_url );
		}
		...
		if ( static::designersModeEnabled() )
		{
			if ( $this->is_default )
			{
				$badge	= array(
					...
					1	=> \IPS\Member::loggedIn()->language()->addToStack('theme_is_default_with_id', FALSE, array( 'sprintf' => array( $this->id ) ) )
				);
			}
			else
			{
	...
	public function form( &$form )
	{
		...
		if ( \IPS\IN_DEV OR \IPS\Theme::designersModeEnabled() )
		{
			...
			$form->add( new \IPS\Helpers\Form\Text( 'theme_template_export_version'        , $this->version     , false, array( 'placeholder' => '1.0.0' ) ) );
			$form->add( new \IPS\Helpers\Form\Number( 'theme_template_export_long_version' , $this->long_version, false ) );
		}
		...
		$form->add( new \IPS\Helpers\Form\Upload( 'core_theme_set_logo', ( isset( $this->logo['front']['url'] ) ? \IPS\File::get( 'core_Theme', $this->logo['front']['url'] ) : NULL ), FALSE, array( 'image' => true, 'storageExtension' => 'core_Theme', 'storageContainer' => 'set_resources_' . $this->id ), NULL, NULL, NULL, 'core_theme_set_logo' ) );

		/* SHARER LOGO */
		$form->addHeader( 'core_theme_set_sharer_logo_manage' );
			
		$form->add( new \IPS\Helpers\Form\Upload( 'core_theme_set_sharer_logo', ( isset( $this->logo['sharer']['url'] ) ? \IPS\File::get( 'core_Theme', $this->logo['sharer']['url'] ) : NULL ), FALSE, array( 'image' => true, 'storageExtension' => 'core_Theme', 'storageContainer' => 'set_resources_' . $this->id ), NULL, NULL, NULL, 'core_theme_set_sharer_logo' ) );
			
		/* FAVICO LOGO */
		$form->addHeader( 'core_theme_set_favico_logo_manage' );
		
		$form->add( new \IPS\Helpers\Form\Upload( 'core_theme_set_favico_logo', ( isset( $this->logo['favicon']['url'] ) ? \IPS\File::get( 'core_Theme', $this->logo['favicon']['url'] ) : NULL ), FALSE, array( 'allowedFileTypes' => array('ico'), 'storageExtension' => 'core_Theme', 'storageContainer' => 'set_resources_' . $this->id ), NULL, NULL, NULL, 'core_theme_set_favico_logo' ) );
		...
		$form->addMessage('core_theme_set_editor_blurb');
		...
		if ( $this->id )
		{
			...
			foreach( $customTabs as $tabKey => $data )
			{
				...
				foreach( $data as $row )
				{
					$form->add( $this->getCustomSettingField( $row ) );
				}
			}
		}
	}
		...
		{					
			...
			switch ( $row['sc_type'] )
			{
				...
				case 'Upload':
					...
					$options['storageContainer'] = 'set_resources_' . $this->id;
	...
	public function getButtons( $url, $subnode=FALSE )
	{
		...
		{
			foreach( array( 'delete', 'copy', 'convert_to_full', 'upload', 'download', 'resources', 'edit_templates' ) as $btn )
			{
	...
	public function saveForm( $values )
	{
		...
		if ( $changedSettings )
		{
			...
			foreach( $themeSetToBuild->children() as $child )
			{
	...
	public function copyResourcesFromSet( $id=0 )
	{
		...
		{
			$key = self::makeBuiltTemplateLookupHash($data['resource_app'], $data['resource_location'], $data['resource_path']) . '_' . $data['resource_name'];
...
EOF;
	...
	public function getRawTemplates( $app=array(), $location=array(), $group=array(), $returnType=null, $returnThisSetOnly=false )
	{
		...
		foreach(
			...
			else if ( ! $row['template_user_added'] and $row['template_set_id'] != $this->_id )
			{
				$row['InheritedValue'] = 'inherit';
			}
			else if ( $row['template_user_added'] and $row['template_set_id'] != $this->_id )
	...
	public function saveSet( $data=array() )
	{
		...
		$save['set_long_version'] = ( isset( $save['set_long_version'] ) ) ? (int) $save['set_long_version'] : 0;
	...
	public static function deleteCompiledTemplate( $app=null, $location=null, $group=null, $themeId=null )
	{
		...
		if ( ! empty( $themeId ) )
		{
			...
			foreach( $themeSet->children() as $child )
			{
				$themeSets[] = $child->_id;
			}
			
			$where[] = array( \IPS\Db::i()->in( 'template_set_id', $themeSets ) );
		}
		...
		foreach(
			...
			{
				if ( $themeId === null OR in_array( $id, $themeSets ) )
				{
	...
	public static function deleteCompiledCss( $app=null, $location=null, $path=null, $name=null, $themeId=null )
	{
		...
		if ( $themeId !== null )
		{
			...
			$where[] = array( \IPS\Db::i()->in( 'css_set_id', $themeSets ) );
		}
	...
	public static function deleteCompiledResources( $app=null, $location=null, $path=null, $name=null, $themeId=null )
	{
		...
		if ( $themeId !== null )
		{
			$query[] = \IPS\Db::i()->in( 'resource_set_id', $themeSets );
		}
	...
	public static function runProcessFunction( $content, $functionName )
	{
		...
		/* Make it */
		if ( \IPS\IN_DEV and \IPS\DEV_DEBUG_TEMPLATES )
		{
	...
	public static function expandShortcuts( $content )
	{
		...
		{
			$content = preg_replace( '/(^|[^\$\\\])' . $class . "\.(.+?)/", '$1\IPS\\' . ucfirst( $class ) . '::' . $function . '()->$2', $content );
			
			/* Now remove the \ before the class name for literal strings */
	...
	{{if !\$licenseData = \IPS\IPS::licenseKey() or !isset(\$licenseData['products']['copyright']) or !\$licenseData['products']['copyright']}}<a title='Community Software by Invision Power Services, Inc.' href='#'>Community Software by Invision Power Services, Inc.</a>{{endif}}
</p>";
		}
...
CONTENT;\n
		...
		{
			\IPS\Log::i( \LOG_ERR )->write( "{$functionName} prevented from running-\n\n" . get_class( \$exception ) . "\n" . \$exception->getCode() . ": " . \$exception->getMessage() . "\n" . \$exception->getTraceAsString() );
		}
		return \$return;
}
PHP;
...
PHP;
			...
			if ( $isHTML === TRUE )
			{
				...
				foreach ( $varMatches as $id => $var )
				{
					...
					else
					{
						...
						{
							$replace = "\nCONTENT;\n\$return .= htmlentities( \$" . str_ireplace( '|doubleencode', '', $var[1] ) . ", ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', TRUE );\n\$return .= <<<CONTENT\n";
						}
						else
						{
							$replace = "\nCONTENT;\n\$return .= htmlentities( \$" . $var[1] . ", ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE );\n\$return .= <<<CONTENT\n";
						}
	...
	public static function addResource( $data, $addToMaster=FALSE )
	{
		...
		foreach( static::themes() as $id => $theme )
		{
			...
			$key = self::makeBuiltTemplateLookupHash($data['app'], $data['location'], $data['path']) . '_' . $data['name'];
	...
	public static function addCss( $data )
	{
		...
		/* Check for existing */
		$check = \IPS\Db::i()->select( 'css_id', 'core_theme_css', array(
		...
		$insertId = \IPS\Db::i()->insert( 'core_theme_css', array(
				'css_set_id'	 => isset( $data['set_id'] ) ? $data['set_id'] : 0,
				...
				'css_unique_key' => md5( '0;' . mb_strtolower( $data['app'] ) . ';' . mb_strtolower( $data['location'] ) . ';' . mb_strtolower( $data['path'] ) . ';' . mb_strtolower( $data['name'] ) )
		) );
