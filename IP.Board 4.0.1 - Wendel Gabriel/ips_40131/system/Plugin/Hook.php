...
class _Hook extends \IPS\Patterns\ActiveRecord
{
		...
		switch ( \IPS\Request::i()->hookTable )
		{
			...
			case 'add':
				...
				if ( $values = $form->values() )
				{
					...
					\file_put_contents( $hookDir . "/{$hook->filename}.php", "//<?php\n\n{$abstract}class {$classname} extends _HOOK_CLASS_\n{\n\n\n}" );
	...
	public static function writeDataFile()
	{
		...
		$json = json_encode( $hooks );
		$encrypted = '';
		$time = date( 'jny' );
		for ( $i = 0; $i < \strlen( $json ); $i++ )
		{
			$encrypted .= chr( ord( \substr( $json, $i, 1 ) ) + \substr( $time, ( $i % \strlen( $time ) ) - 1, 1 ) );
		}

		\file_put_contents( \IPS\ROOT_PATH . '/plugins/hooks.txt', $encrypted );
	}
