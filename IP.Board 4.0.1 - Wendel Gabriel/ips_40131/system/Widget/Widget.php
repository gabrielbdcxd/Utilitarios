...
abstract class _Widget
{
	...
	public function getTemplateLocation()
	{
		...
		{
			$bits = explode( '\\', $class );
			list( $class, $app, $location, $group ) = explode( '_', $bits[2] );

			$params = array( 'app' => $app, 'location' => $location, 'group' => $group );
		}
	...
	public static function devTable( $json, $url, $widgetDirectory, $subpackage, $namespace, $version, $appKeyOrPluginId )
	{
		...
		switch ( \IPS\Request::i()->widgetTable )
		{
			case 'form':
				...
				$form->add( new \IPS\Helpers\Form\Text( 'dev_widget_key', $current ? $current['dev_widget_key'] : NULL, TRUE, array( 'maxLength' => 255 ), function( $val ) use ( $current )
				{
