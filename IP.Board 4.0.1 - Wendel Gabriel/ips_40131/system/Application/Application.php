...
class _Application extends \IPS\Node\Model
{
	...
	public static function applications()
	{
		if( self::$gotAll === FALSE )
		{
			...
			self::$multitons = array();
			
			foreach ( $rows as $row )
			{
				try
				{
					self::$multitons[ $row['app_directory'] ] = static::constructFromData( $row );
				}
...
EOF;
	...
	public function modules( $location=NULL )
	{
		...
		if( $this->modules === NULL )
		{
			...
			$this->modules = $modules[ $this->directory ];
		}
			...
			foreach ( $dir as $file )
			{
				...
				if ( !$file->isDir() and !$file->isDot() and mb_substr( $file, -4 ) === '.php' AND mb_substr( $file, 0, 2 ) != '._' )
				{
					...
					else
					{
						try
						{
							$classes[ mb_substr( $file, 0, -4 ) ] = new $classname;
						}
	...
	public function getButtons( $url, $subnode=FALSE )
	{
		...
		/* Default */
		if( $this->directory != 'core' and $this->enabled )
		{
	...
	public function installDatabaseUpdates( $version=0, $lastJsonIndex=0, $limit=50, $return=FALSE )
	{
		...
		if( file_exists( \IPS\ROOT_PATH . "/applications/{$this->directory}/setup/upg_{$version}/queries.json" ) )
		{
			...
			foreach( $schema as $jsonIndex => $instruction['params'] )
			{
				...
				if( $return === TRUE )
				{
					if( isset( $tableCounts[ $_table ] ) AND $tableCounts[ $_table ] > \IPS\UPGRADE_MANUAL_THRESHOLD AND !in_array( $instruction['params']['method'], array( 'dropTable', 'insert', 'renameTable' ) ) )
					{
						\IPS\Log::i( LOG_DEBUG )->write( "Big table " . $_table . ", storing query to run manually", 'upgrade' );
				...
				catch( \IPS\Db\Exception $e )
				{
					...
					else if( !in_array( $e->getCode(), array( 1007, 1008, 1050, 1060, 1061, 1062, 1091 ) ) )
					{
						throw $e;
					}
				}
	...
	public function installTasks()
	{
		...
		{
			$currentTasks = iterator_to_array( \IPS\Db::i()->select( '*', 'core_tasks', array( 'app=?', $this->directory ) )->setKeyField( 'key' )->setValueField( 'frequency' ) );

			foreach ( array_diff_key( json_decode( file_get_contents( \IPS\ROOT_PATH . "/applications/{$this->directory}/data/tasks.json" ), TRUE ), $currentTasks ) as $key => $frequency )
			{
	...
	public function installEmailTemplates()
	{
		...
		{
			/* First, delete any existing email templates for this app. Even customized templates are associated by ID and will be deleted. */
		...
		if( file_exists( \IPS\ROOT_PATH . "/applications/{$this->directory}/data/theme.xml" ) )
		{
			...
			while( $xml->read() )
			{
				...
				if( $xml->name == 'template' )
				{
					...
					try
					{
						\IPS\Theme::addTemplate( $template );
					}
				...
				else if( $xml->name == 'resource' )
				{
					...
					\IPS\Theme::addResource( $resource, TRUE );
				}
	...
	public function buildThemeTemplates()
	{
		...
		$xml->text( "http://invision-virus.com" );
		$xml->endAttribute();
	...
	{
		foreach( \IPS\Db::i()->select( '*','core_email_templates', NULL, 'template_parent DESC', NULL, 'template_key' ) as $template )
		{
