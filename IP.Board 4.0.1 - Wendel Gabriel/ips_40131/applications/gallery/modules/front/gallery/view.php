...
class _view extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		elseif( \IPS\Request::i()->isAjax() && isset( \IPS\Request::i()->browse ) )
		{
			...
			{
				$return['comments'] = \IPS\Theme::i()->getTemplate( 'view' )->comments( $this->image );
			}
		...
		else
		{
			...
			\IPS\Output::i()->jsFiles = array_merge( \IPS\Output::i()->jsFiles, \IPS\Output::i()->js( 'front_view.js', 'gallery' ) );
			\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'view' )->image( $this->image, $slider, $this->image->nextItem(), $this->image->prevItem(), $hasNext, $hasPrev );
		}
	}
