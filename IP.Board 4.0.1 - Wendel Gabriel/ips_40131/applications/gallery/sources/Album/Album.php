...
class _Album extends \IPS\Node\Model implements \IPS\Node\Ratings, \IPS\Content\Embeddable
{
	...
	public static function formFields( \IPS\gallery\Album $album = NULL, $forOther = FALSE, $required = TRUE )
	{
		...
		$return[] = new \IPS\Helpers\Form\Select( 'album_sort_options', ( $album and $album->sort_options ) ? $album->sort_options : 'updated', FALSE, array( 'options' => array( 'updated' => 'sort_updated', 'rating' => 'sort_rating', 'num_comments' => 'sort_num_comments' ) ), NULL, NULL, NULL, 'album_sort_options' );
	...
	public function formatFormValues( $values )
	{
		...
		if( isset( $values['set_album_owner'] ) )
		{
			...
			if( isset( $values['album_owner'] ) )
			{
				unset( $values['album_owner'] );
			}
		}
		else if( isset( $values['album_owner'] ) )
		{
	...
	public function url()
	{
		if( $this->_url === NULL )
		{
			$this->_url = \IPS\Http\Url::internal( "app=gallery&module=gallery&controller=browse&album={$this->_id}", 'front', 'gallery_album', $this->name_seo );
		}
