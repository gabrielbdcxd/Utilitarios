...
namespace IPS\gallery\setup\upg_100011;
...
class _Upgrade
{
	...
	public function step1()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'gallery_categories', NULL, 'category_id ASC', array( $limit, $perCycle ) ) as $category )
		{
	...
	public function step1CustomTitle()
	{
		...
		{
			$_SESSION['_step1Count'] = \IPS\Db::i()->select( 'COUNT(*)', 'gallery_categories' )->first();
		}

		return "Rebuilding last poster data for gallery categories (Upgraded so far: " . ( ( $limit > $_SESSION['_step1Count'] ) ? $_SESSION['_step1Count'] : $limit ) . ' out of ' . $_SESSION['_step1Count'] . ')';
	}
}
