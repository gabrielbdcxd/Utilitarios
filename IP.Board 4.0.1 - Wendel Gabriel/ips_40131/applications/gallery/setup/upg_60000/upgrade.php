...
class _Upgrade
{
	...
	public function step1()
	{
		...
		{
			\IPS\Lang::saveCustom( 'gallery', "gallery_category_{$category['category_id']}", html_entity_decode( $category['category_name'], ENT_QUOTES, 'UTF-8' ) );
			\IPS\Lang::saveCustom( 'gallery', "gallery_category_{$category['category_id']}_desc", html_entity_decode( $category['category_description'], ENT_QUOTES, 'UTF-8' ) );

			$_rules	= unserialize( $category['category_rules'] );

			\IPS\Lang::saveCustom( 'gallery', "gallery_category_{$category['category_id']}_rulestitle", html_entity_decode( $_rules['title'], ENT_QUOTES, 'UTF-8' ) );
	...
	{
		\IPS\Db::i()->insert( 'core_ratings', 
			\IPS\Db::i()->select( "NULL, 'IPS\\gallery\\Image', rate_type_id, rate_member_id, rate_rate, ''", 'gallery_ratings', array( 'rate_type=?', 'image' ), NULL, NULL, array( 'rate_type', 'rate_type_id', 'rate_member_id' ) )
		);

		\IPS\Db::i()->insert( 'core_ratings', 
			\IPS\Db::i()->select( "NULL, 'IPS\\gallery\\Album', rate_type_id, rate_member_id, rate_rate, ''", 'gallery_ratings', array( 'rate_type=?', 'album' ), NULL, NULL, array( 'rate_type', 'rate_type_id', 'rate_member_id' ) )
		);
	...
	public function step3()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'gallery_images', NULL, 'image_id ASC', array( $limit, $perCycle ) ) as $row )
		{
			...
			$prefix	= $row['image_directory'] . '/';
			$hasSmall	= FALSE;

			if( file_exists( $settings['gallery_images_path'] . '/' . $row['image_directory'] . '/' . 'sml_' . $row['image_masked_file_name'] ) )
			{
				$hasSmall = TRUE;
			}
			...
			{
				$imageUpdate['image_metadata']	= json_encode( unserialize( $row['image_metadata'] ) );
			}

			/* Simply rebuild the image size values */
			$large	= @getimagesize( str_replace( $prefix, $config->configuration['dir'] . '/' . $row['image_directory'] . '/', $imageUpdate['image_masked_file_name'] ) );
			$medium	= @getimagesize( str_replace( $prefix, $config->configuration['dir'] . '/' . $row['image_directory'] . '/', $imageUpdate['image_medium_file_name'] ) );
			$small	= @getimagesize( str_replace( $prefix, $config->configuration['dir'] . '/' . $row['image_directory'] . '/', $imageUpdate['image_small_file_name'] ) );
			$thumb	= @getimagesize( str_replace( $prefix, $config->configuration['dir'] . '/' . $row['image_directory'] . '/', $imageUpdate['image_thumb_file_name'] ) );
			...
			if( $row['image_notes'] )
			{
				...
				$imageUpdate['image_metadata']	= json_encode( $notes );
			}
		...
		else
		{
			...
			if( isset( $settings['gallery_watermark_path'] ) AND $settings['gallery_watermark_path'] )
			{
				...
				try
				{
					...
					\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => $url ), array( 'conf_key=?', 'gallery_watermark_path' ) );
				}
				catch( \Exception $e )
				{
					\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => '' ), array( 'conf_key=?', 'gallery_watermark_path' ) );
				}
			}

			\IPS\Db::i()->dropColumn( 'gallery_images', array( 'image_thumbnail', 'image_directory', 'image_media_thumb' ) );

			\IPS\Db::i()->delete( 'core_sys_conf_settings', "conf_key IN ( 'gallery_watermark_path', 'gallery_watermark_opacity', 'gallery_images_url', 'gallery_images_path' )" );

			unset( \IPS\Data\Store::i()->settings, $_SESSION['_step3Count'] );
	...
	public function step5()
	{
		...
		if ( count( $toRun ) )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'gallery', 'extra' => array( '_upgradeStep' => 6 ) ) );
