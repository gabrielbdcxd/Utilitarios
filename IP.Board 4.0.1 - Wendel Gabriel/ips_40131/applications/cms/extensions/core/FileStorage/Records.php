...
class _Records
{
	...
	public function count()
	{
		...
		{
			$where = array( array( 'record_image IS NOT NULL' ) );
			
			if ( isset( $fields[ $id ] ) )
			{
				foreach( $fields[ $id ] as $field_id )
				{
					$where[] = array( 'field_' . $field_id . ' IS NOT NULL' );
				}
			}
			
			$count += \IPS\Db::i()->select( 'COUNT(*)', 'cms_custom_database_' . $id, $where )->first();
		}
			...
			if ( is_array( $fields ) )
			{
				foreach( $fields as $databaseId => $data )
				{
					foreach( $data as $fieldId )
					{
						try
						{
							$record	= \IPS\Db::i()->select( '*', 'cms_custom_database_' . $databaseId, array( 'field_' . $fieldId .'=?', (string) $file ) )->first();
