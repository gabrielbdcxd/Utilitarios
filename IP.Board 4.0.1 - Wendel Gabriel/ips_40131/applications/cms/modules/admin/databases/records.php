...
{	
	...
	protected function manage()
	{
		...
		$table->advancedSearch = array(
				'category_id'	=> array( \IPS\Helpers\Table\SEARCH_NODE, array(
						'class'		      => '\IPS\cms\Categories',
						'disabled'	      => false,
						'zeroVal'         => 'content_db_table_as_no_cat',
						'permissionCheck' => function( $node )
						{					
							return $node->database_id == \IPS\Request::i()->database_id;
						}
					),
				),
	...
	public function delete()
	{
		...
		try
		{
			$recordClass::load( \IPS\Request::i()->id )->delete();
		}
		catch( \OutofRangeException $ex )
		{
			\IPS\Output::i()->error( 'no_permission', '2T253/1', 403, '' );
		}	
	...
	protected function select()
	{
		...
		if ( $values = $form->values() )
		{
			if ( $move and $record )
			{
				$record->category_id = $values['category']->id;
				$record->save();
	...
	public function form()
	{
		...
		$customFields = $fieldsClass::fields( $customValues, 'view', $container );
		...
		$form->add( new \IPS\Helpers\Form\Radio( 'record_author_choice', $current ? ( $current->member_id === \IPS\Member::loggedIn()->member_id ? 'me' : 'notme' )  : 'me', FALSE, array(
				'options' => array(
		...
		$form->add( new \IPS\Helpers\Form\Member( 'record_member_id', ( $current ? \IPS\Member::load( $current->member_id ) : NULL ), FALSE, array(), NULL, NULL, NULL, 'record_member_id' ) );
		...
		if ( $values = $form->values() )
		{
			...
			if ( empty( $current ) )
			{
				...
				{
					if ( ucfirst( $field->type ) === 'Editor' )
					{
						\IPS\File::claimAttachments( 'RecordField_'. $field->id , $current->id, NULL, 'RecordField_'. $field->id, FALSE );
					}
				}
			}
			...
			$changeAuthor = FALSE;
			if ( ! empty( \IPS\Request::i()->id ) and $values['record_author_choice'] === 'notme' and ! empty( $values['record_member_id'] ) )
			{
				$changeAuthor = TRUE;
			}
