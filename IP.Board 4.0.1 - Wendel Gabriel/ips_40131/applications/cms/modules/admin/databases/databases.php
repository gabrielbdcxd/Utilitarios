...
class _databases extends \IPS\Node\Controller
{
	...
	protected function manage()
	{
		...
		/* Columns */
		$table->joins = array(
			array( 'select' => 'w.word_custom as database_name', 'from' => array( 'core_sys_lang_words', 'w' ), 'where' => "w.word_key=CONCAT( 'content_db_', cms_databases.database_id ) AND w.lang_id=" . \IPS\Member::loggedIn()->language()->id )
		);
		...
		$table->rowButtons = function( $row )
		{
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'cms', 'fields', 'fields_manage' ) )
			{
				$return['fields'] = array(
		...
		/* Display */
		\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( \IPS\Member::loggedIn()->language()->addToStack('menu__cms_databases_databases'), (string) $table );
	...
	public function add()
	{
		...
		if ( $values = $form->values() )
		{
			if ( $values['cms_database_import'] )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=cms&module=databases&controller=databases&do=import&file=' . urlencode( $tempFile ) . '&key=' . md5_file( $tempFile ) ) );
			}
			else
			{
	...
	public function import()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			\IPS\Http\Url::internal( 'app=cms&module=databases&controller=databases&do=import&file=' . urlencode( \IPS\Request::i()->file ) . '&key=' .  \IPS\Request::i()->key ),
			function( $data )
			{
				/* Open XML file */
				$xml = new \XMLReader;
				...
				if ( !is_array( $data ) )
				{
					...
					while ( $xml->read() )
					{
						...
						if ( $xml->name == 'data' )
						{
							...
							\IPS\Lang::saveCustom( 'cms', "digest_area_records" . $database->id, $langs['content_db_lang_pu'] );
							\IPS\Lang::saveCustom( 'cms', "digest_area_categories" . $database->id, $langs['content_db_lang_pu'] );
						...
						if ( $xml->name == $data['next'] )
						{
							...
							switch ( $areaName )
							{
								case 'field':
									...
									$obj->saveForm( $obj->formatFormValues( $attrs ) );
									...
									\IPS\Db::i()->update( 'core_permission_index', array_merge( $perms, array( 'perm_id' => $existingPerms['perm_id'], 'perm_type_id' => $obj->id ) ), array( 'perm_id=?', $existingPerms['perm_id'] ) );
								...
								case 'template':
									...
									{
										/* Got this template? */
										if ( isset( $templates['cms']['database'][ $attrs['template_location'] ][ $attrs['template_group'] ] ) )
										{
											/* Compare with version stored in DB and don't bother importing unless we need to */
											if ( $templates['cms']['database'][ $attrs['template_location'] ][ $attrs['template_group'] ]['template_content'] != $attrs['template_content'] )
											{
												$updateDb = TRUE;
												$add      = TRUE;
											}
										...
										if ( $add )
										{
											...
											$obj->key = 'database_' . \IPS\Http\Url::seoTitle( $obj->group ) . '_' . \IPS\Http\Url::seoTitle( $obj->title ) . '_' . $obj->id;
											$obj->save();
											...
											if ( $updateDb )
											{
												if ( $obj->group !== 'template_form' )
												{
													...
													if ( $templateArea )
													{
														/* Template group has same name as existing */
														$obj->group .= '_' . $database->_id;
														$obj->save();

														$database->$templateArea = $obj->group;
														$database->save();
													}
												}
												else if ( $database->template_form === $obj->title )
												{
													$templateArea = $obj->title;
													...
													if ( $templateArea )
													{
														...
														$database->$templateArea = $obj->title;
														$database->save();
													}
												}
											}
	...
	protected function download()
	{
		...
		else
		{
				...
				as $row )
			{
				...
				foreach( $row as $k => $v )
				{
					if ( ! in_array( $k, $removeFields ) AND ! in_array( $k, $textFields ) )
					{
	...
	protected function delete()
	{
		...
		$form->addMessage( \IPS\Member::loggedIn()->language()->addToStack('content_delete_db_confirm'), 'ipsMessage ipsMessage_warning' );
		$form->add( new \IPS\Helpers\Form\YesNo( 'content_delete_db', 0, FALSE, array(), NULL, NULL, NULL, 'content_delete_db' ) );
		
		if ( $values = $form->values() )
		{
			if ( $values['content_delete_db'] == 1 )
			{
	...
	public function form()
	{
		...
		$form->add( new \IPS\Helpers\Form\Radio( 'database_cat_index_type' , $current ? $current->cat_index_type : 0, FALSE, array(
				...
				'toggles' => array(
						...
						'1' => array( 'database_template_featured', 'database_featured_featured', 'database_featured_categories', 'database_featured_perpage', 'database_featured_pagination', 'database_featured_sort', 'database_featured_direction' )
				)
		...
		$fields = array(
			...
			'record_last_comment' => 'database_field__updated',
			'rating_real' 	      => 'database_field__rating'
		);
		...
		if ( ! $current )
		{
			...
			foreach( \IPS\cms\Pages\Page::formElements() as $name => $field )
			{
				...
				{
					/* Overwrite field */
					$field = new \IPS\Helpers\Form\Text( 'page_name', FALSE, NULL, array( 'maxLength' => 64 ), function( $val )
					{
						if ( empty( $val ) AND \IPS\Request::i()->database_create_page === 'new' )
						{
		...
		if ( $values = $form->values() )
		{
			...
			if ( empty( $current ) )
			{
				...
				$current->key = uniqid(); # This is modified below to use a proper key
			...
			if ( ! $values['database_key'] )
			{
				...
				{
					$current->key = \IPS\Http\Url::seoTitle( $values['database_name'][ key( $values['database_name'] ) ] );
				}
				else
				{
					$current->key = \IPS\Http\Url::seoTitle( $values['database_name'] );
				}
			...
			\IPS\Lang::saveCustom( 'cms', "digest_area_records" . $current->id, $values['database_lang_pu'] );
			\IPS\Lang::saveCustom( 'cms', "digest_area_categories" . $current->id, $values['database_lang_pu'] );
			...
			if ( $new )
			{
				...
				{
					$values['page_content'] = '{database="' . $current->id . '"}';
					...
					$pageId = \IPS\cms\Pages\Page::createFromForm( $pageValues, 'html' );
				}
			}
			else
			{
		...
		\IPS\Output::i()->title  = ( $current ) ? \IPS\Member::loggedIn()->language()->addToStack('cms_editing_database', NULL, array( 'sprintf' => array( $current->_title ) ) ) : \IPS\Member::loggedIn()->language()->addToStack('cms_adding_database');
	}
}