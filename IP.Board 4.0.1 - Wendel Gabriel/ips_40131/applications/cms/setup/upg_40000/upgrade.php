...
class _Upgrade
{
	...
	public function step3()
	{
		...
		foreach( \IPS\Member\Group::groups( TRUE ) as $group )
		{
		...
		foreach( \IPS\Db::i()->select( '*', 'cms_database_fields' ) as $field )
		{
			...
			$save = array( 'field_type' => ucfirst( $field['field_type'] ) );
			...
			if ( $database['database_id'] == 1 )
			{
				...
				if( count( $change ) )
				{
					try
					{
						...
						\IPS\Log::i( LOG_DEBUG )->write( "Run " . 'ALTER TABLE ' . \IPS\Db::i()->prefix . 'cms_custom_database_' . $database['database_id'] . " " . implode( ', ', $change ), 'upgrade_pages' );
					}
					catch ( \IPS\Db\Exception $e )
					{
						# Don't care, sync below will add columns
						\IPS\Log::i( LOG_ERR )->write( $e, 'upgrade_pages' );
					}
				}
	...
	public function step7()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'cms_database_categories', NULL, 'category_id asc', array( $limit, $perCycle ) ) as $row )
		{
			...
			{
				$save['category_furl_name'] = \IPS\Http\Url::seoTitle( $row['category_name'] );
			...
			if ( $row['category_parent_id'] )
			{
				...
				while( $parentId != 0 )
				{
					...
					try
					{
						...
						{
							$parent['category_furl_name'] = \IPS\Http\Url::seoTitle( $parent['category_name'] );
						}
						else
						{
	...
	public function finish()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'cms_databases' ) as $database )
		{
			try
			{
				\IPS\Task::queue( 'cms', 'RebuildRecords'   , array( 'class' => 'IPS\cms\Records' . $database['database_id'] ), 3 );
				\IPS\Task::queue( 'cms', 'RebuildCategories', array( 'class' => 'IPS\cms\Categories' . $database['database_id'] ), 3 );
			}
