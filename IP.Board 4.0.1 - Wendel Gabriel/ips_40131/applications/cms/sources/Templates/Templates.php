...
class _Templates extends \IPS\Patterns\ActiveRecord
{
	 ...
	 */
	const RETURN_DATABASE_AND_IN_DEV = 128;
	...
	public static function load( $id, $idField=NULL, $extraWhereClause=NULL )
	{
		...
		catch( \OutOfRangeException $ex )
		{
			...
			{
				$templates = \IPS\cms\Theme::i()->getRawTemplates( NULL, NULL, NULL, \IPS\cms\Theme::RETURN_ONLY_TEMPLATE | \IPS\cms\Theme::RETURN_AS_OBJECT );
	...
	public static function getTemplates( $returnType=1 )
	{
		...
		if ( ( \IPS\IN_DEV or \IPS\Theme::designersModeEnabled() ) AND ! ( $returnType & self::RETURN_DATABASE_ONLY ) )
		{
	...
	public static function add( $template )
	{
		...
		/* Create a unique key */
		$newTemplate->key = $newTemplate->location . '_' . \IPS\Http\Url::seoTitle( $newTemplate->title ) . '_' . $newTemplate->id;
	...
	public function delete()
	{
		if ( $this->user_added )
		{
			\IPS\Db::i()->delete( 'cms_templates', array( 'template_key=? AND template_user_added=?', $this->key, 1 ) );
			
			if ( isset( static::$multitons[ $this->key ] ) )
			{
				unset( static::$multitons[ $this->key ] );
			}
		}
		else
		{
	...
	public function get__file_object()
	{
		if ( ! $this->file_object )
		{
			...
			/* Build on demand */
			if ( mb_stristr( $this->content, "{block=" ) )
			{
