...
class _Block extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public function getTemplate()
	{
		...
		/* Still here */
		if ( ! in_array( $functionName, array_keys( self::$calledTemplates ) ) )
		{
			if ( ! isset( \IPS\Data\Store::i()->$functionName ) )
			{
				if ( $this->content )
				...
				else if ( $this->template )
				{
					try
					{
						...
						return call_user_func_array( array( \IPS\cms\Theme::i()->getTemplate( $template->group, 'cms', $template->location ), $template->title ), $args );
					}
					catch( \OutOfRangeException $ex )
					{
...
EOF;
			...
			/* Init */
			self::$calledTemplates[ $functionName ] = new $class();
		}

		return call_user_func_array( array( self::$calledTemplates[ $functionName ], 'run' ), $args );
	}
	...
	public function form( &$form )
	{
		...
		{
			$form->addHtml( \IPS\Theme::i()->getTemplate( 'blocks', 'cms', 'admin' )->previewBlock() );
		...
		if ( $block_editor === 'editor' )
		{
			$form->add( new \IPS\Helpers\Form\Editor( 'block_content', $this->content, FALSE, array(
				'app'         => 'cms',
				'key'         => 'BlockContent',
				'autoSaveKey' => 'block-content-' . ( $this->id ? $this->id : 'new' ) ), NULL, NULL, NULL, 'block_content_editor' ) );
		}
		else
		{
			$form->add( new \IPS\Helpers\Form\Codemirror( 'block_content', $this->content, FALSE, array(), NULL, NULL, NULL, 'block_content' ) );
		}
	...
	public function formatFormValues( $values )
	{
		if ( ! $this->id )
		{
			...
			{
				\IPS\File::claimAttachments( 'block-content/block-' . $this->id, $this->id );
			}
		}
		...
		if( isset( $values['block_key'] ) )
		{
			if ( ! $values['block_key'] )
			{
				...
				{
					$values['block_key'] = \IPS\Http\Url::seoTitle( $values['block_name'][ key( $values['block_name'] ) ] );
				}
				else
				{
					$values['block_key'] = \IPS\Http\Url::seoTitle( $values['block_name'] );
				}

				/* Now test it */
				try
				{
					$block = \IPS\cms\Blocks\Block::load( $this->key, 'block_key');

					/* It's taken... */
					if ( $this->id != $block->id )
					{
						$values['block_key'] .= '_' . uniqid();
					}
				}
				catch( \OutOfRangeException $ex )
				{
			...
			else
			{
				...
				if ( isset( $values['block_save_as_template'] ) AND $values['block_save_as_template'] )
				{
					if ( $values['block_template_id'] == '_default_' )
					{
						...
						$templateArray = array(
							'key' 		   => 'template_' . $templateBit['template_name'] . '.' . uniqid(),
							'title'		   => str_replace( '-', '_', \IPS\Http\Url::seoTitle( $values['block_save_as_template_name'] ? $values['block_save_as_template_name'] : $templateBit['template_name'] . '_' . \IPS\Member::loggedIn()->language()->get('copy_noun') ) ),
							'desc' 		   => null,
					...
					else
					{
						try
						{
							...
							$templateArray = array(
								'key' 		   => 'template_' . $template->name . '.' . uniqid(),
								'title'		   => str_replace( '-', '_', \IPS\Http\Url::seoTitle( $values['block_save_as_template_name'] ? $values['block_save_as_template_name'] : $template->name . '_' . \IPS\Member::loggedIn()->language()->get('copy_noun') ) ),
								'desc' 		   => null,
								'content' 	   => $values['block_content'],
								'location' 	   => 'block',
								'group' 	   => \IPS\Request::i()->block_plugin,
	...
	public function widget()
	{
		if ( $this->type === 'plugin' AND $this->plugin )
		{
			...
			{
				$this->widgetLoaded = \IPS\Widget::load( $this->plugin_app ? \IPS\Application::load( $this->plugin_app ) : \IPS\Plugin::load( $this->plugin_plugin ), $this->plugin, uniqid(), $this->_plugin_config, NULL, $this->orientation );
			}
