...
class _Page extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	static public function formElements( $item=NULL )
	{
		...
		else
		{
			...
			$return['page_template'] = new \IPS\Helpers\Form\Select( 'page_template', $item->template ? $item->template : FALSE, FALSE, array( 'options' => $builderTemplates ), NULL, NULL, NULL, 'page_template' );
		}
		...
		if ( $pageType === 'html' )
		{
			...
			foreach( \IPS\cms\Databases::roots( NULL, NULL ) as $id => $db )
			{
				if ( ! $db->page_id )
				{
					$tags['{database="' . $db->_id . '" title="' . $db->_title . '"}'] = $db->_title;
				}
			}
			
			foreach( \IPS\cms\Blocks\Block::roots( NULL, NULL ) as $id => $block )
			{
				$tags['{block="' . $block->_id . '" title="' . $block->_title . '"}'] = $block->_title;
			}
			
			$return['page_content'] = new \IPS\Helpers\Form\Codemirror( 'page_content', $item ? $item->content : NULL, FALSE, array( 'tags' => $tags ), function( $val )
			{
				/* Test */
				try
				{
	...
	public function form( &$form )
	{
		...
		if ( ! $this->id )
		{
			...
			foreach( \IPS\cms\Pages\Menu::formElements( NULL, 'page' ) as $name => $field )
			{
				$toggles[] = $name;
	...
	public function formatFormValues( $values )
	{
		...
		if ( isset( $values['page_add_to_menu'] ) )
		{
			if( $values['page_add_to_menu'] )
			{
				$menuValues = array(
					'menu_type'            => 'page',
					'menu_title_page'      => $values['menu_title_page'],
					'menu_content_page'    => $this->id,
					'menu_permission'      => $values['menu_permission'],
					'menu_permission_type' => $values['menu_permission_type'],
					'menu_parent_id'       => $values['menu_parent_id']
				);

				$menu = new \IPS\cms\Pages\Menu();
				$menu->saveForm( $menu->formatFormValues( $menuValues ) );
			}

			unset( $values['page_add_to_menu'], $values['menu_title_page'], $values['menu_permission'], $values['menu_permission_type'], $values['menu_parent_id'] );
		}
	...
	public function delete()
	{
		...
		\IPS\Db::i()->delete( 'cms_page_menu', array( 'menu_type=? AND menu_content=?', 'page', $this->id ) );
	...
	{
		\IPS\cms\Pages\Menu::clearCaches();
