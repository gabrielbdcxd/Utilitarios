	...
	\IPS\Content\Embeddable
{
	...
	{
		/* Normally we check if we are using the forums at the record level, however this is a static method called so we can only try to do our best to guess */
		if( static::database()->forum_record and static::database()->forum_comments and \IPS\Application::appIsEnabled('forums') )
		{
	...
	public static function loadFromUrl( \IPS\Http\Url $url )
	{
		...
		if ( isset( $qs['path'] ) )
		{
			try
			{
				return static::loadFromSlug( $qs['path'], FALSE );
			}
			catch ( \Exception $e ) { }
		}
	...
	{
		if ( static::database()->revisions and $this->record_edit_time and ( $this->record_edit_show or \IPS\Member::loggedIn()->modPermission('can_view_editlog') ) )
		{
