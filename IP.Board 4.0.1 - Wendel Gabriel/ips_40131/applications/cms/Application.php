...
spl_autoload_register( function( $class )
{
	...
	{
		$databaseId   = mb_substr( $class, 15 );
		$titleField   = \IPS\cms\Databases::load( $databaseId )->field_title;
		$contentField = \IPS\cms\Databases::load( $databaseId )->field_content;
		$titleLang    = 'content_db_lang_sl_' . $databaseId;
		...
		class Records{$databaseId} extends Records
		{
			...
			public static \$reputationType = 'record_id_{$databaseId}';
			public static \$databaseColumnMap = array(
				...
				'last_comment_name_seo'	=> 'record_last_comment_seo',
				'views'					=> 'record_views',
...
class _Application extends \IPS\Application
{
	...
	public function acpMenu()
	{
		...
		/* Check for, and build IN_DEV templates if required */
		$this->_writeInDevTemplates();
	...
	{
		$classes = parent::extensions( $app, $extension, $construct );
		...
		if ( $extension === 'EditorLocations' )
		{
			foreach( \IPS\cms\Databases::databases() as $obj )
			{
				...
				else
				{
					try
					{
						$classes[ 'Records' . $obj->_id ] = new $classname;
					}
					catch( \RuntimeException $e ){}
				}
			}
		}
		else if ( $checkAccess and $extension === 'ContentRouter' )
		{
			if ( isset( $classes['Records'] ) )
			{
				foreach ( $classes['Records']->classes as $id => $name )
				{
					if ( ! $name::database()->can( 'view' ) )
					{
						unset( $classes['Records']->classes[ $id ] );
						continue;
					}

					if ( isset( $name::$reviewClass ) and ! $name::database()->options['reviews'] )
					{
						$_class = $classes['Records']->classes[ $id ];
						if ( class_exists( $_class ) )
						{
							$_class::$reviewClass = NULL;
						}
					}
				}
			}
	...
	public function installOther()
	{
		...
		/* Add in permissions */
		$groups = array();
		foreach( \IPS\Db::i()->select( 'g_id', 'core_groups', array( 'g_access_cp=1' ) ) as $row )
		{
			$groups[] = $row;
		}
		...
		\IPS\Lang::saveCustom( 'cms', "digest_area_records" . $database->id, "Articles" );
		...
		catch ( \Exception $ex )
		{
			...
			\IPS\Log::i( LOG_ERR )->write( $ex, 'pages_create_db_error' );
		...
		$pageValues = array(
			...
			'page_ipb_wrapper'  => 1,
			'page_show_sidebar' => 1,
			'page_content'      => '{database="' . $database->id . '"}',
		);
	...
	/**
	 * Write the IN_DEV templates to disk
	 *
	 * @return void
	 */
	public function _writeInDevTemplates()
	{
		if ( \IPS\IN_DEV )
		{
			//\IPS\cms\Theme::writeInDev();
		}
