...
class _themes extends \IPS\Node\Controller
{
	...
	protected function manage()
	{
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'customization', 'theme_designers_mode' ) )
		{
					...
					switch( $type )
					{
						case 'lang':
							if ( file_exists( \IPS\ROOT_PATH . "/themes/{$themeId}/lang.php" ) )
							{
								...
								if ( count( $languageInserts ) )
								{
									\IPS\Db::i()->insert( 'core_sys_lang_words', $languageInserts );
								}
							}
						break;
						case 'html':
	...
	public function resources()
	{
		...
		$table->parsers = array(
			...
			'resource_name' => function( $val, $row )
			{
						...
						while ( $xml->read() )
						{
							if ( $xml->name == 'theme' )
							{
								...
								$set->saveSet( array(
									...
									'set_long_version' => $xml->getAttribute('long_version'),
										...
										switch ( $xml->name )
										{
											case 'field':
												...
												$fieldId = \IPS\Db::i()->insert( 'core_theme_settings_fields', array(
													...
													'sc_default'       => $xml->getAttribute('sc_default'),
												...
												\IPS\Db::i()->insert( 'core_theme_settings_values', array( 'sv_id' => $fieldId, 'sv_value' => $value ) );
											break;
											case 'template':
											...
											case 'css':
												...
												{
													\IPS\Db::i()->replace( 'core_theme_css', array(
	...
	public function conflicts()
	{
		...
		{
			if ( md5( $data['conflict_content'] ) != md5( $templates[ $key ]['template_content'] ) )
			{
		...
		{
			if ( md5( $data['conflict_content'] ) != md5( $css[ $key ]['css_content'] ) )
			{
	...
	public function exportForm()
	{
		...
		if ( $values = $form->values() or \IPS\Request::i()->form_submitted )
		{
			...
			foreach ( \IPS\Db::i()->select( '*', 'core_sys_lang_words', array( 'word_theme=?', $themeSet->id ) ) as $row )
			{
	...
	public function delete()
	{
		...
		try
		{
			if ( \IPS\Theme::load( \IPS\Request::i()->id )->is_default )
			{
	...
	public function templateForm()
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'theme_template_name', NULL, TRUE, array( 'regex' => '/^([A-Z0-9_\\-]+?)$/i' ),
		function( $val )
		{
			if ( ! preg_match( '/^([A-Z0-9_\-]+?)$/i', $val ) )
			{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'theme_template_location_new', NULL, FALSE, array( 'regex' => '/^([A-Z]+?)?$/i' ), NULL, NULL, NULL, 'location_new' ) );
		...
		$form->add( new \IPS\Helpers\Form\Text( 'theme_template_group_new', NULL, FALSE, array( 'regex' => '/^([a-z0-9_]+?)?$/' ), NULL, NULL, NULL, 'group_new' ) );
	...
	public function deleteTemplate()
	{
		...
		\IPS\Session::i()->log( 'acplogs__themetemplate_deleted', array( $template['name'] => FALSE ) );
	...
	{
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			\IPS\Http\Url::internal( 'app=core&module=customization&controller=themes&do=devImport&id=' . \IPS\Request::i()->id ),
			function( $data )
			{
				/* Is this the first cycle? */
				if ( ! is_array( $data ) )
							...
							foreach( $templateNames[ $app ][ $location ] as $path => $items )
							{
								if ( $path === 'custom' )
								{
									foreach( $items as $name => $data )
									{
										if ( $name !== 'custom.css' )
										{
											unset( $templateNames[ $app ][ $location ][ $path ][ $name ] );
										}
									}
								}
								else
