...
class _members extends \IPS\Dispatcher\Controller
{
	...
	public function doMove()
	{
		...
		function( $data ) use ( $cycle, $group, $self )
		{
			...
			$select	= \IPS\Db::i()->select( '*', 'core_members', $_SESSION['members_manage_where'], 'member_id ASC', array( $data['done'], $cycle ), NULL, NULL, \IPS\Db::SELECT_SQL_CALC_FOUND_ROWS );
	...
	public function ip()
	{
		...
		/* Init Table */
		$table = new \IPS\Helpers\Table\Custom( $member->ipAddresses(), \IPS\Http\Url::internal( "app=core&module=members&controller=members&id={$member->member_id}&do=ip" ) );
