...
class _moderators extends \IPS\Dispatcher\Controller
{
	...
	protected function add()
	{
		...
		{
			$form->add( new \IPS\Helpers\Form\Radio( 'acprestrictions_type', NULL, TRUE, array( 'options' => array( 'g' => 'group', 'm' => 'member' ), 'toggles' => array( 'g' => array( 'acprestrictions_group' ), 'm' => array( 'acprestrictions_member' ) ) ) ) );
		}
		...
		{
			$form->add( new \IPS\Helpers\Form\Member( 'acprestrictions_member', NULL, ( \IPS\Request::i()->acprestrictions_type === 'member' ), array(), NULL, NULL, NULL, 'acprestrictions_member' ) );
		}
		...
		if ( $values = $form->values() )
		{
			...
			if ( $values['acprestrictions_type'] === 'g' or !\IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'staff', 'moderators_add_member' ) )
			{
			...
			if ( $rowId !== NULL )
			{
				try
				{
					$current = \IPS\Db::i()->select( '*', 'core_moderators', array( "id=? AND type=?", $rowId, $values['acprestrictions_type'] ) )->first();
				}
				...
				if ( !count( $current ) )
				{
					$current = array(
						'id'		=> $rowId,
						'type'		=> $values['acprestrictions_type'],
						'perms'		=> '*',
						'updated'	=> time()
					);
					...
					\IPS\Session::i()->log( 'acplog__moderator_created', array( ( $values['acprestrictions_type'] == 'g' ? \IPS\Member::loggedIn()->language()->get( "core_group_{$values['acprestrictions_group']}" ) : $values['acprestrictions_member']->name ) => FALSE ) );
	...
	protected function actionLogs()
	{
		...
		$table->parsers = array(
				...
				'module'	=> function( $val, $key )
				{
					$key = "module__{$key['appcomponent']}_{$val}";
		...
		$table->advancedSearch	= array(
				...
				'module'			=> array( \IPS\Helpers\Table\SEARCH_NODE, array( 'class' => 'IPS\Application', 'subnodeWhere' => array( 'sys_module_area=?', 'front' ), 'searchProp' => 'key' ) )
		);
