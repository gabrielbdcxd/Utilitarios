...
class _toolbar extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		\IPS\Output::i()->sidebar['actions'] = array(
			...
			'edit'	=> array(
				'icon'	=> 'cog',
				'title'	=> 'editor_advanced_config',
				'link'	=> \IPS\Http\Url::internal( "app=core&module=editor&controller=toolbar&do=advanced" ),
				'data'	=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack('editor_advanced_config') )
			),
		);
	}

	/**
	 * Advanced ckeditor configuration options
	 *
	 * @return void
	 */
	...
	protected function addPlugin()
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if ( !$f->isDot() and $f->isDir() and $f !== 'index.html' and !in_array( (string) $f, array_merge( array( 'autogrow', 'basicstyles', 'button', 'clipboard', 'colorbutton', 'dialog', 'dialogui', 'enterkey', 'entities', 'fakeobjects', 'floatingspace', 'floatpanel', 'font', 'indent', 'indentblock', 'indentlist', 'index.html', 'ipsattach', 'ipsautolink', 'ipsemoticon', 'ipsimage', 'ipslink', 'ipsmentions', 'ipspage', 'ipsprefs', 'ipsquote', 'ipsspoiler', 'justify', 'link', 'list', 'listblock', 'magicline', 'panel', 'panelbutton', 'pastetext', 'pbckcode', 'removeformat', 'richcombo', 'sourcearea', 'toolbar', 'undo', 'wysiwygarea' ), explode( ',', \IPS\Settings::i()->ckeditor_extraPlugins ) ) ) )
					{
