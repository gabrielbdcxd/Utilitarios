...
class _reputation extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		if ( $activeTab === 'settings' )
		{
			...
			/* Fake names for the "like" preview */
			$names = array();
			foreach ( range( 1, 3 ) as $i )
			{
				try
				{
					$member = \IPS\Member::constructFromData( \IPS\Db::i()->select( '*', 'core_members', empty( $names ) ? array() : 'member_id NOT IN(' . implode( ',', array_keys( $names ) ) . ')', 'RAND()', 1 )->first() );
		...
		{
			/* Create the table */
			$table = new \IPS\Helpers\Table\Db( 'core_reputation_levels', \IPS\Http\Url::internal( 'app=core&module=membersettings&controller=reputation' ) );
