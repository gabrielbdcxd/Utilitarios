...
class _login extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		{
			$url = $url->setQueryString( 'ref', base64_encode( \IPS\Request::i()->ref ) );
		}
		...
		try
		{
			...
			if ( $member !== NULL and $member->member_id )
			{
				...
				if ( $member->isAdmin() )
				{
					...
					if( isset( \IPS\Request::i()->ref ) )
					{
						$queryString	= preg_replace( "/adsess=([a-zA-Z0-9]+)(?:&|$)/", '', base64_decode( \IPS\Request::i()->ref ) );
					}
					/* Boink */
					\IPS\Output::i()->redirect( \IPS\Http\Url::internal( $queryString ) );
				}
				/* Error */
				else
				{
