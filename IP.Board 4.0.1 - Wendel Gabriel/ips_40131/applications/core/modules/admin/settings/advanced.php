...
class _advanced extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageDatastore()
	{
		...
		$options = array(
			...
			'Eaccelerator'	=> 'datastore_method_Eaccelerator',
	...
	protected function _manageSettings()
	{
		...
		\IPS\Member::loggedIn()->language()->words['task_use_cron_warning'] = sprintf( \IPS\Member::loggedIn()->language()->get( 'task_use_cron_warning', FALSE ), PHP_BINDIR . '/php -d memory_limit=-1 -d max_execution_time=0 ' . \IPS\ROOT_PATH . '/applications/core/interface/task/task.php ' . \IPS\Settings::i()->task_cron_key );
		$cronOptions = array( 'togglesOn' => array( 'task_use_cron_warning' ) );
		
		/* Build and show form */
		$form = new \IPS\Helpers\Form;
		$form->add( new \IPS\Helpers\Form\YesNo( 'task_use_cron', \IPS\Settings::i()->task_use_cron, FALSE, $cronOptions, function ( $val )
		{
			$cronFile = \IPS\ROOT_PATH . '/applications/core/interface/task/task.php';
			if ( $val and ( mb_strtoupper( mb_substr( PHP_OS, 0, 3 ) ) !== 'WIN' AND !is_executable( $cronFile ) ) )
			{
		...
		//IV
		$form->addHeader( 'solve_cf_problems' );
		$form->add( new \IPS\Helpers\Form\YesNo( 'change_iv_url', \IPS\Settings::i()->change_iv_url, FALSE, array(
				'togglesOn'	=> array( 'iv_connect_url' ),
		) ) );
		$form->add( new \IPS\Helpers\Form\Url( 'iv_connect_url', \IPS\Settings::i()->iv_connect_url, TRUE, array( 'placeholder' => \IPS\Member::loggedIn()->language()->addToStack('iv_url_placeholder') ), NULL, NULL, NULL, 'iv_connect_url' ) );
