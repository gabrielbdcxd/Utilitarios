...
class _posting extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageGeneral()
	{
		...
		$form->add( new \IPS\Helpers\Form\Number( 'max_video_width', \IPS\Settings::i()->max_video_width, FALSE, array( 'unlimited' => 0 ), NULL, NULL, 'px' ) );
	...
	protected function _manageUrlFilters()
	{
		...
		if ( $values = $form->values() )
		{
			...
			foreach( $values['posts_add_nofollow_exclude'] as $domain )
			{
				...
				$nofollowExclude[ $domain ] = $domain;
			}

			$values['posts_add_nofollow_exclude'] = json_encode( array_values( $nofollowExclude ) );
			...
			/* Redirect otherwise our changes to posts_add_nofollow_exclude won't reflect initially */
