...
class _email extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		if ( $values = $settingsForm->values() )
		{
			...
			foreach( array( 'mail_method', 'smtp_host', 'smtp_port', 'smtp_pass', 'smtp_user', 'php_mail_extra' ) as $setting )
			{
			...
			if( $sendTestEmail )
			{
				$email			= \IPS\Email::buildFromContent( \IPS\Member::loggedIn()->language()->addToStack('email_test_subject'), \IPS\Member::loggedIn()->language()->addToStack('email_test_message'), \IPS\Member::loggedIn()->language()->addToStack('email_test_message') );
				$email->from	= \IPS\Settings::i()->email_out;
				$result = $email->send( \IPS\Member::loggedIn()->email );

				if ( $result === TRUE )
				{
					$messageHtml = \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( 'email_test_okay_auto', 'success' );
				}
				elseif( $result instanceof \IPS\Email\Outgoing\Exception )
				{
					$messageHtml = \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( 'email_test_error_auto', 'error', $result->getMessage() );
				}
				/* If php mail fails it throws a regular PHP error */
				elseif( $result instanceof \ErrorException )
				{
					$messageHtml = \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( 'email_test_error_auto', 'error', $result->getMessage() );
				}
			}
