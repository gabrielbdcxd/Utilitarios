...
{	
	...
	public function manage()
	{
				...
				if( $row['mail_active'] )
				{
					...
					if ( \IPS\Settings::i()->mandrill_api_key AND \IPS\Settings::i()->mandrill_use_for )
					{
						$return['resend'] = array(
							...
							'link'		=> \IPS\Http\Url::internal( 'app=core&module=bulkmail&controller=bulkmail&resetTask=1&do=sendMandrill&id=' ) . $row['mail_id'],
						);
					}
				}
				else
		...
		/* Shortcut to Mandrill configuration */
		\IPS\Output::i()->sidebar['actions'] = array(
			'configure' => array(
				'title'		=> 'mandrill_cross_link',
				'icon'		=> 'cog',
				'link'		=> \IPS\Http\Url::internal( "app=core&module=applications&controller=enhancements&do=edit&id=core_Mandrill" ),
			),
		);
	...
	public function preview()
	{
		...
		if( !\IPS\Settings::i()->mandrill_api_key )
		{
	...
	public function iframePreview()
	{
		...
		/* Format the email */
		$email	= \IPS\Email::buildFromContent( $mail->subject, $mail->formatBody( \IPS\Member::loggedIn() ) );
		$email->buildUnsubscribe( 'bulk', \IPS\Member::loggedIn() );

		\IPS\Output::i()->sendOutput( $email->body['html'] );
	}
	...
	/**
	 * Send Mandrill bulk mails immediately
	 *
	 * @return void
	 */
	public function sendMandrill()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			\IPS\Http\Url::internal( "app=core&module=bulkmail&controller=bulkmail&do=sendMandrill&id=" . intval( \IPS\Request::i()->id ) ),
			function( $data )
			{
				/* Retrieve the bulk mail details */
				try
			...
			function()
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=bulkmail&controller=bulkmail' ), 'bm_mandrill_sent' );
			}
		);
	}
	...
	public function form()
	{
		...
		if( \IPS\Settings::i()->mandrill_api_key AND \IPS\Settings::i()->mandrill_use_for )
		{
			$form->add( new \IPS\Helpers\Form\Stack( 'mandrill_tags', ( !empty( $mail->_options['mandrill_tags'] ) ) ? $mail->_options['mandrill_tags'] : array(), FALSE ) );
		}
		...
		if ( $values = $form->values() )
		{
			...
			if( \IPS\Settings::i()->mandrill_api_key AND \IPS\Settings::i()->mandrill_use_for )
			{
				$_options['mandrill_tags']	= $values['mandrill_tags'];
			}
