...
class _plugins extends \IPS\Node\Controller
{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			...
			function( $data )
			{
				...
				if ( !is_array( $data ) )
				{
					...
					if ( !$plugin->location )
					{
						$plugin->location = 'p' . mb_substr( md5( uniqid() ), 0, 10 );
					}
					...
					if ( !in_array( $xml->name, $data['done'] ) )
					{
						...
						switch ( $step )
						{
							...
							case 'lang':
								...
								\IPS\Db::i()->delete( 'core_sys_lang_words', array( 'word_plugin=?', $plugin->id ) );
				...
				{
					\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: found " . count( $data['setUpClasses'] ). " set up classes to run ", 'plugin_setup' );
					...
					\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: looking for class key " . $key, 'plugin_setup' );

					if ( isset( \IPS\Data\Store::i()->$key ) )
					{
						\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: found class key " . $key . " class " . $class, 'plugin_setup' );
						\IPS\Log::i( LOG_DEBUG )->write( \IPS\Data\Store::i()->$key, 'plugin_setup' );
						...
						\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: looking for class " . $class, 'plugin_setup' );
						...
						if ( class_exists( $class ) )
						{
							...
							\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: looking for class key " . $key . ", " . $method, 'plugin_setup' );

							if ( method_exists( $upgrader, $method ) )
							{
								\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: Running " . $key . ", " . $method, 'plugin_setup' );
								...
								if ( $result === TRUE )
								{
									...
									if ( method_exists( $upgrader, $method ) )
									{
										\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: Running " . $key . ", next method " . $method . " found", 'plugin_setup' );
					...
					\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: Class " . $versionToRun . " completed", 'plugin_setup' );

					/* Done this class completely */
					$data['step'] = 0;
					unset( $data['setUpClasses'][ $versionToRun ] );

					\IPS\Log::i( LOG_DEBUG )->write( json_encode( $data ), 'plugin_setup' );

					/* Go for another hit on multiredirector */
					return array( $data, \IPS\Member::loggedIn()->language()->addToStack('plugins_install_setup_method', FALSE, array( 'sprintf' => array( $versionToRun ) ) ) );
				}

				\IPS\Log::i( LOG_DEBUG )->write( "Plugin setup: All set up classes run", 'plugin_setup' );
	...
	protected function editHook()
	{
		try
		{
			...
			\IPS\Plugin\Hook::load( \IPS\Request::i()->hook )->editForm( \IPS\Request::i()->url() );
		}
		catch ( \OutOfRangeException $e )
		{
	...
	public function download()
	{
		...
		/* Build */
		\IPS\Output::i()->sendOutput( $xml->asXML(), 200, 'application/xml', array( "Content-Disposition" => \IPS\Output::getContentDisposition( 'attachment', $plugin->name . '.xml' ) ) );
	}
