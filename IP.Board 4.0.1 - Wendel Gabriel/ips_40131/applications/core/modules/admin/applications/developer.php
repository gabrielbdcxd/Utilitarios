...
class _developer extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		{
			$activeTabContents = call_user_func( array( $this, '_manage'.ucfirst( mb_substr( $this->activeTab, 0, $pos ) ) ), mb_substr( $this->activeTab, $pos + 1 ) );
		}
		else
		{
	...
	protected function _manageModules( $location )
	{
		...
		if ( isset( \IPS\Request::i()->delete ) )
		{
			...
			/* delete all the other stuff http://community.---.com/resources/bugs.html/_/4-0-0/1019-deleting-a-controller-does-not-delete-its-menu-entry-r48752 */
			...
			foreach ( $order as $parent => $items )
			{
				if ( array_key_exists( $parent, $menu ) )
				{
					...
					foreach ( $items as $key => $position )
					{
						...
						{
							if ( array_key_exists( $key, $_items ) )
							{
		...
		return new \IPS\Helpers\Tree\Tree( $url, 'dev_acpmenu',
			...
			function ( $k ) use ( $url, $menu, $appKey )
			{
				...
				foreach ( $menu[ $k ] as $id => $row )
				{
					...
					$rows[ $id ] = \IPS\Theme::i()->getTemplate( 'trees' )->row( $url, $id, \IPS\Member::loggedIn()->language()->addToStack( $lang ), FALSE, array(
						'edit'	=> array(
							'icon'	=> 'pencil',
							'title'	=> 'edit',
	...
	protected function _manageAcprestrictions()
	{
		...
		if ( \IPS\Request::i()->do === 'reorder' )
		{
			...
			if( isset( \IPS\Request::i()->ajax_order ) )
			{
				$order = array();
				$position = 1;
				foreach( \IPS\Request::i()->ajax_order as $i )
				{
					$order[ $i ] = $position++;
				}
			}
			...
			uasort( $this->restrictions[ $id[0] ][ $id[1] ], function( $a, $b ) use ( $order ) {
				return $order[ str_replace( '_', '~', $a ) ] - $order[ str_replace( '_', '~', $b ) ];
			});
	...
	protected function addController()
	{
		...
		if ( $values = $form->values() )
		{
			...
			$contents = str_replace(
				...
				file_get_contents( \IPS\ROOT_PATH . "/applications/core/data/defaults/Controller" . ucfirst( $values['type'] ) . '.txt' )
			);
	...
	protected function _manageSchema()
	{
		...
		/* Get out latest version */
		$latestVersion = $this->_getLatestVersion();
		$human = $latestVersion['human'];
		...
		$appKey = $this->application->directory;
		$table->rowButtons = function( $row ) use ( $human, $appKey )
		{
			return array(
				'edit' => array(
					'icon'	=> 'pencil',
				...
				'delete' => array(
					...
					'data'	=> array( 'delete' => '', 'delete-warning' => \IPS\Member::loggedIn()->language()->addToStack( 'database_droptable_info', FALSE, array( 'sprintf' => array( $human ) ) ) )
				)
			);
		};
	...
	protected function addTable()
	{
		/* Get out latest version */
		$latestVersion = $this->_getLatestVersion();
		$queriesJson = $this->_getQueries( $latestVersion['long'] );
		...
		switch ( $activeTab )
		{
			...
			case 'new':
				...
				$message = \IPS\Theme::i()->getTemplate( 'global', 'core' )->message( \IPS\Member::loggedIn()->language()->addToStack('database_newtable_info', FALSE, array( 'sprintf' => array( $latestVersion['human'] ) ) ), 'information' );
				break;
			...
			case 'import':
				...
				$message = \IPS\Theme::i()->getTemplate( 'global', 'core' )->message( \IPS\Member::loggedIn()->language()->addToStack('database_renametable_info', FALSE, array( 'sprintf' => array( "{$this->application->directory}_", $latestVersion['human'] ) ) ), 'information' );
		...
		if( $values = $form->values() )
		{
			...
			switch ( $activeTab )
			{
				...
				case 'new':
					...
					$this->_writeQueries( $latestVersion['long'], $queriesJson );
				...
				case 'upload':
					...
					$appKey = $this->application->directory;
					$contents = preg_replace_callback( '/CREATE (TEMPORARY )?TABLE (IF NOT EXISTS )?`?(.+?)`?\s+?/', function( $matches ) use ( $appKey )
					{
						if ( mb_substr( $matches[3], 0, mb_strlen( $appKey ) + 1 ) !== "{$appKey}_" )
						{
							return str_replace( $matches[3], "{$appKey}_{$matches[3]}", $matches[0] );
						}
	...
	protected function editSchema()
	{
		...
		\IPS\Output::i()->output .= \IPS\Theme::i()->getTemplate( 'global', 'core' )->message( \IPS\Member::loggedIn()->language()->addToStack('database_changes_info', FALSE, array( 'sprintf' => array( $latestVersion['human'] ) ) ), 'information' );
		
		/* Does it match the database? */
		$_definition = $definition;
		unset( $_definition['inserts'] );
		unset( $_definition['comment'] );
		unset( $_definition['engine'] );
        unset( $_definition['collation'] );
		try
		{
		...
		unset( $localDefinition['collation'] );
		unset( $localDefinition['comment'] );
		unset( $localDefinition['engine'] );
			...
			try
			{
				...
				else
				{
					if ( !$column )
					{
						...
						$this->_writeQueries( $latestVersion['long'], $this->_addQueryToJson( $this->_getQueries( $latestVersion['long'] ), array( 'method' => 'addColumn', 'params' => array( $definition['name'], $save ) ) ) );
					}
					else
					{
						\IPS\Db::i()->changeColumn( $definition['name'], $column['name'], $save );
						$this->_writeQueries( $latestVersion['long'], $this->_addQueryToJson( $this->_getQueries( $latestVersion['long'] ), array( 'method' => 'changeColumn', 'params' => array( $definition['name'],  $column['name'], $save ) ) ) );
					...
					if( $column AND $save['name'] !== $column['name'] AND isset( $schema[ $definition['name'] ]['indexes'] ) )
					{
	...
	protected function editSchemaDeleteColumn()
	{
		try
		{
			...
			$this->_writeQueries( $latestVersion['long'], $this->_addQueryToJson( $this->_getQueries( $latestVersion['long'] ), array( 'method' => 'dropColumn', 'params' => array( \IPS\Request::i()->_name, \IPS\Request::i()->column ) ) ) );
			...
			if ( isset( $schema[ \IPS\Request::i()->_name ]['indexes'] ) and is_array( $schema[ \IPS\Request::i()->_name ]['indexes'] ) )
			{
				foreach( $schema[ \IPS\Request::i()->_name ]['indexes'] as $name => $definition )
				{
					...
					else if ( $changed )
					{
						...
						$this->_writeQueries( $latestVersion['long'], $this->_addQueryToJson( $this->_getQueries( $latestVersion['long'] ), array( 'method' => 'changeIndex', 'params' => array( $name,  $name, $definition ) ) ) );
					}
				}
			}
			...
			try
			{
				...
				else
				{
					if ( !$index )
					{
						...
						$this->_writeQueries( $latestVersion['long'], $this->_addQueryToJson( $this->_getQueries( $latestVersion['long'] ), array( 'method' => 'addIndex', 'params' => array( $definition['name'], $save ) ) ) );
					...
					else
					{
						...
						$this->_writeQueries( $latestVersion['long'], $this->_addQueryToJson( $this->_getQueries( $latestVersion['long'] ), array( 'method' => 'changeIndex', 'params' => array( $definition['name'],  $index['name'], $save ) ) ) );
					}
	...
	protected function editSchemaDeleteIndex()
	{
		try
		{
			...
			$this->_writeQueries( $latestVersion['long'], $this->_addQueryToJson( $this->_getQueries( $latestVersion['long'] ), array( 'method' => 'dropIndex', 'params' => array( \IPS\Request::i()->_name, \IPS\Request::i()->index ) ) ) );
	...
	protected function editSchemaInsert()
	{
		...
		foreach ( $definition['columns'] as $column )
		{
			...
			{
				$element = new \IPS\Helpers\Form\Text( $column['name'], $data ? $data[ $column['name'] ] : $column['default'], FALSE, $column['allow_null'] ? array( 'nullLang' => 'NULL' ) : array() );
			}
			elseif ( in_array( $column['type'], array( 'TEXT', 'MEDIUMTEXT', 'BIGTEXT', 'TINYBLOB', 'BLOB', 'MEDIUMBLOB', 'BIGBLOB' ) ) )
			{
				$element = new \IPS\Helpers\Form\TextArea( $column['name'], $data ? $data[ $column['name'] ] : $column['default'], FALSE, $column['allow_null'] ? array( 'nullLang' => 'NULL' ) : array() );
			}
			elseif ( $column['type'] === 'ENUM' )
			{
				$element = new \IPS\Helpers\Form\Select( $column['name'], $data ? $data[ $column['name'] ] : $column['default'], FALSE, array( 'options' => array_combine( $column['values'], $column['values'] ) ) );
			}
			elseif ( $column['type'] === 'SET' )
			{
				$element = new \IPS\Helpers\Form\Select( $column['name'], $data ? $data[ $column['name'] ] : $column['default'], FALSE, array( 'options' => array_combine( $column['values'], $column['values'] ), 'multiple' => TRUE ) );
			}
			else
			{
				$element = new \IPS\Helpers\Form\Text( $column['name'], $data ? $data[ $column['name'] ] : $column['default'], FALSE, $column['allow_null'] ? array( 'nullLang' => 'NULL' ) : array() );
	...
	protected function deleteTable()
	{
		...
		$latestVersion = $this->_getLatestVersion();
		$queries = $this->_getQueries( $latestVersion['long'] );
		$queries = $this->_addQueryToJson( $queries, $query );
		$this->_writeQueries( $latestVersion['long'], $queries );
	...
	protected function addExtension()
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'dev_extensions_classname', NULL, TRUE ) );
	...
	public function _getVersionRow( $long, $root=FALSE )
	{
		...
		$buttons = array_merge( $buttons, array(
			'add'		=> array(
				...
				'data'		=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack('versions_query') )
			),
			'delete'	=> array(
				'icon'		=> 'times-circle',
				'title'		=> 'delete',
		...
		return \IPS\Theme::i()->getTemplate( 'trees', 'core' )->row( $this->url, $long, isset( $this->json[ $long ] ) ? $this->json[ $long ] : $long, $hasChildren, $buttons, $long, NULL, NULL, $root );
	}
	...
	protected function addVersion()
	{
		...
		switch ( $activeTab )
		{
			...
			case 'new':
				...
				{
					if ( !preg_match( '/^\d*\.\d*\.\d*(( (Alpha|Beta|RC)\s?[a-zA-Z0-9]*)|(\.\d*))?$/', $val ) )
					{
		...
		if( $values = $form->values() )
		{
			...
			switch ( $activeTab )
			{
				...
				case 'new':
					...
					{
						foreach ( \IPS\Application::$ipsApps as $_appKey )
						{
	...
	protected function addVersionQuery()
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'versions_query_code', '\IPS\Db::i()->', TRUE, array( 'size' => 45 ), function( $val ) use ( $latestVersion )
		{
			...
			/* Run it, but only if we are editing the latest version */
			if( \IPS\Request::i()->id == $latestVersion )
			{
				try
				{
					if ( @eval( $val ) === FALSE )
	...
	protected function versionCode()
	{
		...
		$phpFilePath = \IPS\ROOT_PATH  . "/applications/{$this->application->directory}/setup/" . ( $long === 'install' ? $long : "upg_{$long}" );
	...
	protected function editHook()
	{
		try
		{
			\IPS\Plugin\Hook::load( \IPS\Request::i()->hook )->editForm( \IPS\Http\Url::internal( "app=core&module=applications&controller=developer&appKey={$this->application->directory}&tab=hooks" ) );
		}
		catch ( \Exception $e )
		{
	...
	protected function _writeModules( $json )
	{
		foreach( $json as $location => $module )
		{
			foreach( $module as $name => $data )
			{
				foreach( $data as $k => $v )
				{
					if ( ! in_array( $k, array( 'protected', 'default_controller' ) ) )
					{
	...
	{ 
		...
		$latestVersion = $this->_getLatestVersion();
		$file          = \IPS\ROOT_PATH . "/applications/{$this->application->directory}/setup/" . ( $latestVersion['long'] === 'install' ? $latestVersion['long'] : "upg_{$latestVersion['long']}" )  . '/queries.json';
	 ...
	 * Get latest version
	 *
	 * @return	array	array( 'human' => '1.0.0', 'long' => 10000 )
	 */
	protected function _getLatestVersion()
	{
		...
		if ( empty( $versions ) )
		{
			\IPS\Output::i()->error( 'database_schema_needs_version', '1C103/C', 403, '' );
		}
		foreach ( array_reverse( $versions, TRUE ) as $long => $human )
		{
			break;
		}
		
		return array( 'human' => $human, 'long' => $long );
	}
