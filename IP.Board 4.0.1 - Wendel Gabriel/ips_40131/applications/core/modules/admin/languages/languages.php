...
class _languages extends \IPS\Node\Controller
{
	...
	{
		\IPS\Dispatcher::i()->checkAcpPermission( 'lang_packs' );
		parent::execute();
	}
		...
		if ( !\IPS\Request::i()->id )
		{
			...
			if ( $tab === 'new' )
			{
				...
				if ( $values = $form->values() )
				{
					...
					/* Log */
					\IPS\Session::i()->log( 'lang_created', array( $values['lang_title'] => FALSE ) );
	...
	protected function translate()
	{
		...
		{
			\IPS\Output::i()->error( 'no_translate_with_vle', '', 403 );		//@todo errorcode
		}
	...
	protected function download()
	{
		...
		if ( \IPS\Db::i()->select( 'COUNT(*)', 'core_sys_lang_words', array( 'lang_id=? AND word_export=1 AND word_custom IS NOT NULL', $lang->id ), 'word_id' )->first() < 1 )
		{
