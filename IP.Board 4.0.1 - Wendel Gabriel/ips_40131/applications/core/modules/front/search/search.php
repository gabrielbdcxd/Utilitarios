...
class _search extends \IPS\Dispatcher\Controller
{
	...
	protected function _results()
	{
		...
		if ( isset( \IPS\Request::i()->type ) and \IPS\Request::i()->type === 'core_members' )
		{
			...
			if ( \IPS\Request::i()->isAjax() )
			{
				\IPS\Output::i()->json( array(
					...
					'content'	=> \IPS\Theme::i()->getTemplate( 'search' )->results( \IPS\Request::i()->q, $title, $results, $pagination, $baseUrl, $count ),
					'title'		=> $title,
					'css'		=> array()
				) );
			}
			else
			{
				...
				\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'search' )->search( \IPS\Request::i()->q, $title, $results, $pagination, $baseUrl, $types, $this->_form()->customTemplate( array( \IPS\Theme::i()->getTemplate( 'search' ), 'filters' ) ), $count );
			}
			return;
		}
		...
		{
			$query->setOrder( \IPS\Content\Search\Query::ORDER_NEWEST_CREATED );
			$baseUrl = $baseUrl->setQueryString( 'sortby', 'newest' );
		}
		...
		if ( \IPS\Request::i()->isAjax() )
		{
			\IPS\Output::i()->json( array(
				'filters'	=> $this->_form()->customTemplate( array( \IPS\Theme::i()->getTemplate( 'search' ), 'filters' ) ),
				'content'	=> \IPS\Theme::i()->getTemplate( 'search' )->results( htmlentities( $term, \IPS\HTMLENTITIES, 'UTF-8', FALSE ), $title, $results, $pagination, $baseUrl, $results->count( TRUE ) ),
				'title'		=> $title,
				'css'		=> array()
			) );
		}
		else
		{
			...
			\IPS\Output::i()->title = $title;
			\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'search' )->search( htmlentities( $term, \IPS\HTMLENTITIES, 'UTF-8', FALSE ), $title, $results, $pagination, $baseUrl, $types, $this->_form()->customTemplate( array( \IPS\Theme::i()->getTemplate( 'search' ), 'filters' ) ), $results->count( TRUE ) );
		}
	}
	...
	public function _form()
	{
		...
		$memberFields	= array();
		foreach ( \IPS\core\ProfileFields\Field::fields( array(), \IPS\core\ProfileFields\SEARCH ) as $group => $fields )
		{
			$fieldsToAdd	= array();
			/* Fields */
			foreach ( $fields as $id => $field )
