...
class _messenger extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		{
			$folders = array_merge( $folders, array_filter( json_decode( \IPS\Member::loggedIn()->pconversation_filters, TRUE ) ) );
		}
		...
		/* Display */
		$pagination = \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->pagination( $baseUrl->setQueryString('_list', 1), ceil( $count / 25 ), ( \IPS\Request::i()->listPage ?: 1 ), 25, TRUE, 'listPage' );
		if ( \IPS\Request::i()->isAjax() )
		{
			if( \IPS\Request::i()->id )
			{
	...
	protected function addParticipant()
	{
		try
		{
			...
			if ( isset( \IPS\Request::i()->member_names ) )
			{
				...
				{
					$memberToAdd = \IPS\Member::load( $name, 'name' );
