...
class _profile extends \IPS\Helpers\CoverPhoto\Controller
{
	...
	protected function manage()
	{
		...
		$canAccessStatuses = ( \IPS\Settings::i()->profile_comments
			and \IPS\Member::loggedIn()->canAccessModule( \IPS\Application\Module::get( 'core', 'status' ) )
			and ( $this->member->pp_setting_count_comments or \IPS\Member::loggedIn()->member_id == $this->member->member_id ) );
		...
		if ( \IPS\Request::i()->status && \IPS\Request::i()->isAjax() && !\IPS\Request::i()->submitting && !\IPS\Request::i()->getUploader && $canAccessStatuses )
		{
			...
			{
				\IPS\Output::i()->output = \IPS\Output::i()->error( 'node_error', '2C138/D', 404, '' );
			}
		...
		if ( $canAccessStatuses and \IPS\Request::i()->status and \IPS\Request::i()->type == 'status' )
		{
			$statuses = $this->_getStatuses();
			$mainContent = \IPS\Theme::i()->getTemplate( 'profile' )->statusFeed( $this->member, $statuses['statuses'], $statuses['count'], $statuses['form'], TRUE );
		...
		else
		{
			...
			if ( $tab == 'activity' )
			{
				$latestActivity = \IPS\Content\Search\Query::init()->filterByAuthor( $this->member )->excludeFirstPostContentItems()->excludeDisabledApps()->setLimit( 15 )->setOrder( \IPS\Content\Search\Query::ORDER_NEWEST_CREATED )->search();
				$latestActivity->init( TRUE );
				$tabContents = \IPS\Theme::i()->getTemplate( 'profile' )->profileActivity( $this->member, $latestActivity );
			}
			elseif ( $tab == 'statuses' )
			{
				$statuses = $this->_getStatuses();
				$tabContents = \IPS\Theme::i()->getTemplate( 'profile' )->statusFeed( $this->member, $statuses['statuses'], $statuses['count'], $statuses['form'] );
			}
			elseif ( mb_substr( $tab, 0, 6 ) == 'field_' )
			{
				$fieldId = mb_substr( $tab, 6 );
				$tabContents = \IPS\Theme::i()->getTemplate( 'profile' )->fieldTab( $fieldId, $fields[ $fieldId ] );
			}
			...
			{
				\IPS\Output::i()->sendOutput( $tabContents );
			}
	...
	public function content()
	{
		...
		else
		{
			...
			else
			{
				...
				$output = new \IPS\Helpers\Table\Content( $currentClass, $this->member->url()->setQueryString( array( 'do' => 'content', 'type' => $currentType ) ), $where, NULL, NULL, 'read', FALSE );
			}
	...
	public function reputation()
	{
		...
		$containerClass = NULL;
		$permClass      = $currentClass;
		
		if ( isset( $currentClass::$itemClass ) )
		{
			$permClass = $currentClass::$itemClass;
		}
		
		if ( in_array( 'IPS\Content\Permissions', class_implements( $permClass ) ) )
		{
			$containerClass = $permClass::$containerNodeClass;
			$permissionKey = 'read';
			...
			if( count( $categories ) )
			{
				$where[] = array( $permClass::$databaseTable . "." . $permClass::$databasePrefix . $permClass::$databaseColumnMap['container'] . ' IN(' . implode( ',', $categories ) . ')' );
			}
			else
			{
				$where[] = array( $permClass::$databaseTable . "." . $permClass::$databasePrefix . $permClass::$databaseColumnMap['container'] . '=0' );
			}
		}
		...
		if ( $containerClass )
		{
			if ( isset( $currentClass::$itemClass ) )
			{
				$itemClass = $currentClass::$itemClass;
				
				$table->joins[] = array(
					'select' => $itemClass::$databaseTable . '.*',
					'from'  => $itemClass::$databaseTable,
					'where' => array( $currentClass::$databaseTable . '.' . $currentClass::$databasePrefix . $currentClass::$databaseColumnMap['item'] . '=' . $itemClass::$databaseTable . '.' . $itemClass::$databasePrefix . $itemClass::$databaseColumnId )
				);
			}
			
			$table->joins[] = array(
				'select' => $containerClass::$databaseTable . '.*',
				'from'  => $containerClass::$databaseTable,
				'where' => array( $permClass::$databaseTable . '.' . $permClass::$databasePrefix . $permClass::$databaseColumnMap['container'] . '=' . $containerClass::$databaseTable . '.' . $containerClass::$databasePrefix . $containerClass::$databaseColumnId )
			);
		}
	...
	protected function _getStatuses()
	{
		...
		if ( !count( $statuses ) )
		{
			...
			$start = ( intval( \IPS\Request::i()->statusPage ?: 1 ) - 1 ) * 25;

			if( $start > $count )
			{
				$start = 0;
				\IPS\Request::i()->statusPage = 1;
			}
