...
class _feed extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		{
			$offset					= ( intval( ( \IPS\Request::i()->statusPage AND \IPS\Request::i()->statusPage > 0 ) ? \IPS\Request::i()->statusPage : 1 ) - 1 ) * 25;
			$select					= \IPS\core\Statuses\Status::getItemsWithPermission( $where, NULL, array( $offset, 25 ), 'read', NULL );
        ...
        $pagination = ( isset( $statuses['count'] ) ) ? trim( \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->pagination( isset( \IPS\Request::i()->following ) ? \IPS\Http\Url::internal( 'app=core&module=status&controller=feed&following=1', NULL, 'status_following' ) : \IPS\Http\Url::internal( 'app=core&module=status&controller=feed', NULL, 'status' ), ceil( $statuses['count'] / 25 ), \IPS\Request::i()->statusPage ?: 1, 25, TRUE, 'statusPage' ) ) : NULL;
