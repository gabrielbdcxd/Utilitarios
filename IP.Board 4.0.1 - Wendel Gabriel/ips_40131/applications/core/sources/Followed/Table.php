...
{	
	...
	public function __construct( $className, $currentType )
	{
		...
		{
			$this->rowsTemplate = $className::contentTableTemplate();
			$containerClass = $className::$containerNodeClass;
			$this->group    = 'follow_id';
			
			$this->joins	= array(
				array( 'from' => 'core_permission_index', 'where' => array( "core_permission_index.app=? AND core_permission_index.perm_type=?", $containerClass::$permApp, $containerClass::$permType ) ),
			...
			{
				$containerClass = $className::$containerNodeClass;
				$this->where[0] .= ' AND (' . \IPS\Db::i()->findInSet( 'core_permission_index.perm_' . $containerClass::$permissionMap['view'], \IPS\Member::loggedIn()->groups ) . ' OR core_permission_index.perm_' . $containerClass::$permissionMap['view'] . "='*' )";
			}
			
			/* Additional where */
			$joinContainer = FALSE;
			$joins = array();
			if ( $extraWhere = $className::followWhere( $joinContainer, $joins ) )
			{
			...
			{
				$containerClass = $className::$containerNodeClass;
				$this->joins[] = array( 'from' => $containerClass::$databaseTable, 'where' => $containerClass::$databaseTable . '.' . $containerClass::$databasePrefix . $containerClass::$databaseColumnId . '=' . $className::$databaseTable . '.' . $className::$databasePrefix . $className::$databaseColumnMap['container'] );
			}
		...
		{
			$this->rowsTemplate = $className::nodeTableTemplate();

			$this->joins	= array(
				array( 'from' => 'core_permission_index', 'where' => array( "core_permission_index.app=? AND core_permission_index.perm_type=? AND core_permission_index.perm_type_id=?", $className::$permApp, $className::$permType, $className::$databaseTable . '.' . $className::$databasePrefix . $className::$databaseColumnId ) ),
			...
			{
				$this->where[1] .= ' AND (' . \IPS\Db::i()->findInSet( 'core_permission_index.perm_' . $className::$permissionMap['view'], \IPS\Member::loggedIn()->groups ) . ' OR ' . 'core_permission_index.perm_' . $className::$permissionMap['view'] . "='*' )";
			}
			
			/* Additional where */
			if ( method_exists( $className, 'followWhere' ) )
			{
				$extraWhere	= $className::followWhere();
