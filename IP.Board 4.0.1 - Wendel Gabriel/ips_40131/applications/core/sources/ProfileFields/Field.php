...
class _Field extends \IPS\CustomField
{
	 ...
	 * @return	array
	 */
	protected static function fieldData()
	{
	 ...
	 * @param	int			$location	\IPS\core\ProfileFields\PROFILE for profile, \IPS\core\ProfileFields\REG for registration screen or \IPS\core\ProfileFields\STAFF for ModCP/ACP
	 * @return	array
	 */
	public static function fields( $values=array(), $location=0 )
	{
		...
		foreach ( static::fieldData() as $groupId => $fields )
		{
			...
			{
				if ( ( $row['pf_admin_only'] and $location !== STAFF ) or ( $location === REG and !$row['pf_show_on_reg'] ) or ( $location === PROFILE and !$row['pf_member_edit'] ) or ( $location === SEARCH and !$row['pf_search_type'] ) )
				{
					continue;
				}
	...
	public static function values( $values, $location=0 )
	{
		...
		foreach ( static::fieldData() as $groupId => $fields )
		{
			foreach ( $fields as $row )
			{
				if ( $row['pf_admin_only'] and $location !== STAFF )
				{
					continue;
				}
	
				if ( $location == CONTENT and $row['pf_format'] != '' and isset( $values[ 'field_' . $row['pf_id'] ] ) )
				{
					$row = static::constructFromData( $row );
					$value = $row->displayValue( $values[ 'field_' . $row->id ] );
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\TextArea( 'pf_format', $this->id ? $this->format : '', FALSE, array( 'placeholder' => "<strong>{title}:</strong> {content}" ) ) );
