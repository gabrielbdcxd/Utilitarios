...
class _Upgrade
{
	...
	public static function systemRequirements()
	{
		...
		if ( version_compare( $mysqlVersion, '5.1.3' ) >= 0 )
		{
			...
			if ( version_compare( $mysqlVersion, '5.6.2' ) == -1 AND !mb_strpos( $mysqlVersion, "MariaDB" ) )
			{
	...
	protected function step3( $data )
	{
		return $this->appLoop( $data, function( $app )
		{
			...
			{
				@touch( \IPS\ROOT_PATH . '/plugins/hooks.txt' );
				@chmod( \IPS\ROOT_PATH . '/plugins/hooks.txt', 0777 );
			}

			if( !is_writable( \IPS\ROOT_PATH . '/plugins/hooks.txt' ) )
			{
				throw new \RuntimeException( sprintf( \IPS\Member::loggedIn()->language()->get("hook_file_not_writable"), \IPS\ROOT_PATH . '/plugins/hooks.txt' ) );
			}
	...
	protected function step5( $data )
	{
		return $this->appLoop( $data, function( $app ) use ($data)
		{
			...
			catch( \Exception $ex )
			{
				try
				{
					\IPS\Log::i( LOG_ERR )->write( "Step 5, " . $app . ' ' . $ex->getMessage(), 'upgrade_error' );
				}
				catch( \RuntimeException $e ){}
