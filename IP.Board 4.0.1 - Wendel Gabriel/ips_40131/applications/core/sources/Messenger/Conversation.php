...
class _Conversation extends \IPS\Content\Item
{
	...
	public static function canCreate( \IPS\Member $member, \IPS\Node\Model $container=NULL, $showError=FALSE )
	{
		...
		if ( $member->group['g_pm_flood_mins'] )
		{
		...
		/* Is our inbox full? */
		if ( $member->group['g_max_messages'] )
		{
		...
		foreach ( parent::formElements( $item, $container ) as $k => $v )
		{
			if ( $k == 'title' )
			{
 				if( !$item )
 				{
					...
					$return['to'] = new \IPS\Helpers\Form\Member( 'messenger_to', $member, TRUE, array( 'multiple' => ( \IPS\Member::loggedIn()->group['g_max_mass_pm'] == 0 ) ? NULL : \IPS\Member::loggedIn()->group['g_max_mass_pm'] ), function ( $members )
					{
	...
	public function canView( $member=NULL )
	{
		...
		if ( $member->modPermission('mod_see_warn') )
		{
			...
			if ( $warning )
			{
				$warning = \IPS\Db::i()->select( '*', 'core_members_warn_logs', array( 'wl_id=? AND wl_content_app=? AND wl_content_module=? AND wl_content_id1=?', $member->member_id, 'core', 'messaging', $this->id ) )->first();
				if ( $warning )
				{
					$_SESSION['report'] = $report->id;
					return TRUE;
