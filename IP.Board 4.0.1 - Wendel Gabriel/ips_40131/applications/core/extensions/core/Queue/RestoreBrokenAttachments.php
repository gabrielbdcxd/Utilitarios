...
class _RestoreBrokenAttachments
{
	...
	public function preQueueData( $data )
	{
		...
		try
		{
			\IPS\Log::i( LOG_DEBUG )->write( "Getting preQueueData for " . $classname, 'RestoreBrokenAttachments' );
		}
		...
		try
		{
			\IPS\Log::i( LOG_DEBUG )->write( "PreQueue count for " . $classname . " is " . $data['count'], 'RestoreBrokenAttachments' );
		}
		catch( \RuntimeException $e ){}
	...
	public function run( $data, $offset )
	{
		...
		try
		{
			\IPS\Log::i( LOG_DEBUG )->write( "Running " . $classname . ", with an offset of " . $offset, 'RestoreBrokenAttachments' );
		}
		catch( \RuntimeException $e ){}
