...
class _Profile
{
	...
	public function process( &$form, $member )
	{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'pp_setting_count_comments', $member->pp_setting_count_comments, FALSE ) );
		...
		if( count( $values ) )
		{
			foreach ( \IPS\core\ProfileFields\Field::fields( $values, \IPS\core\ProfileFields\STAFF ) as $group => $fields )
			{
	...
	public function save( $values, &$member )
	{
		...
		$member->pp_setting_count_comments								= $values['pp_setting_count_comments'];
		...
		foreach ( \IPS\core\ProfileFields\Field::fields( $profileFields, \IPS\core\ProfileFields\STAFF ) as $group => $fields )
		{
