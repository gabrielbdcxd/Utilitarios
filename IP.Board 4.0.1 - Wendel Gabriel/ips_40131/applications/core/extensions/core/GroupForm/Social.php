	...
	public function process( &$form, $group )
	{
		if( $group->g_id != \IPS\Settings::i()->guest_group )
		{
			...
			if ( $group->canAccessModule( \IPS\Application\Module::get( 'core', 'messaging', 'front' ) ) )
			{
				...
				$form->add( new \IPS\Helpers\Form\Number( 'g_pm_perday', $group->g_pm_perday, FALSE, array( 'unlimited' => 0, 'min' => 1 ), NULL, NULL, NULL, 'g_pm_perday' ) );
				$form->add( new \IPS\Helpers\Form\Number( 'g_pm_flood_mins', $group->g_pm_flood_mins, FALSE, array( 'unlimited' => 0, 'min' => 1 ), NULL, NULL, NULL, 'g_pm_flood_mins' ) );
				$form->add( new \IPS\Helpers\Form\Number( 'g_max_mass_pm', $group->g_max_mass_pm, FALSE, array( 'unlimited' => 0, 'max' => 500, 'min' => 1 ), NULL, NULL, NULL, 'g_max_mass_pm' ) );
				$form->add( new \IPS\Helpers\Form\Number( 'g_max_messages', $group->g_max_messages, FALSE, array( 'unlimited' => 0, 'min' => 1 ), NULL, NULL, NULL, 'g_max_messages' ) );
		...
		if ( \IPS\Settings::i()->reputation_enabled )
		{
			...
			if ( !$group->g_id or !in_array( $group->g_id, explode( ',', \IPS\Settings::i()->reputation_protected_groups ) ) )
			{
