...
class _Content
{
	...
	public function process( &$form, $group )
	{
		...
		if ( \IPS\Settings::i()->attach_allowed_types != 'none' )
		{
			...
			{
				$form->add( new \IPS\Helpers\Form\Number( 'g_attach_max', $group->g_attach_max, FALSE, array( 'unlimited' => -1 ), NULL, NULL, 'kB', 'g_attach_max' ) );
			}
			$form->add( new \IPS\Helpers\Form\Number( 'g_attach_per_post', $group->g_attach_per_post, FALSE, array( 'unlimited' => 0 ), NULL, NULL, 'kB', 'g_attach_per_post' ) );
		...
		$form->add( new \IPS\Helpers\Form\Custom( 'g_mod_post_unit', array( $group->g_mod_post_unit, $group->g_bitoptions['gbw_mod_post_unit_type'] ), FALSE, array( 'getHtml' => function( $element )
		{
	...
	public function save( $values, &$group )
	{
		...
		/* Bitwise */
		$values['gbw_disable_tagging'] = !$values['gbw_disable_tagging'];
		$values['gbw_disable_prefixes'] = !$values['gbw_disable_prefixes'];
