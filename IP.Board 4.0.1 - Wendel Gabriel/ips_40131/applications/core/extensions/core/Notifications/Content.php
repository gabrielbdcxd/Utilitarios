...
class _Content
{
	...
	public function parse_unapproved_content_bulk( $notification )
	{
		...
		$author = \IPS\Member::load( $notification->item_sub_id );
	...
	public function parse_new_review( $notification )
	{
		...
		$commenters = \IPS\Db::i()->select( 'DISTINCT ' . $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['author'], $commentClass::$databaseTable, array( $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['item'] . '=? AND ' . $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['date'] . '>=? AND ' . $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['date'] . '<? AND ' . $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['author'] . ' !=?', $item->id, $notification->sent_time->getTimestamp(), $between, \IPS\Member::loggedIn()->member_id ) );
	...
	public function parse_new_likes( $notification )
	{
		...
		$likers = \IPS\Db::i()->select( 'DISTINCT member_id', 'core_reputation_index', array( 'app=? AND type=? AND rep_date>=? AND rep_date<? AND type_id=?', $comment::$application, $comment::$reputationType, $notification->sent_time->getTimestamp(), $between, $comment->$idColumn ), 'rep_date desc' );
		...
		{
			return array(
					'title'		=> \IPS\Member::loggedIn()->language()->addToStack( 'notification__new_likes', FALSE, array( 'sprintf' => array( \IPS\Member::loggedIn()->language()->formatList( $names ), mb_strtolower( $comment->indefiniteArticle() ) ) ) ) . ' ' . $item->mapped('title'),
					'url'		=> ( $comment instanceof \IPS\Content\Comment ) ? $comment->url('find') : $comment->url(),
					'content'	=> $comment->truncated(),
					'author'	=> \IPS\Member::load( $likers->first() )
			);
		}
