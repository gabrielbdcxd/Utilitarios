<?php
/**
 * @brief		Bulk mail filter extension: member join date
 *
...
/**
 * @brief	Bulkmail filter: Member join date
 */
class _Joined
{
	/** 
	...
	{
		return array(
					new \IPS\Helpers\Form\DateRange( 'bmf_members_joined', isset( $criteria['range'] ) ? $criteria['range'] : '', FALSE ),
					);
	}
	...
	{
		return ( empty($post['bmf_members_joined']) ) ? FALSE : array( 'range' => $post['bmf_members_joined'] );
	}
	...
	public function getQueryWhereClause( $data )
	{
		if( !empty($data['range']) AND !empty($data['range']['end']) )
		{
			...
			return "core_members.joined BETWEEN {$start} AND {$end}";
		}
