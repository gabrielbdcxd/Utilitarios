...
class ipsConnect
{
	...
	public function __construct()
	{
		...
		$this->key	= md5( md5( \IPS\Settings::i()->sql_user . \IPS\Settings::i()->sql_pass ) . \IPS\Settings::i()->board_start );
	}
	...
	public function dispatch()
	{
		...
		if( \IPS\Request::i()->slaveCall == 1 )
		{
			...
			try
			{
				...
				{	
					if ( $noJson and isset( \IPS\Request::i()->returnTo ) )
					{
	...
	public function changeEmail()
	{
		...
		foreach ( \IPS\Login::handlers( TRUE ) as $handler )
		{
			try
			{
				$handler->changeEmail( $member, $member->email, \IPS\Request::i()->email );
			}
	...
	{
		if( $extra !== NULL AND \IPS\Request::i()->key != md5( $this->key . $extra ) )
		{
			if ( $returnOnError )
			{
				return FALSE;
		...
		else if( $extra === NULL AND \IPS\Request::i()->key != $this->key )
		{
			if ( $returnOnError )
			{
				return FALSE;
	...
	protected function _log( $message, $data=NULL )
	{
		...
		\IPS\Log::i( LOG_INFO )->write( implode( "\n", $log ), 'ipsconnect' );
	}
