...
class _Upgrade
{
	...
	public function step1()
	{
		...
		if ( !isset( \IPS\Request::i()->extra ) )
		{
			...
			{
				\IPS\Log::i( LOG_ERR )->write( "Error in upg_40000/Upgrade.php - About Me: " . $ex->getMessage(), 'upgrade' );
			}
		...
		if ( count( $select ) )
		{
			...
			foreach ( $select as $row )
			{
					...
					if ( is_array( $membersCache ) and isset( $membersCache['notifications'] ) )
					{
						...
						if ( count( $notifications ) )
						{
							foreach( $notifications as $type => $selected )
							{
								try
								{
									/* @link http://community.---.com/resources/bugs.html/_/4-0-0/error-upgrade-data-truncated-r49262 */
				...
				if( $row['pp_main_photo'] )
				{
					...
					if( !isset( $_test['host'] ) OR !$_test['host'] )
					{
						$row['pp_main_photo']	= $settings['upload_url'] . '/' . $row['pp_main_photo'];
					}
				}
				...
				if( $row['pp_thumb_photo'] )
				{
					...
					if( !isset( $_test['host'] ) OR !$_test['host'] )
					{
						$row['pp_thumb_photo']	= $settings['upload_url'] . '/' . $row['pp_thumb_photo'];
					}
				}
				...
				/* http://community.---.com/4bugtrack/temp_ban-issues-while-3x-to-4-upgrade-r1404/ */
				...
				{
					if ( ! in_array( $secondaryGroup, array( $bannedGroup, $validatingGroup, $guestGroup ) ) )
					{
		...
		else
		{
			...
			if ( $bannedGroup and $validatingGroup )
			{
				\IPS\Db::i()->delete( 'core_groups', 'g_id IN(' . $bannedGroup . ',' . $validatingGroup . ')' );
			}
			...
			unset( $_SESSION['aboutMe_Field'], $_SESSION['_step1Count'] );
	...
	public function step2()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'login_methods' ) as $login )
		{
			...
			$login['login_folder_name'] = ucfirst( $login['login_folder_name'] );
	...
	public function step3()
	{
		...
		\IPS\Db::i()->dropColumn( 'core_member_ranks', 'title' );
	...
	public function step6()
	{
		...
		if( isset( \IPS\Request::i()->extra ) and \IPS\Request::i()->extra !== true )
		{
			...
			foreach( \IPS\Db::i()->select( '*', 'core_pfields_content', NULL, NULL, array( $data['_count'], 500 ) ) as $row )
			{
				...
				foreach( $_SESSION['pfields'] as $k => $v )
				{
					...
					{
						list( $fieldKey, $fieldValue )	= explode( '=', $option );
					...
					if( $row['field_' . $k ] )
					{
						...
						{
							$update['field_' . $k ]	= ( isset( $realOptions[ $row['field_' . $k ] ] ) ) ? $realOptions[ $row['field_' . $k ] ][0] : NULL;
						}
					}
				}
		...
		foreach( \IPS\Db::i()->select( '*', 'core_pfields_data' ) as $field )
		{
			...
			if( $field['pf_content'] )
			{
				...
				$_SESSION['pfields'][ $field['pf_id'] ]	= $field['pf_content'];
	...
	public function step7()
	{
		try
		{
			...
			foreach( $notifications as $key => $data )
			{
				...
				/* @link http://community.---.com/4bugtrack/beta-4a-upgrade-error-location-step-7-application-core-r937/ */
				$selected	= '';
				...
				\IPS\Db::i()->insert( 'core_notification_defaults', array( 'notification_key' => $key, 'default' => $selected, 'disabled' => $disabled, 'editable' => $data['disable_override'] ), TRUE );
			}
		}
		catch( \UnderflowException $e )
		{
...
DROP INDEX like_lookup_area"
		...
		if ( count( $toRun ) )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'core', 'extra' => array( '_upgradeStep' => 9 ) ) );
	...
	public function step9()
	{
		...
		$groups	= array_filter( explode( ',', trim( \IPS\Settings::i()->cannot_ignore_groups, ',' ) ) );
	...
	public function step11()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'rc_reports_index', null, 'id ASC', array( $limit, $perCycle ) ) as $report )
		{
			...
			$insert	= array(
				'id'				=> $report['id'],
				'class'				=> null,
				'content_id'		=> $report['exdat3'] ?: ( $report['exdat2'] ?: $report['exdat1'] ),
				'perm_id'			=> 0,
			...
			switch( $classes[ $report['rc_class'] ] )
			{
				...
				case 'ccs':
					if( $report['exdat3'] )
					{
						$insert['class']	= "IPS\\cms\\Records\\Comment";
					}
					else
					{
						$insert['class']	= "IPS\\cms\\Records" . $report['exdata1'];
					}
					$table = "ccs_custom_database_" . $report['exdata1'];
					break;
			...
			try
			{
				...
				if( $table )
				{
					$row = \IPS\DB::i()->select( "*", $table, array( "{$className::$databaseColumnId} = ?", $report['exdat3'] ? $report['exdat2'] : $report['exdat1'] ) )->first();
					$content = $className::constructFromData( $row );

					if( $itemTable )
					{
						$itemClass = $className::$itemClass;
						$itemRow = \IPS\DB::i()->select("*", $itemTable, array("{$itemClass::$databaseColumnId} = ?", $content->mapped('item')))->first();
						$item = $itemClass::constructFromData($itemRow);

						$containerClass = $itemClass::$containerNodeClass;
						$containerRow = \IPS\DB::i()->select("*", $containerTable, array("{$containerClass::$databaseColumnId} = ?", $item->mapped('container')))->first();
			...
			/* Insert new report */
			\IPS\Db::i()->insert( 'core_rc_index', $insert );
		}
	...
	public function step12()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'core_members_warn_logs', null, 'wl_id ASC', array( $limit, $perCycle ) ) as $log )
		{
			...
			$update	= array(
				'wl_mq'				=> ( $log['wl_mq_unit'] AND $log['wl_mq'] AND $log['wl_mq'] != -1 ) ? 'P' . $log['wl_mq'] . ( ( mb_strtoupper( $log['wl_mq_unit'] ) == 'H' ) ? 'T' : '' ) . mb_strtoupper( $log['wl_mq_unit'] ) : null,
				'wl_rpa'			=> ( $log['wl_rpa_unit'] AND $log['wl_rpa'] AND $log['wl_rpa'] != -1 ) ? 'P' . $log['wl_rpa'] . ( ( mb_strtoupper( $log['wl_rpa_unit'] ) == 'H' ) ? 'T' : '' ) . mb_strtoupper( $log['wl_rpa_unit'] ) : null,
				'wl_suspend'		=> ( $log['wl_suspend_unit'] AND $log['wl_suspend'] AND $log['wl_suspend'] != -1 ) ? 'P' . $log['wl_suspend'] . ( ( mb_strtoupper( $log['wl_suspend_unit'] ) == 'H' ) ? 'T' : '' ) . mb_strtoupper( $log['wl_suspend_unit'] ) : null,
			...
			switch( $attach['attach_rel_module'] )
			{
				...
				case 'msg':
					...
					try
					{
						...
						$attach['attach_parent_id']	= $message;
					}
					catch( \UnderflowException $e ){}
				break;

				case 'ccs':
					$locationKey	= 'content_Records';
				break;
	...
	public function step14()
	{
		...
		if ( count( $toRun ) )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'core', 'extra' => array( '_upgradeStep' => 15 ) ) );
			
			/* Queries to run manually */
			return array( 'html' => \IPS\Theme::i()->getTemplate( 'forms' )->queries( $toRun, \IPS\Http\Url::internal( 'controller=upgrade' )->setQueryString( array( 'key' => $_SESSION['uniqueKey'], 'mr_continue' => 1, 'mr' => $mr, 'extra' => 0 ) ) ) );
		}
		...
		switch( $step )
		{
			case 'polls':
				foreach( \IPS\Db::i()->select( '*', 'core_polls', NULL, 'pid ASC', array( $limit, $perCycle ) ) as $poll )
				{
					...
					/* @link http://community.---.com/4bugtrack/beta-8-json_encode-invalid-utf-8-sequence-in-argument-during-upgrade-from-347-r2636/
	...
	public function step18()
	{
		...
		$url        = \IPS\Request::i()->url();
		$settings   = array( 'upload_dir' => \IPS\ROOT_PATH . '/uploads' , 'upload_url' => str_replace( 'http://', '//', preg_replace( '#/admin/.*$#', '/uploads', $url ) ) );
			...
			'idm_remotepass', 'idm_remotesspath', 'idm_remotefilepath', 'blog_upload_dir', 'blog_upload_url' )" ) ) as $row )
		{
			if ( $row['conf_value'] )
			{
				if ( in_array( $row['conf_key'], array( 'upload_dir', 'gallery_images_path', 'idm_localsspath', 'idm_localfilepath', 'blog_upload_dir' ) ) and !@is_dir( $row['conf_value'] ) )
				{
					continue;
				}
		...
		else
		{
			...
			if( \IPS\Db::i()->select( 'COUNT(*)', 'core_file_storage' )->first() == 0 )
			{
				...
				/* Gallery */
				if( isset( $settings['gallery_images_path'] ) AND $settings['gallery_images_path'] != $settings['upload_dir'] )
				{
					$url		= $settings['gallery_images_url'] ?: rtrim( \IPS\Settings::i()->base_url, '/' ) . '/applications/gallery/interface/legacy/image.php?path=';
					$gallery	= \IPS\Db::i()->insert( 'core_file_storage', array( 'method' => 'FileSystem', 'configuration' => json_encode( array( 'dir' => str_replace( \IPS\ROOT_PATH, '{root}', $settings['gallery_images_path'] ), 'url' => str_replace( $strReplace, '', $url ) ) ) ) );
				}
				else
				{
					$gallery	= $attachments;
				...
				/* Blog */
				if( isset( $settings['blog_upload_dir'] ) AND $settings['blog_upload_dir'] != $settings['upload_dir'] )
				{
					$blog	= \IPS\Db::i()->insert( 'core_file_storage', array( 'method' => 'FileSystem', 'configuration' => json_encode( array( 'dir' => str_replace( \IPS\ROOT_PATH, '{root}', $settings['blog_upload_dir'] ), 'url' => str_replace( $strReplace, '', $settings['blog_upload_url'] ) ) ) ) );
				}
				else
				{
					$blog	= $attachments;
					...
					else if( $settings['idm_filestorage'] == 'db' )
					{
						...
						if( \IPS\Db::i()->checkForTable( 'downloads_files_records' ) )
						{
							\IPS\Db::i()->insert( 'core_files',
								\IPS\Db::i()->select( "NULL, " . \IPS\Db::i()->prefix . "downloads_files_records.record_realname, MD5( RAND() ), " . \IPS\Db::i()->prefix . "downloads_filestorage.storage_file, NULL", 'downloads_filestorage' )->join( 'downloads_files_records', \IPS\Db::i()->prefix . "downloads_filestorage.storage_id=" . \IPS\Db::i()->prefix . "downloads_files_records.record_id" )
							);

							\IPS\Db::i()->insert( 'core_files',
								\IPS\Db::i()->select( "NULL, " . \IPS\Db::i()->prefix . "downloads_files_records.record_realname, MD5( RAND() ), " . \IPS\Db::i()->prefix . "downloads_filestorage.storage_ss, NULL", 'downloads_filestorage' )->join( 'downloads_files_records', \IPS\Db::i()->prefix . "downloads_filestorage.storage_id=" . \IPS\Db::i()->prefix . "downloads_files_records.record_id" )
							);

							\IPS\Db::i()->insert( 'core_files',
								\IPS\Db::i()->select( "NULL, " . \IPS\Db::i()->prefix . "downloads_files_records.record_realname, MD5( RAND() ), " . \IPS\Db::i()->prefix . "downloads_filestorage.storage_thumb, NULL", 'downloads_filestorage' )->join( 'downloads_files_records', \IPS\Db::i()->prefix . "downloads_filestorage.storage_id=" . \IPS\Db::i()->prefix . "downloads_files_records.record_id" )
							);
						}
					...
					else if( $settings['idm_filestorage'] == 'disk' )
					{
						...
						if( $useCustom )
						{
							$screenshotConfig = array( 
								...
								'url'			=> '',
								'toggle'		=> 1,
								'custom_url'	=> $_SESSION['screenshots_url'],
							);
						}
						else
						{
				...
				foreach( $tables[ $table ]['columns'] as $column )
				{
					if( $table === 'core_attachments' )
					{
						if( mb_strpos( $row[ $column ], '.ipb' ) !== FALSE AND file_exists( $settings['upload_dir'] . '/' . $row[ $column ] ) )
						{
							...
							$newPathname = implode( '/', $pathDetails ) . '/' . $row['attach_file'] . '.' . md5( uniqid( microtime() ) );
					...
					{
						if ( mb_strpos( $row[ $column ], 'team_icons' ) AND file_exists( \IPS\ROOT_PATH . '/' . $row[ $column ] ) )
						{
						...
						else
						{
							if( $row[ $column ] )
							{
								...
								if( !isset( $_test['host'] ) OR !$_test['host'] )
								{
									$update[ $column ]	= rtrim( \IPS\Settings::i()->base_url, '/' ) . '/' . $row[ $column ];
								}
							}
						}
	...
	public function step19()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'core_bulk_mail', null, 'mail_id ASC', array( $limit, $perCycle ) ) as $mail )
		{
			...
			try
			{
				$mail['mail_content']	= \IPS\Text\Parser::parseStatic( $mail['mail_content'], TRUE, NULL, NULL, TRUE, TRUE, TRUE );
			}
	...
	public function step20()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'core_follow', null, 'follow_id ASC', array( $limit, $perCycle ) ) as $follow )
		{
			...
			\IPS\Db::i()->update( 'core_follow', array( 'follow_notify_sent' => time() ), array( 'follow_id=?', $follow['follow_id'] ) );
		}
	...
	public function step20CustomTitle()
	{
		...
		{
			$_SESSION['_step20Count'] = \IPS\Db::i()->select( 'COUNT(*)', 'core_follow' )->first();
		}

		return "Upgrading last notify times for follows (Upgraded so far: " . ( ( $limit > $_SESSION['_step20Count'] ) ? $_SESSION['_step20Count'] : $limit ) . ' out of ' . $_SESSION['_step20Count'] . ')';
	}
	...
	public function step21()
	{
		...
		/* Update Search Engine setting - Sphinx needs new special setup for 4 */
		\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 'mysql' ), array( 'conf_key=?', 'search_engine' ) );

		/* Make sure license key does not have extra spaces at end */
		\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => trim( \IPS\Settings::i()->ipb_reg_number ) ), array( 'conf_key=?', 'ipb_reg_number' ) );
		...
		try
		{
			$guidelines	= \IPS\Text\Parser::parseStatic( \IPS\Settings::i()->gl_guidelines, TRUE, NULL, NULL, TRUE, TRUE, TRUE );
		}
		catch( \InvalidArgumentException $e )
		{
			if( $e->getcode() == 103014 )
		...
		try
		{
			$reg_rules	= \IPS\Text\Parser::parseStatic( \IPS\Settings::i()->reg_rules, TRUE, NULL, NULL, TRUE, TRUE, TRUE );
		}
		catch( \InvalidArgumentException $e )
		{
			if( $e->getcode() == 103014 )
		...
		try
		{
			$priv_body	= \IPS\Text\Parser::parseStatic( \IPS\Settings::i()->priv_body, TRUE, NULL, NULL, TRUE, TRUE, TRUE );
		}
		catch( \InvalidArgumentException $e )
		{
			if( $e->getcode() == 103014 )
		...
		\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 0 ), array( 'conf_key=?', 'task_use_cron' ) );
		...
		\IPS\Db::i()->delete( 'core_sys_conf_settings', "conf_key IN ('admob_bottom', 'admob_pub_id', 'admob_top', 'ad_code_board_index_footer', 'ad_code_board_index_header', 'ad_code_board_sidebar', 
			...
			'disable_subforum_show', 'disable_summary', 'disable_text_ajax', 'display_max_posts', 'display_max_topics', 'email_use_html', 'error_log_notify', 'etfilter_punct', 'etfilter_shout', 'fbc_appid', 
		...
		\IPS\Db::i()->insert( 'core_themes', array(
            ...
            "set_author_url"		=> "http:\/\/invision-virus.com",
            "set_emo_dir"			=> "'default'",
            "set_added"				=> 0,
            "set_updated"			=> 0,
		...
		foreach( array( 'ipb_url_whitelist', 'ipb_url_blacklist' ) as $list )
		{
			...
			{
				\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => implode( ",", explode( "\n", $settings[ $list ] ) ) ), array( 'conf_key=?', $list ) );
			}
		}
		
		if ( !isset( $settings['ipb_use_url_filter'] ) OR empty( $settings['ipb_use_url_filter'] ) )
		{
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 'none' ), array( 'conf_key=?', 'ipb_url_filter_option' ) );
		}
		...
		/* http://community.---.com/resources/bugs.html/_/ip-board/warn-logs-vs-members-warn-logs-r42251 */
		...
		\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => $acpNotes ), array( 'conf_key=?', 'acp_notes' ) );
