...
class _ICSParser
{
		...
		if ( count( $this->_parsedIcsData ) )
		{
			...
			foreach( $this->_parsedIcsData['events'] as $event )
			{
				...
				if( isset( $event['recurr'] ) )
				{
					try
					{
						$rrule = self::parseRrule( $event['recurr'] );
					}
				...
				if( $event['uid'] )
				{
					try
					{
						$eventId	= \IPS\Db::i()->select( 'import_event_id', 'calendar_import_map', array( 'import_guid=?', $event['uid'] ) )->first();
	 ...
	 *
	 * @param	array	$params	Parameters to use to generate UID
	 * @param	string
	 */
	...
	protected function _addEvents()
	{
		...
		if( count( $this->_events ) )
		{
			foreach( $this->_events as $event )
			{
				...
				$output	.= "UID:" . $this->_buildUid( array( $event->saved, $event->id ) ) . "\r\n";
				...
				if( $event->_end_date )
				{
					...
					{
						$output	.= "DTSTART;TZID=Europe/London:" . $event->_start_date->modifiedIso8601( TRUE, TRUE ) . "\r\n";
						$output	.= "DTEND;TZID=Europe/London:" . $event->_end_date->modifiedIso8601( TRUE, TRUE ) . "\r\n";
					}
				}
				else
				{
	 ...
	 * @link	http://community.---.com/tracker/issue-33787-formatting-problem-with-imported-ics-file-calendar-app/
	 */
	 ...
	 * @param	int		$start	Line number
	 * @return	void
	 * @link	http://community.---.com/resources/bugs.html/_/ip-calendar/recurring-events-can-sometimes-be-skipped-in-ics-r41033
	 */
	protected function _processEventObject( $start )
	{
		...
		for( $i = $start, $j = count( $this->_rawIcsData ); $i < $j; $i++ )
		{
			...
			switch( $_type )
			{
				...
				case 'SUMMARY':
					/* @link	http://community.---.com/tracker/issue-32941-ical-summary/ */
