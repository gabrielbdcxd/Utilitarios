...
\IPS\Content\Embeddable
{
	...
	public function get__recurring_text()
	{
		...
		if( $recurringData['event_repeat_freq'] > 1 )
		{
			...
			$params[]	= $recurringData['event_repeat_freq'];
		}
		...
		return  \IPS\Member::loggedIn()->language()->addToStack( $langString, FALSE, array( 'sprintf' => $params ) );
	}
	...
	public function findOccurrences( $startDate, $endDate )
	{
		...
		/* Now figure out when this event occurs on */
		$instances		= array();
		$occurrences	= 0;
		$keyword		= NULL;
		...
		switch( $recurringData['event_repeats'] )
		{
			...
			case 'weekly':
				...
				{
					$date			= $this->_start_date;
					$eDate			= $this->_end_date;
		...
		$this->parsedOccurrences	= $results;
	...
	public function url( $action=NULL )
	{
		...
		{
			$this->_url[ $_key ] = \IPS\Http\Url::internal( "app=calendar&module=calendar&controller=event&id={$this->id}", 'front', 'calendar_event', $this->title_seo );
	...
	public function moderateNewComments( \IPS\Member $member )
	{
		...
		return $this->container()->comment_moderate and !$member->group['g_avoid_q'];
	}
	...
	public function moderateNewReviews( \IPS\Member $member )
	{
		...
		return $this->container()->review_moderate and !$member->group['g_avoid_q'];
	}
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		$dateValues	= array(
			'single_day'				=> $item ? !$item->end_date : TRUE,
		...
		$return['location']		= new \IPS\Helpers\Form\Address( 'event_location', ( $item AND $item->location ) ? \IPS\GeoLocation::buildFromJson( $item->location ) : NULL, FALSE, array( 'minimize' => TRUE ) );

		/* Gallery album association */
		if( \IPS\Application::appIsEnabled( 'gallery' ) )
		{
			$return['album']	= new \IPS\Helpers\Form\Node( 'event_album', ( $item AND $item->event_album ) ? $item->event_album : NULL, FALSE, array( 
				'url'					=> \IPS\Http\Url::internal( 'app=calendar&module=calendar&controller=submit', 'front', 'calendar_submit' ),
	...
	public function processForm( $values )
	{
		...
		/* RSVP options */
		$this->rsvp			= $values['event_rsvp'];
		$this->rsvp_limit	= $values['event_rsvp_limit'];
	 ...
	 * @param	\IPS\calendar\Calendar|NULL	$calendar		Calendar to filter by
	...
	public static function retrieveEvents( $startDate, $endDate=NULL, $calendar=NULL, $limit=NULL, $formatEvents=TRUE, $member=NULL )
	{
		...
		$events	= \IPS\calendar\Event::getItemsWithPermission( array_merge( $where, $nonRecurring ), 'event_start_date ASC', NULL, 'read', NULL, 0, $member  );
		...
		$recurringEvents	= \IPS\calendar\Event::getItemsWithPermission( array_merge( $where, array( array( 'event_recurring IS NOT NULL' ) ) ), 'event_start_date ASC', NULL, 'read', NULL, 0, $member );
