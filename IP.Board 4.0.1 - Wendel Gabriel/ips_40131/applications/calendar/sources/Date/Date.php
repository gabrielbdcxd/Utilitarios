...
class _Date extends \IPS\DateTime
{
	...
	public function calendarDate()
	{
		...
		{
			return \IPS\Member::loggedIn()->language()->convertString( strftime( \IPS\Settings::i()->calendar_date_format_custom, $this->getTimestamp() + $this->getTimezone()->getOffset( $this ) ) );
		}
		elseif( isset( static::$dateFormats[ \IPS\Settings::i()->calendar_date_format ] ) )
		{
			return \IPS\Member::loggedIn()->language()->convertString( strftime( static::$dateFormats[ \IPS\Settings::i()->calendar_date_format ], $this->getTimestamp() + $this->getTimezone()->getOffset( $this ) ) );
		}
		else
		{
	 ...
	 * @return	array
	 */
	public function getBirthdays( $day=FALSE )
	{
		$birthdays	= array();
		...
		$members	= \IPS\DB::i()->select( '*', 'core_members', $where );
		...
		if( $members->count() )
		{
			foreach( $members as $member )
			{
				...
				{
					if ( $member->temp_ban > 0 OR $member->temp_ban == -1 )
					{
						$banned = TRUE;
					}
				}
