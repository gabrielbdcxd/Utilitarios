...
class _topic extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		/* One for the brits */
		if ( mb_strpos( $topic->title, base64_decode( 'c3BhbmlzaA==' ) ) !== FALSE and $topic->isQuestion() )
		{
			\IPS\Output::i()->output .= base64_decode( 'PCEtLSBOTyBPTkUgRVhQRUNUUyBUSEUgU1BBTklTSCBJTlFVSVNJVElPTiAtLT4=' );
		}
		...
		$metaDescription = strip_tags( $topic->isQuestion() ? $question->post : $current->post );
		...
		if( isset( $_SESSION['_findComment'] ) )
		{
			...
			try
			{
				...
				\IPS\Output::i()->metaTags['description'] = \IPS\Output::i()->metaTags['og:description'] = strip_tags( $comment->content() );
			}
			catch( \Exception $e ){}
		}
	...
	public function edit()
	{
		...
		if ( !$topic->canEdit() )
		{
			\IPS\Output::i()->error( 'node_noperm_edit', '2F173/E', 403, '' );
		}
		...
		if ( $class::modPermission( 'lock', NULL, $forum ) )
		{
			$hasModOptions = TRUE;
		}
	...
	public function rateAnswer()
	{
		...
		$answer->post_field_int = \IPS\Db::i()->select( 'SUM(rating)', 'forums_answer_ratings', array( 'post=?', $answer->pid ) )->first();
		$answer->save();
