...
class _forums extends \IPS\Dispatcher\Controller
{
	...
	public function _forum( $forum )
	{
		...
		$table = new \IPS\Helpers\Table\Content( 'IPS\forums\Topic', $forum->url(), $where, $forum, NULL, NULL );
		...
		$table->advancedSearchCallback = function( $table, $values )
		{
			...
			switch ( $values['topic_type'] )
			{
				...
				case 'replied':
					...
					break;
				case 'answered':
					$table->where[] = 'topic_answered_pid<>0';
					break;
				case 'unanswered':
					$table->where[] = 'topic_answered_pid=0';
					break;
				case 'queued_topics':
					$table->where[] = 'approved=0';
					break;
				case 'queued_posts':
					$table->where[] = 'topic_queuedposts>0';
					break;
			}
			...
			if ( $days !== NULL )
			{
				$table->where[] = array( 'forums_topics.last_post>?', \IPS\DateTime::create()->sub( new \DateInterval( 'P' . $days . 'D' ) ) );
			}
		};
		...
		if ( \IPS\Settings::i()->forums_rss and $forum->topics )
		{
			...
			if ( $forum->forums_bitoptions['bw_enable_answers'] )
			{
				$rssTitle = \IPS\Member::loggedIn()->language()->addToStack( 'forum_rss_title_questions', FALSE, array( 'sprintf' => array( $forum->_title ) ) );
			}
			else
			{
				$rssTitle = \IPS\Member::loggedIn()->language()->addToStack( 'forum_rss_title_topics', FALSE, array( 'sprintf' => array( $forum->_title ) ) );
			}
			...
			if ( isset( \IPS\Request::i()->rss ) )
			{
				...
				/* We can't include "moved" links in the RSS feed, as there is no content to include ( $topic->content() results in BadMethodCallException) */
	...
	protected function add()
	{
		...
		$formTemplate = $form->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'submit', 'forums' ) ), 'createTopicForm' ), $forum, $hasModOptions );
