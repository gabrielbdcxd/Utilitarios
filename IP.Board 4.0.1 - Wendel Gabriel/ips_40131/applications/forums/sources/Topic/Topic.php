	...
	\IPS\Content\Embeddable
{
	...
	{
		$joinContainer = FALSE;
		return array_merge( parent::vncWhere( $joinContainer, $joins ), static::getItemsWithPermissionWhere( array(), 'read', NULL, $joinContainer ) );
	}
	...
	public static function getItemsWithPermissionWhere( $where, $permissionKey, $member, &$joinContainer, $skipPermission=FALSE )
	{
		...
		if ( !$skipPermission and in_array( $permissionKey, array( 'view', 'read' ) ) )
		{
			...
			/* Don't show links to moved topics */
			$where['item'][] = array( "( NULLIF(forums_topics.moved_to, '') IS NULL )" );
	...
	public function processForm( $values )
	{
        ...
        else if( isset( $values['topic_create_state'] ) and !in_array( 'lock', $values['topic_create_state'] ) )
        {
