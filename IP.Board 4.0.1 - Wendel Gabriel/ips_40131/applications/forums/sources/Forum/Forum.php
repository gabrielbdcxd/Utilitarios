...
class _Forum extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	{
		if ( $this->sort_key === 'last_post' )
		{
			return 'last_real_post';
		}
	...
	public function lastPost()
	{
		...
		(
			...
			(
				!(
					\IPS\Member::loggedIn()->modPermission( 'forums' ) === 0
					or
					(
		...
		else
		{
			...
			{
				$result = array(
					'author'		=> \IPS\Member::load( $this->last_poster_id ),
	...
	public function loggedInMemberHasPasswordAccess()
	{
		...
		if ( isset( \IPS\Request::i()->cookie[ 'ipbforumpass_' . $this->id ] ) and md5( $this->password ) === \IPS\Request::i()->cookie[ 'ipbforumpass_' . $this->id ] )
		{
			return TRUE;
		}
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'forum_permission_showtopic', $this->permission_showtopic, FALSE, array(), NULL, NULL, NULL, 'forum_permission_showtopic' ) );
		$form->add( new \IPS\Helpers\Form\YesNo( 'forum_permission_showtopic_qa', $this->permission_showtopic, FALSE, array(), NULL, NULL, NULL, 'forum_permission_showtopic_qa' ) );
		...
		$form->add( new \IPS\Helpers\Form\Select( 'forum_sort_key', $this->id ? $this->sort_key : 'last_post', FALSE, array( 'options' => array( 'last_post' => 'sort_updated', 'posts' => 'sort_num_comments', 'views' => 'sort_views', 'title' => 'sort_title', 'starter_name' => 'sort_author_name', 'last_poster_name' => 'sort_last_comment_name', 'start_date' => 'sort_date' ) ), NULL, NULL, NULL, 'forum_sort_key' ) );
		...
		if ( \IPS\Settings::i()->tags_enabled )
		{
			...
			{
				$form->add( new \IPS\Helpers\Form\Text( 'forum_tag_predefined', $this->tag_predefined, FALSE, array( 'autocomplete' => array( 'unique' => 'true' ), 'nullLang' => 'forum_tag_predefined_unlimited' ), NULL, NULL, NULL, 'forum_tag_predefined' ) );
			}
		}
	}
	...
	public function url()
	{
		if( $this->_url === NULL )
		{
			$this->_url = \IPS\Http\Url::internal( "app=forums&module=forums&controller=forums&id={$this->_id}", 'front', 'forums_forum', array( $this->name_seo ) );
		}
	...
	/**
	 * If there is only one forum (and it isn't a redirect forum), that forum, or NULL
	 *
	...
	public function searchIndexPermissions()
	{
		...
		foreach ( $this->parents() as $parent )
		{
			...
			$parentResult = $permissions['perm_view'];
