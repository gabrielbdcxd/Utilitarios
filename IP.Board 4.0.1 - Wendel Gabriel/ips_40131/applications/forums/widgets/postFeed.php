...
class _postFeed extends \IPS\Widget\PermissionCache
{
	...
	public function configuration( &$form=null )
	{
 		if ( $form === null )
		{
 		...
 		$form->add( new \IPS\Helpers\Form\Text( 'tfb_title', isset( $this->configuration['tfb_title'] ) ? $this->configuration['tfb_title'] : \IPS\Member::loggedIn()->language()->addToStack('block_postFeed_title') ) );

		$form->add( new \IPS\Helpers\Form\Node( 'tfb_forums', ( isset( $this->configuration['tfb_forums'] ) and is_array( $this->configuration['tfb_forums'] ) ) ? array_keys( $this->configuration['tfb_forums'] ) : 0, FALSE, array(
		...
		$form->add( new \IPS\Helpers\Form\Member( 'tfb_author', $author, FALSE, array( 'multiple' => NULL ) ) );
 	...
 	/**
 	 * Adjust configuration values we are about to save
 	 *
 	...
 	{
 		if( isset( $values['tfb_author'] ) AND $values['tfb_author'] )
 		{
 			$values['tfb_author']	= array_keys( $values['tfb_author'] );
 		}
	...
	public function render()
	{
		...
		{
			$where['item'][] = array( 'forum_id IN (' . implode( ",", array_keys( $this->configuration['tfb_forums'] ) ) . ')' );
		}
		...
		if ( isset( $this->configuration['tfb_topic_status'] ) and is_array( $this->configuration['tfb_topic_status'] ) )
		{
			...
			if ( ! in_array( 'open', $status ) or ! in_array( 'closed', $status ) )
			{
				...
				{
					$where['item'][] = array( "state='closed'" );
				}
				else if ( ! in_array( 'closed', $status ) )
				{
					$where['item'][] = array( "state='open'" );
				}
			}
			...
			if ( ! in_array( 'featured', $status ) or ! in_array( 'notfeatured', $status ) )
			{
				...
				{
					$where['item'][] = array( 'featured=0' );
				}
				else if ( ! in_array( 'notfeatured', $status ) )
				{
					$where['item'][] = array( 'featured=1' );
				}
			}
			...
			if ( ! in_array( 'pinned', $status ) or ! in_array( 'notpinned', $status ) )
			{
				...
				{
					$where['item'][] = array( 'pinned=0' );
				}
				else if ( ! in_array( 'notpinned', $status ) )
				{
					$where['item'][] = array( 'pinned=1' );
				}
			}
		}
		...
		if ( count( $posts ) )
		{
			return $this->output( $posts, isset( $this->configuration['tfb_title'] ) ? $this->configuration['tfb_title'] : \IPS\Member::loggedIn()->language()->addToStack('block_postFeed_title') );
		}
		else
		{
			return '';
