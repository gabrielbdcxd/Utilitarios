...
class _Upgrade
{
	...
	public function step1()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'blog_blogs', null, 'blog_id ASC', array( $limit, 50 ) ) as $blog )
		{
			...
			\IPS\Lang::saveCustom( 'blog', "blogs_blog_{$blog->id}", html_entity_decode( $original['blog_name'], ENT_QUOTES, 'UTF-8' ) );
			...
			\IPS\Lang::saveCustom( 'blog', "blogs_groupblog_name_{$blog->id}", html_entity_decode( $original['blog_groupblog_name'], ENT_QUOTES, 'UTF-8' ) );
		...
		{
			$columns = array( 'blog_name', 'blog_desc', 'blog_groupblog_name' );

			if( \IPS\Db::i()->checkForColumn( 'blog_blogs', 'blog_type' ) )
			{
				$columns[]	= 'blog_type';
			}
	...
	public function step2()
	{
		\IPS\Db::i()->insert( 'core_ratings', 
			\IPS\Db::i()->select( "NULL, 'IPS\\blog\\Entry', entry_id, member_id, rating, ''", 'blog_ratings', NULL, NULL, NULL, array( 'entry_id', 'member_id' ) )
		);
