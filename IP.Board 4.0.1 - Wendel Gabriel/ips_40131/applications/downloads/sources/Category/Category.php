...
class _Category extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public function get_name_furl()
	{
		...
		{
			$this->name_furl	= \IPS\Http\Url::seoTitle( \IPS\Lang::load( \IPS\Lang::defaultLanguage() )->get( 'downloads_category_' . $this->id ) );
			$this->save();
		}

		return $this->_data['name_furl'] ?: \IPS\Http\Url::seoTitle( \IPS\Lang::load( \IPS\Lang::defaultLanguage() )->get( 'downloads_category_' . $this->id ) );
	}
	...
	public function form( &$form )
	{
		...
		{
			$form->add( new \IPS\Helpers\Form\Select( 'ccfields', $this->id ? explode( ',', $this->_data['cfields'] ) : array(), FALSE, array( 'options' => $customFields, 'multiple' => TRUE ), NULL, NULL, NULL, 'cfields' ) );
		}
		...
		$form->add( new \IPS\Helpers\Form\Select( 'csortorder', $this->sortorder ?: 'updated', FALSE, array( 'options' => array( 'file_updated' => 'sort_updated', 'file_last_comment' => 'last_reply', 'file_name' => 'file_title', 'rating' => 'sort_rating', 'file_submitted' => 'sort_date', 'num_comments' => 'sort_num_comments', 'num_reviews' => 'sort_num_reviews', 'file_views' => 'sort_num_views' ) ), NULL, NULL, NULL, 'csortorder' ) );
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'clog_on', $this->log !== 0, FALSE, array( 'togglesOn' => array( 'clog' ) ) ) );
		$form->add( new \IPS\Helpers\Form\Number( 'clog', $this->log === NULL ? -1 : $this->log, FALSE, array( 'unlimited' => -1 ), NULL, NULL, array(
			...
			'postUnlimited'	=> ( $this->id and \IPS\Member::loggedIn()->hasAcpRestriction( 'downloads', 'downloads', 'categories_recount_downloads' ) ) ? '<a data-confirm data-confirmSubMessage="' . \IPS\Member::loggedIn()->language()->addToStack('clog_recount_desc') . '" href="' . \IPS\Http\Url::internal( "app=downloads&module=downloads&controller=categories&do=recountDownloads&id={$this->id}") . '">' . \IPS\Member::loggedIn()->language()->addToStack('clog_recount') . '</a>' : ''
		), 'clog' ) );
		$form->add( new \IPS\Helpers\Form\YesNo( 'cbitoptions_submitter_log', $this->bitoptions['submitter_log'], FALSE ) );
		...
		if ( \IPS\Application::appIsEnabled( 'forums' ) )
		{
			...
			$form->add( new \IPS\Helpers\Form\YesNo( 'cforum_on', $this->forum_id, FALSE, array( 'togglesOn' => array(
				'cforum_id',
				'ctopic_prefix',
				'ctopic_suffix',
	...
	public function formatFormValues( $values )
	{
		...
		foreach ( array( 'cname' => "downloads_category_{$this->id}", 'cdesc' => "downloads_category_{$this->id}_desc", 'cdisclaimer' => "downloads_category_{$this->id}_disclaimer", 'csubmissionterms' => "downloads_category_{$this->id}_subterms", 'noperm_view' => "downloads_category_{$this->id}_npv", 'noperm_dl' => "downloads_category_{$this->id}_npd" ) as $fieldKey => $langKey )
		{
			if ( isset( $values[ $fieldKey ] ) )
			{
				...
				if ( $fieldKey === 'cname' )
				{
					$this->name_furl = \IPS\Http\Url::seoTitle( $values[ $fieldKey ][ \IPS\Lang::defaultLanguage() ] );
				}
		...
		{
			if ( isset( $values[ "cbitoptions_{$k}" ] ) )
			{
		...
		if( isset( $values['clog_on'] ) )
		{
			unset( $values['clog_on'] );
		}

		if( isset( $values['cversioning_on'] ) )
		{
	...
	{
		$fields = array();
		foreach ( explode( ',', $this->_data['cfields'] ) as $fieldId )
		{
			try
			{
				$fields[ $fieldId ] = \IPS\downloads\Field::load( $fieldId );
			}
	...
	public function url()
	{
		if( $this->_url === NULL )
		{
			$this->_url = \IPS\Http\Url::internal( "app=downloads&module=downloads&controller=browse&id={$this->_id}", 'front', 'downloads_cat', $this->name_furl );
		}
	...
	public function lastFile()
	{
		...
		try
		{
			$latestFile = \IPS\downloads\File::load( $this->last_file_id );
		}
		catch ( \OutOfRangeException $e )
		{
			$latestFile = NULL;
		...
		{
			$childLatest = $child->lastFile();

			if( $childLatest !== NULL AND ( $latestFile === NULL OR $childLatest->updated > $latestFile->updated ) )
			{
	...
	public function setLastFile( \IPS\downloads\File $file=NULL )
	{
		if( $file === NULL )
		{
			try
			{
				$file	= \IPS\downloads\File::constructFromData( \IPS\Db::i()->select( '*', 'downloads_files', array( 'file_cat=? AND file_open=1', $this->id ), 'file_submitted DESC', 1 )->first() );
			}
