...
class _view extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		$previousVersions = iterator_to_array( \IPS\Db::i()->select( '*', 'downloads_filebackup', array( "b_fileid=?", $this->file->id ), 'b_backup DESC' )->setKeyField( 'b_id' ) );
	...
	protected function buy()
	{
		...
		if ( $fees = json_decode( \IPS\Settings::i()->idm_nexus_transfee, TRUE ) )
		{
	...
	protected function _download( \IPS\File $file )
	{
		...
		/* Log session */
		$downloadSessionId = md5( uniqid( microtime(), true ) );
	...
	protected function deletePreviousVersion()
	{
		...
		/* Delete */
		foreach ( new \IPS\File\Iterator( \IPS\Db::i()->select( 'record_location', 'downloads_files_records', array( array( 'record_type=?', 'upload' ), \IPS\Db::i()->in( 'record_id', explode( ',', $version['b_records'] ) ) ) ), 'downloads_Files' ) as $file )
		{
			try
			{
				$file->delete();
		...
		foreach ( new \IPS\File\Iterator( \IPS\Db::i()->select( 'record_location', 'downloads_files_records', array( array( 'record_type=?', 'ssupload' ), \IPS\Db::i()->in( 'record_id', explode( ',', $version['b_records'] ) ) ) ), 'downloads_Screenshots' ) as $file )
		{
			try
			{
				$file->delete();
	...
	protected function newVersion()
	{
		...
		$form->add( new \IPS\Helpers\Form\Editor( 'file_changelog', $this->file->changelog, FALSE, array( 'app' => 'downloads', 'key' => 'Downloads', 'autoSaveKey' => "downloads-{$this->file->id}-changelog", 'attachIds' => array( $this->file->id ) ) ) );
		...
		if ( $category->bitoptions['allowss'] )
		{
			...
			if ( \IPS\Member::loggedIn()->group['idm_linked_files'] )
			{
				$form->add( new \IPS\Helpers\Form\Stack( 'url_screenshots', iterator_to_array( \IPS\Db::i()->select( 'record_location', 'downloads_files_records', array( 'record_file_id=? AND record_type=? AND record_backup=0', $this->file->id, 'sslink' ) ) ), FALSE, array( 'stackFieldType' => 'Url' ), array( 'IPS\downloads\File', 'blacklistCheck' ) ) );
			}
		}
		...
		if ( $values = $form->values() )
		{
			...
			if ( $category->versioning !== 0 and ( !\IPS\Member::loggedIn()->group['idm_bypass_revision'] or $values['file_save_revision'] ) )
			{
				...
				\IPS\Db::i()->insert( 'downloads_filebackup', array(
					'b_fileid'		=> $this->file->id,
					'b_filetitle'	=> $this->file->name,
					'b_filedesc'	=> $this->file->desc,
					'b_hidden'		=> FALSE,
					'b_backup'		=> $this->file->updated,
					'b_updated'		=> time(),
					'b_records'		=> implode( ',', iterator_to_array( \IPS\Db::i()->select( 'record_id', 'downloads_files_records', array( 'record_file_id=? AND record_backup=0', $this->file->id ) ) ) ),
					'b_version'		=> $this->file->version,
					'b_changelog'	=> $this->file->changelog,
				) );
				
				/* Set the old records to be backups */
				\IPS\Db::i()->update( 'downloads_files_records', array( 'record_backup' => TRUE ), array( 'record_file_id=? AND record_backup=0', $this->file->id ) );
								
				/* Delete any old versions we no longer keep */
				if ( $category->versioning !== NULL )
				{
					$count = \IPS\Db::i()->select( 'COUNT(*)', 'downloads_filebackup', array( 'b_fileid=?', $this->file->id ) )->first();
					if ( ( $count - $category->versioning + 1 ) > 0 )
					{
						foreach ( \IPS\Db::i()->select( '*', 'downloads_filebackup', array( 'b_fileid=?', $this->file->id ), 'b_backup ASC', $count - $category->versioning + 1 ) as $backUp )
						{
			...
			{
				$existingRecords = array_unique( iterator_to_array( \IPS\Db::i()->select( 'record_location', 'downloads_files_records', array( 'record_file_id=? AND record_type=?', $this->file->id, 'upload' ) ) ) );
				$existingScreenshots = array_unique( iterator_to_array( \IPS\Db::i()->select( 'record_location', 'downloads_files_records', array( 'record_file_id=? AND record_type=?', $this->file->id, 'ssupload' ) ) ) );
				$existingLinks = array_unique( iterator_to_array( \IPS\Db::i()->select( 'record_id, record_location', 'downloads_files_records', array( 'record_file_id=? AND record_type=?', $this->file->id, 'link' ) )->setKeyField('record_id')->setValueField('record_location') ) );
                $existingScreenshotLinks = array_unique( iterator_to_array( \IPS\Db::i()->select( 'record_id, record_location', 'downloads_files_records', array( 'record_file_id=? AND record_type=?', $this->file->id, 'sslink' ) )->setKeyField('record_id')->setValueField('record_location') ) );
			}
			...
			{
				foreach ( $values['url_screenshots'] as $url )
				{
			...
			try
			{
				$this->file->primary_screenshot = \IPS\Db::i()->select( 'record_id', 'downloads_files_records', array( 'record_file_id=? AND record_type=? AND record_backup=0', $this->file->id, 'ssupload' ), 'record_default DESC, record_id ASC' )->first();
			}
			catch ( \UnderflowException $e ) { }
