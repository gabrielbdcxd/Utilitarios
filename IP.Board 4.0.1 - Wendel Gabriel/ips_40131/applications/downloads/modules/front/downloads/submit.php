...
class _submit extends \IPS\Dispatcher\Controller
{
	...
	public function execute()
	{
		...
		\IPS\downloads\File::canCreate( \IPS\Member::loggedIn(), NULL, TRUE );
	...
	protected function manage()
	{
		...
		$form->add( new \IPS\Helpers\Form\Node( 'category', isset( $data['category'] ) ? $data['category'] : NULL, TRUE, array(
		...
		{
			$url = \IPS\Http\Url::internal( 'app=downloads&module=downloads&controller=submit&do=submit', 'front', 'downloads_submit' )->setQueryString( 'category', $values['category']->_id );
	...
	{		
		...
		$steps['upload_files'] = function( $data )
		{
			...
			/* Add the fields */
			$form->add( new \IPS\Helpers\Form\Upload( 'files', $files, ( !\IPS\Member::loggedIn()->group['idm_linked_files'] and !\IPS\Member::loggedIn()->group['idm_import_files'] ), array( 'storageExtension' => 'downloads_Files', 'allowedFileTypes' => $category->types, 'maxFileSize' => $category->maxfile !== NULL ? ( $category->maxfile / 1024 ) : NULL, 'multiple' => TRUE, 'minimize' => FALSE ) ) );
			...
			if ( !isset( \IPS\Request::i()->bulk ) )
			{
				...
				if ( $category->bitoptions['allowss'] )
				{
					...
					if ( \IPS\Member::loggedIn()->group['idm_linked_files'] )
					{
						$form->add( new \IPS\Helpers\Form\Stack( 'url_screenshots', isset( $data['url_screenshots'] ) ? $data['url_screenshots'] : array(), FALSE, array( 'stackFieldType' => 'Url' ), array( 'IPS\downloads\File', 'blacklistCheck' ) ) );
					}
				}
			...
			if ( $values = $form->values() )
			{
				...
				if ( empty( $values['files'] ) and empty( $values['url_files'] ) and empty( $values['import_files'] ) )
				{
					...
					return \IPS\Theme::i()->getTemplate( 'submit' )->submitForm( $form, $category, $category->message('subterms'), ( \IPS\Member::loggedIn()->group['idm_bulk_submit'] && \IPS\Request::i()->bulk ) );
				}
					...
					foreach ( $values['screenshots'] as $_key => $file )
					{
						...
						if ( !isset( $existing[ (string) $file ] ) )
						{
							...
							\IPS\Db::i()->insert( 'downloads_files_records', array(
								...
								'record_realname'		=> $file->originalFilename,
								'record_size'			=> \strlen( $file->contents() ),
								'record_time'			=> time(),
		...
		$steps['file_information'] = function ($data)
		{
			...
			if ( $values = $form->values() )
			{
				...
				foreach ( $data['files'] as $key => $fileUrl )
				{
					...
					$save = array(
						'file_title'	=> $values["filedata_{$key}_file_title"],
						'file_desc'		=> $values["filedata_{$key}_file_desc"],
						'file_tags'		=> $values["filedata_{$key}_file_tags"],
						'file_version'	=> $values["filedata_{$key}_file_version"],
						'postKey'		=> md5( "{$data['postKey']}-{$key}" )
					);
					\IPS\downloads\File::createFromForm( $save, $category, FALSE );
				}
		...
		if ( \IPS\downloads\File::moderateNewItems( \IPS\Member::loggedIn() ) )
		{
