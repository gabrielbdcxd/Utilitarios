...
{	
	...
	{		
		/* Create the table */
		$table = new \IPS\Helpers\Table\Db( 'nexus_ship_orders', \IPS\Http\Url::internal( 'app=nexus&module=payments&controller=shipping' ) );
	...
	public function ship()
	{
		...
		if ( \IPS\Settings::i()->easypost_api_key )
		{
			...
			$matrix = NULL;
			$fromCountry = \IPS\GeoLocation::buildFromJson( \IPS\Settings::i()->site_address )->country;
	...
	public function easypost()
	{
		...
		if ( $values = $form->values() )
		{
			try
			{
				...
				{
					\IPS\Output::i()->error( 'err_err_easypost', '3X193/9', 500, '', array(), $response['error'] );
				}
	...
	public function printout()
	{
		...
		{
			$output = \IPS\Theme::i()->getTemplate('shiporders')->packingLabel( $shipment );
			\IPS\Output::i()->cssFiles = array_merge( \IPS\Output::i()->cssFiles, \IPS\Theme::i()->css( 'shipping.css', 'nexus' ) );			
		}
		else
		{
			$output = \IPS\Theme::i()->getTemplate('shiporders')->packingSheet( $shipment );
			\IPS\Output::i()->cssFiles = array_merge( \IPS\Output::i()->cssFiles, \IPS\Theme::i()->css( 'shipping.css', 'nexus' ) );
		}

		\IPS\Output::i()->sendOutput( \IPS\Theme::i()->getTemplate( 'global', 'core', 'front' )->blankTemplate( $output ), 200, 'text/html', \IPS\Output::i()->httpHeaders );
	}
