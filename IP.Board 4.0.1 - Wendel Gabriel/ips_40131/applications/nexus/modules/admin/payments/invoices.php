...
{	
	...
	public function paid()
	{
		...
		/* Send Email */
		\IPS\Email::buildFromTemplate( 'nexus', 'invoiceMarkedPaid', array( $invoice, $invoice->summary() ) )
			->send(
				$invoice->member,
				array_map(
					function( $contact )
		...
		\IPS\Output::i()->output = new \IPS\Helpers\Wizard(
			array(
				...
				{					
					...
					$form->add( new \IPS\Helpers\Form\Number( 't_amount', $amountToPay, TRUE, array( 'min' => 0.01, 'max' => $amountToPay, 'decimals' => TRUE ), NULL, NULL, $invoice->currency ) );
				...
				'checkout_pay'	=> function( $data ) use ( $invoice, $gateways, $self )
				{
					...
					{
						foreach ( $gateway->paymentScreen( $invoice, $amountToPay ) as $element )
						{
							if ( !$element->htmlId )
							{
	...
	public function credit()
	{
		...
		$maxCanCharge = ( $credit < $amountToPay ) ? $credit : $amountToPay;
		
		/* Build Form */
		$form = new \IPS\Helpers\Form( 'amount', 'invoice_charge_to_credit' );
		$form->add( new \IPS\Helpers\Form\Number( 't_amount', $maxCanCharge, TRUE, array( 'min' => 0.01, 'max' => $maxCanCharge, 'decimals' => TRUE ), NULL, NULL, $invoice->currency ) );
		...
		{			
			...
			$credits[ $invoice->currency ]->amount -= $values['t_amount'];
	...
	public function unpaid()
	{
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'payments', 'transactions_refund' ) )
		{
			foreach ( $transactions as $transaction )
			{
				...
				/* Partial refund? */
				$field = new \IPS\Helpers\Form\Number( $transaction->id . '_refund_amount', ( $transaction->status === \IPS\nexus\Transaction::STATUS_PART_REFUNDED ? $transaction->amount->amount - $transaction->partial_refund->amount : 0 ), TRUE, array( 'unlimited' => 0, 'unlimitedLang' => \IPS\Member::loggedIn()->language()->addToStack( 'refund_full', FALSE, array( 'sprintf' => array( $transaction->amount ) ) ), 'max' => ( $transaction->amount->amount - $transaction->partial_refund->amount ), 'decimals' => TRUE ), NULL, NULL, $transaction->amount->currency, $transaction->id . '_refund_amount' );
		...
		{
			$statusOptions = array();
			if ( $invoice->total->amount !== (float) 0 )
			{
	...
	public function generate()
	{
		...
		if ( !isset( \IPS\Request::i()->member ) )
		{
			$steps['invoice_generate_member'] = function( $data )
			{
				...
				$form->add( new \IPS\Helpers\Form\Member( 'invoice_generate_member' ) );
				if ( $values = $form->values() )
				{
		...
		$steps['invoice_generate_settings'] = function( $data )
		{
			...
			if ( count( $addresses ) )
			{
				...
				$form->add( new \IPS\Helpers\Form\Radio( 'shipping_address', ( $shipping == $billing ) ? 'x' : $shipping, TRUE, array( 'unlimited' => 'x', 'unlimitedLang' => 'same_as_billing_address', 'options' => $options, 'toggles' => array( 0 => array( 'new_shipping_address' ) ) ) ) );
			...
			{
				$form->add( new \IPS\Helpers\Form\Address( 'new_billing_address', NULL, TRUE ) );
				$form->add( new \IPS\Helpers\Form\Checkbox( 'same_shipping_address', TRUE, FALSE, array( 'togglesOff' => array( 'new_shipping_address' ) ) ) );
			...
			if ( $values = $form->values() )
			{
				...
				elseif ( ( isset( $values['same_shipping_address'] ) and $values['same_shipping_address'] ) or $values['shipping_address'] === 'x' )
				{
				...
				else
				{
					...
					{
						$form->error = \IPS\Member::loggedIn()->language()->addToStack('shipping_address_required');
						return $form;
					}
		...
		$steps['invoice_generate_items'] = function( $data ) use ( $url )
		{
			...
			if ( isset( \IPS\Request::i()->add ) and isset( $itemTypes[ \IPS\Request::i()->add ] ) )
			{
				...
				$formUrl = \IPS\Http\Url::internal('app=nexus&module=payments&controller=invoices&do=generate')->setQueryString( 'add', \IPS\Request::i()->add );				
