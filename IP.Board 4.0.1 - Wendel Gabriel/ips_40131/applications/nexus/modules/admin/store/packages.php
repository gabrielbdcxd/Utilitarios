...
class _packages extends \IPS\Node\Controller
{
		...
		if ( !isset( \IPS\Request::i()->processing ) )
		{
			if ( isset( $changes['renew_options'] ) )
			{
				...
				{				
					...
					{
						$changes['renew_options'][] = array( 'old' => $data, 'new' =>  $values[ $k ]['now_pay'] );
					}
					$changes['renew_options'][] = array( 'old' => 'x', 'new' => $values['x']['now_pay'] );
				}
				else
				{
	 ...
	 * @return	void
	 */
	protected function view_purchases()
	{
		...
		$table = new \IPS\Helpers\Table\Db( 'nexus_purchases', \IPS\Http\Url::internal( "app=nexus&module=store&controller=packages&do=view_purchases&id={$package->id}" ), array( 'ps_app=? AND ps_type=? AND ps_item_id=?', 'nexus', 'package', $package->id ) );
