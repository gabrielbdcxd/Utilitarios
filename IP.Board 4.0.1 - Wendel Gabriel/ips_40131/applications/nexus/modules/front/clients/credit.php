	...
	protected function manage()
	{
		...
		if ( \IPS\Settings::i()->nexus_min_topup )
		{
			...
			foreach ( array_merge( array( \IPS\nexus\Customer::loggedIn() ), iterator_to_array( \IPS\nexus\Customer::loggedIn()->parentContacts( array('billing=1') ) ) ) as $account )
			{
				...
				{
					$max = $maximumBalanceAmounts ? ( $maximumBalanceAmounts[ $value->currency ]['amount'] - $value->amount ) : NULL;
		...
		/* Display */
		\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate('clients')->template( \IPS\Theme::i()->getTemplate('clients')->credit( $balance, $pastWithdrawals, $pastWithdrawalsPagination, $canWithdraw, $canTopup ) );
	}
	...
	protected function withdraw()
	{
		...
		if ( count( $withdrawOptions ) )
		{
			...
			foreach ( \IPS\nexus\Money::currencies() as $currency )
			{
				...
				$field = new \IPS\Helpers\Form\Number( 'withdraw_amount_' . $currency, $max, count( $currencyOptions ) ? NULL : TRUE, array( 'min' => $min, 'max' => $max, 'decimals' => TRUE ), count( $currencyOptions ) ? NULL : function( $val ) use ( $currency )
				{
			...
			if ( $canWithdraw )
			{
				...
				{					
					...
					$credits[ $currency ]->amount -= $values[ 'withdraw_amount_' . $currency ];
		...
		if ( \IPS\Settings::i()->nexus_min_topup )
		{
			...
			foreach ( array_merge( array( \IPS\nexus\Customer::loggedIn() ), iterator_to_array( \IPS\nexus\Customer::loggedIn()->parentContacts( array('billing=1') ) ) ) as $account )
			{
				...
				foreach ( $account->cm_credits as $value )
				{
					...
					if ( is_null( $max ) or $max > 0 )
					{
						$maximums[ $account->member_id ][ $value->currency ] = $max;
					}
				}
			}
			...
			if ( count( $maximums ) )
			{
				...
				foreach ( $maximums as $accountId => $data )
				{
					...
					foreach ( $data as $currency => $maximum )
					{
						...
						$field = new \IPS\Helpers\Form\Number( 'topup_amount_' . $accountId . '_' . $currency, NULL, NULL, array( 'decimals' => TRUE ), function( $val ) use ( $currency, $accountOptions, $accountId, $data, $min, $maximum )
						{
	...
	protected function cancel()
	{
		...
		try
		{
			...
			if ( $payout->member->member_id === \IPS\nexus\Customer::loggedIn()->member_id and $payout->status === $payout::STATUS_PENDING )
			{
				...
				$credits[ $payout->amount->currency ]->amount += $payout->amount->amount;
