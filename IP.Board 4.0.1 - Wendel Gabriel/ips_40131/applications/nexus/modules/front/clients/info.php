	...
	protected function manage()
	{
		...
		if ( $values = $form->values( TRUE ) )
		{
			...
			foreach ( \IPS\nexus\Customer\CustomField::roots() as $field )
			{
				...
				{
					$changes[] = array( 'name' => 'nexus_ccfield_' . $field->id, 'value' => $values["nexus_ccfield_{$field->id}"], 'old' => \IPS\nexus\Customer::loggedIn()->$column );
					\IPS\nexus\Customer::loggedIn()->$column = $values["nexus_ccfield_{$field->id}"];
				}
		...
		\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate('clients')->template( \IPS\Theme::i()->getTemplate('clients')->info( $form ) );
	}
}