...
class _checkout extends \IPS\Dispatcher\Controller
{
	...
	{		
		...
		{
			\IPS\Output::i()->error( 'no_module_permission', '2X196/1', 403, '' );
		}
	...
	public function _customer()
	{
		...
		$form = new \IPS\Helpers\Form( 'customer', $buttonLang, $this->invoice->checkoutUrl() );
		...
		if ( $this->needsToLogin )
		{
			$customFields = \IPS\core\ProfileFields\Field::fields( $guestData ? $guestData['profileFields'] : NULL, \IPS\core\ProfileFields\REG );
		...
		if ( $this->needsToLogin )
		{
			...
			\IPS\Member::loggedIn()->language()->words[ "reg_agreed_terms" ] = \IPS\Member::loggedIn()->language()->get( 'reg_agreed_terms', FALSE, array( 'sprintf' => array( \IPS\Http\Url::internal( 'app=core&module=system&controller=terms', 'front', 'terms' ) ) ) );
			
			if ( \IPS\Settings::i()->privacy_type == "internal" )
			{
				\IPS\Member::loggedIn()->language()->words[ "reg_agreed_terms" ] .= \IPS\Member::loggedIn()->language()->get( 'reg_privacy_link', FALSE, array( 'sprintf' => array( \IPS\Http\Url::internal( 'app=core&module=system&controller=privacy', 'front', 'privacy' ), 'data-ipsDialog data-ipsDialog-size="wide" data-ipsDialog-title="' . \IPS\Member::loggedIn()->language()->get("privacy") . '"' ) ) );
			}
			else if ( \IPS\Settings::i()->privacy_type == "external" )
			{
				\IPS\Member::loggedIn()->language()->words[ "reg_agreed_terms" ] .= \IPS\Member::loggedIn()->language()->get( 'reg_privacy_link', FALSE, array( 'sprintf' => array( \IPS\Http\Url::external( \IPS\Settings::i()->privacy_link ), 'target="_blank"' ) ) );
			}
		...
		if ( $values = $form->values() )
		{
			...
			if ( $this->needsToLogin )
			{
				...
				foreach ( \IPS\core\ProfileFields\Field::fields( array(), \IPS\core\ProfileFields\REG ) as $group => $fields )
				{
					...
					{
						$profileFields[ "field_{$id}" ] = $values[ $field->name ];
					}
				}
			...
			else
			{
				...
				foreach ( \IPS\nexus\Customer\CustomField::roots() as $field )
				{
					if ( $field->purchase_show )
					{
						...
						{
							$changes['other'][] = array( 'name' => 'nexus_ccfield_' . $field->id, 'value' => $values["nexus_ccfield_{$field->id}"], 'old' => $this->invoice->member->$column );
	...
	public function _shipping()
	{
		...
		{
			if ( $rate->isAvailable( $this->invoice->shipaddress ) )
			{
			...
			foreach ( $shippingGroups as $key => $data )
			{
				if ( array_key_exists( 'easypost', $data['methods'] ) )
				{
					...
					foreach ( $data['items'] as $item )
					{
						...
						{
							$v = "{$k}InInches";
							$$v += ( $item->$k->float('in') * $item->quantity );
						}
					}
	...
	public function _pay( $data )
	{
		...
		if ( isset( \IPS\Request::i()->split ) and \IPS\Request::i()->split < $amountToPay->amount )
		{
			$amountToPay->amount = \IPS\Request::i()->split;
		}
		
		/* Nothing to pay? */
		if ( $amountToPay->amount === (float) 0 )
		{
		...
		{
			foreach ( $gateway->paymentScreen( $this->invoice, $amountToPay ) as $element )
			{
				if ( !$element->htmlId )
				{
		...
		{
			$form->add( new \IPS\Helpers\Form\Checkbox( 'i_agree_to_tac', FALSE, TRUE, array( 'labelHtmlSprintf' => array( "<a href='" . htmlentities( \IPS\Settings::i()->nexus_tac_link, \IPS\HTMLENTITIES, 'UTF-8', FALSE ) . "' target='_blank'>" . \IPS\Member::loggedIn()->language()->addToStack( 'terms_and_conditions' ) . '</a>' ) ), function( $val )
			{
				if ( !$val )
				{
		...
		if ( $values !== FALSE )
		{
			...
			if ( isset( $values['payment_method'] ) and $values['payment_method'] == 0 )
			{
				...
				if ( $transaction->amount->amount > $inWallet )
				{
				...
				$credits[ $this->invoice->currency ]->amount -= $transaction->amount->amount;
				...
				if ( !$this->invoice->amountToPay()->amount )
				{	
					$this->invoice->markPaid();
				}
			...
			try
			{
				$transaction->auth = $gateway->auth( $transaction, $values, $maxMind );
			}
		...
		/* Display */
		return \IPS\Theme::i()->getTemplate('checkout')->confirmAndPay( $this->invoice, $this->invoice->summary(), $form->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'checkout', 'nexus' ) ), 'paymentForm' ), $this->invoice, $amountToPay ), $amountToPay, $couponForm ? $couponForm->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'checkout', 'nexus' ) ), 'couponForm' ) ) : NULL );
	}
	...
	public function split()
	{
		...
		/* What is the max? */
		$maxSplitAmount = $invoice->amountToPay()->amount - $minSplitAmount;
