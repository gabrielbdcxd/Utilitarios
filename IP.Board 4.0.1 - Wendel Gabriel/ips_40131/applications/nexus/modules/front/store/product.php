...
class _product extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		$category = \IPS\nexus\Package\Group::load( $item->group );
		...
		else
		{
			...
			if ( $package->physical )
			{
				...
				{
					if ( ( $shipAddress and $rate->isAvailable( $shipAddress ) ) or ( $rate->locations === '*' ) )
					{	
				...
				/* Easypost */
				if ( \IPS\Settings::i()->easypost_api_key and \IPS\Settings::i()->easypost_show_rates and ( $package->shipping == '*' or in_array( 'easypost', explode( ',', $package->shipping ) ) ) )
				{
					try
					{
						$fromAddress = \IPS\GeoLocation::buildFromJson( \IPS\Settings::i()->site_address );
	...
	protected function _getForm( \IPS\nexus\Package $package, $inCart, $verticalForm = FALSE )
	{
		...
		if ( $values = $form->values() )
		{
			...
			{
				if ( mb_substr( $k, 0, 13 ) == 'nexus_pfield_' )
				{
					$class = \IPS\nexus\Package\CustomField::load( mb_substr( $k, 13 ) )->buildHelper();
					$details[ mb_substr( $k, 13 ) ] = $class::stringValue( $v );
				}
			}
			...
			if ( \IPS\Request::i()->isAjax() && isset( \IPS\Request::i()->stockCheck ) )
			{
				...
				if ( !empty( $renewOptions ) )
				{
					...
					if ( $term['add'] )
					{
						$data['price']->amount += $term['cost'][ $memberCurrency ]['amount'];
						$normalPrice->amount += $term['cost'][ $memberCurrency ]['amount'];
					}
					
					$return['renewal'] = \IPS\Member::loggedIn()->language()->addToStack( 'and_renewal', FALSE, array( 'sprintf' => array( new \IPS\nexus\Purchase\RenewalTerm( new \IPS\nexus\Money( $term['cost'][ $memberCurrency ]['amount'] + $_data['renewalAdjustment'], $memberCurrency ), new \DateInterval( 'P' . $term['term'] . mb_strtoupper( $term['unit'] ) ), isset( $this->tax ) ? \IPS\nexus\Tax::load( $this->tax ) : NULL, $term['add'] ) ) ) );
				}
				else
				{
					$return['renewal'] = '';
				...
				if ( \IPS\Settings::i()->nexus_show_tax and $package->tax )
				{
					try
					{
						...
						$data['price']->amount += ( $data['price']->amount * $taxRate );
						$normalPrice->amount += ( $normalPrice->amount * $taxRate );
					}
					catch ( \OutOfRangeException $e ) { }
				}
				
				/* Format and return */
				if ( $data['price']->amount != $normalPrice->amount )
				{
			...
			if ( count( $renewOptions ) )
			{
				...
				$renewalTerm = new \IPS\nexus\Purchase\RenewalTerm( new \IPS\nexus\Money( $chosenRenewOption['cost'][ $memberCurrency ]['amount'] + $data['renewalAdjustment'], $memberCurrency ), new \DateInterval( 'P' . $chosenRenewOption['term'] . mb_strtoupper( $chosenRenewOption['unit'] ) ), isset( $this->tax ) ? \IPS\nexus\Tax::load( $this->tax ) : NULL, $chosenRenewOption['add'], $package->grace_period ? new \DateInterval( 'P' . $package->grace_period . 'D' ) : NULL );
			}
			...
			/* Blah */
			$package->addItemsToCartData( $details, $quantity, $renewalTerm, $parent, $values );
