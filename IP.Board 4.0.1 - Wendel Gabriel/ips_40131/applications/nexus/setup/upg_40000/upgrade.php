...
class _Upgrade
{
	...
	{		
		...
		\IPS\Db::i()->update( 'nexus_paymethods', array( 'm_gateway' => 'SagePay' ), array( 'm_gateway=?', 'sagepay' ) );
	...
	{			
		...
		if ( count( $select ) )
		{
			foreach ( $select as $customer )
			{
				...
				$address->addressLines[] = $customer['cm_address_1'];
				if ( $customer['cm_address_2'] )
				{
					$address->addressLines[] = $customer['cm_address_2'];
				}
				$address->city = $customer['cm_city'];
				$address->region = $customer['cm_state'];
	...
	public function step3()
	{
		...
		$toRun = \IPS\core\Setup\Upgrade::runManualQueries( array( array(
			'table' => 'core_members',
			'query' => "ALTER TABLE " . \IPS\Db::i()->prefix . "core_members CHANGE cm_credits cm_credits TEXT NULL;"
		),
		array(
			'table' => 'core_members',
		...
		if ( count( $toRun ) )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'nexus', 'extra' => array( '_upgradeStep' => 4 ) ) );
	...
	public function step6()
	{
		if( isset( \IPS\Settings::i()->idm_nexus_transfee ) )
		{
			...
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => json_encode( $transFee ) ), array( 'conf_key=?', 'idm_nexus_transfee' ) );
		}
	...
	public function step8()
	{
		foreach ( \IPS\Db::i()->select( '*', 'nexus_hosting_servers' ) as $server )
		{
			\IPS\Db::i()->update( 'nexus_hosting_servers', array(
				'server_nameservers'	=> json_encode( explode( '<br />', $server['server_nameservers'] ) ),
	...
	public function step14()
	{
		...
		{		
			...
			{		
				...
				\IPS\Lang::saveCustom( 'nexus', "nexus_package_{$row['p_id']}_desc", \IPS\Text\Parser::parseStatic( $row['p_desc'], TRUE, NULL, NULL, TRUE, TRUE, TRUE ) );
				\IPS\Lang::saveCustom( 'nexus', "nexus_package_{$row['p_id']}_page", \IPS\Text\Parser::parseStatic( $row['p_page'], TRUE, NULL, NULL, TRUE, TRUE, TRUE ) );
	...
	/**
	 * Translatables: Support Severities
	 *
	 * @return	mixed	If returns TRUE, upgrader will proceed to next step. If it returns any other value, it will set this as the value of the 'extra' GET parameter and rerun this step (useful for loops)
	 */
		...
		{		
			...
			{		
				...
				\IPS\Db::i()->update( 'nexus_tax', array(
					't_rate'	=> json_encode( \unserialize( $row['t_rate'] ) ),
				), array( 't_id=?', $row['t_id'] ) );
			}
	...
	public function step20()
	{
		...
		{		
			...
			{		
				...
				\IPS\Db::i()->update( 'nexus_support_stock_actions', array( 'action_message' => \IPS\Text\Parser::parseStatic( $row['action_message'], TRUE, NULL, NULL, TRUE, TRUE, TRUE ) ), array( 'action_id=?', $row['action_id'] ) );
			}
		...
		{		
			...
			{		
				...
				\IPS\Db::i()->update( 'nexus_customer_fields', array(
					'f_extra'		=> json_encode( \unserialize( $row['f_extra'] ) ),
					'f_type'		=> $type,
					'f_multiple'	=> $multiple,
				), array( 'f_id=?', $row['f_id'] ) );
			}
	 ...
	 * @return string
	 */
	public function step26CustomTitle()
	{
		$limit = isset( \IPS\Request::i()->extra ) ? \IPS\Request::i()->extra : 0;

		if( !isset( $_SESSION['_step26Count'] ) )
		{
			$_SESSION['_step26Count'] = \IPS\Db::i()->select( 'COUNT(*)', 'nexus_customer_history' )->first();
		}

		return "Upgrading commerce customer history (Upgraded so far: " . ( ( $limit > $_SESSION['_step26Count'] ) ? $_SESSION['_step26Count'] : $limit ) . ' out of ' . $_SESSION['_step26Count'] . ')';
	}
		...
		{		
			...
			{		
				\IPS\Db::i()->update( 'nexus_purchases', array(
					...
					'ps_extra'				=> json_encode( \unserialize( $row['ps_extra'] ) ),
