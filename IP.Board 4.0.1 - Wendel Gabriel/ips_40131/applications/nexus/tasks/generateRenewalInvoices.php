...
class _generateRenewalInvoices extends \IPS\Task
{
	...
	public function execute()
	{
        ...
        $select = \IPS\Db::i()->select( '*', 'nexus_purchases', array(
            'ps_renewals>0 AND ps_invoice_pending=0 AND ps_active=1 AND ps_expire>0 AND ps_expire<?',
		...
		foreach ( $groupedPurchases as $memberId => $currencies )
		{
			...
			{				
				...
				/* Nothing to pay? */
				if ( $invoice->amountToPay()->amount === (float) 0 )
				{
				...
				if ( $invoice->status !== $invoice::STATUS_PAID )
				{
					...
					if( $credit > 0 )
					{
						$take = $credit > $invoice->total->amount ? $invoice->total->amount : $credit;
