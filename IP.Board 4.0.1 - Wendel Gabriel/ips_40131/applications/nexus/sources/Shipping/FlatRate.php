...
class _FlatRate extends \IPS\Node\Model implements \IPS\nexus\Shipping\Rate
{
	...
	{		
		...
		{			
			$rates = array();
			/* Populate */
			foreach ( $values[ "s_rates_{$values['s_type']}" ] as $rate )
			{
	...
	{		
		...
		else
		{
			...
			{
				if ( $locations[ $destination->country ] === '*' )
				{
					return TRUE;
				}
				else
				{
					return in_array( $destination->region, $locations[ $destination->country ] );
				}
			}
	...
	public function getPrice( array $items, $currency, \IPS\nexus\Invoice $invoice = NULL )
	{
		switch ( $this->type )
		{
			case 't':
				...
				else
				{
					...
					{
						$criteria += ( $item->price->amount * $item->quantity );
					}
				}
				break;
			...
			case 'w':
				...
				foreach ( $items as $item )
				{
					...
					{
						$criteria += ( $item->weight->kilograms * $item->quantity );
					}
				}
		}
		...
		foreach ( $rates as $rate )
		{
			...
			if ( ( $min === '*' or $criteria >= $min ) and ( $max === '*' or $criteria <= $max ) )
			{
