						...
						switch ( $val['type'] )
						{
							case 'used':
								return 
									...
									$val['code'],
									\IPS\Theme::i()->getTemplate('global', 'nexus')->userLink( \IPS\nexus\Customer::load( $val['by'] ) )
								) ) );
								break;
						...
						switch ( $val['type'] )
						{
							...
							case 'attach':
								...
								if ( isset( $val['ps_id'] ) and $val['ps_id'] )
								{
									...
									{
										$options = array( 'htmlsprintf' => array( $file, htmlentities( $val['ps_name'], ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ) ) );
									}
