...
class _Package extends \IPS\Node\Model
{
	...
	public function __clone()
	{
		...
		\IPS\Db::i()->update( 'nexus_package_fields', "cf_packages=CONCAT( cf_packages, ',{$id}' )", \IPS\Db::i()->in( 'cf_packages', array( $this->_id ) ) );
		\IPS\Db::i()->update( 'nexus_referral_rules', "rrule_purchase_packages=CONCAT( rrule_purchase_packages, ',{$id}' )", \IPS\Db::i()->in( 'rrule_purchase_packages', array( $this->_id ) ) );
		\IPS\Db::i()->update( 'nexus_support_departments', "dpt_packages=CONCAT( dpt_packages, ',{$id}' )", \IPS\Db::i()->in( 'dpt_packages', array( $this->_id ) ) );
	...
	protected function _lowestPrice( \IPS\nexus\Customer $customer, $currency, &$priceMayChange = FALSE, &$priceNotIncludingDiscounts = NULL )
	{
		...
		if ( $this->stock == -2 )
		{
			...
			foreach ( \IPS\Db::i()->select( '*', 'nexus_product_options', array( 'opt_package=?', $this->id ) ) as $option )
			{
				...
				if ( isset( $basePriceAdjustments[ $currency ] ) and ( $basePriceAdjustments[ $currency ] or $basePriceAdjustments[ $currency ] === '0' ) )
				{
					...
					{
						$mostReducedOption = $basePriceAdjustments[ $currency ];
					}
					
					$priceMayChange = TRUE;
				}
			}
			$mostReducedOption = $mostReducedOption ?: 0;
			
			$baseNoDiscounts += $mostReducedOption;
			$base += $mostReducedOption;
		}
		...
		if ( !empty( $renewOptions ) )
		{
			...
			foreach ( $renewOptions as $term )
			{
				if ( $term['add'] )
				{
					...
					$_lowest = $base + $term['cost'][ $currency ]['amount'];
					$_lowestNoDiscounts = $baseNoDiscounts + $term['cost'][ $currency ]['amount'];
				}
				else
				{
					$_lowest = $base;
				...
				if ( $lowest === NULL or $_lowest < $lowest )
				{
					$lowest = $_lowest;
				}
				if ( $lowestNoDiscounts === NULL or $_lowestNoDiscounts < $lowestNoDiscounts )
				{
		...
		if ( \IPS\Settings::i()->nexus_show_tax and $this->tax )
		{
			try
			{
				...
				/* What is the rate? */
				$taxRate = $tax->rate( $customer->estimatedLocation() );
				
				/* Add it on */
				$lowest += ( $lowest * $taxRate );
				$lowestNoDiscounts += ( $lowestNoDiscounts * $taxRate );
			}
	...
	public function getButtons( $url, $subnode=FALSE )
	{
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_view' ) )
		{
			$buttons['view_purchases'] = array(
				...
				'link'	=> $url->setQueryString( array( 'do' => 'view_purchases', 'id' => $this->_id ) )
			);
		}
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'p_reg', $this->reg ) );
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'p_group_renewals', $this->group_renewals, FALSE, array(), NULL, NULL, NULL, 'p_group_renewals' ) );
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'p_upsell', $this->upsell, FALSE, array(), NULL, NULL, NULL, 'p_upsell' ) );
		...
		$form->add( new \IPS\Helpers\Form\Upload( 'p_images', iterator_to_array( new \IPS\File\Iterator( \IPS\Db::i()->select( 'image_location', 'nexus_package_images', array( 'image_product=?', $this->id ), 'image_primary' ), 'nexus_Products' ) ), FALSE, array( 'storageExtension' => 'nexus_Products', 'multiple' => TRUE, 'image' => TRUE, 'template' => "nexus.store.images" ), NULL, NULL, NULL, 'p_images' ) );
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'p_review_moderate', $this->review_moderate, FALSE, array(), NULL, NULL, NULL, 'p_review_moderate' ) );
	...
	public function updatePurchase( \IPS\nexus\Purchase $purchase, $changes )
	{
		if ( array_key_exists( 'renew_options', $changes ) )
		{
			...
			foreach ( $changes['renew_options'] as $k => $data )
			{
				if ( $data['old'] != 'x' )
				{
					if ( $data['old']['cost'][ $purchase->renewals->cost->currency ]['amount'] == $purchase->renewals->cost->amount )
					{
	...
	public function optionValuesStockAndPrice( $optionValues, $includeDiscounts=TRUE )
	{
		...
		if ( $this->stock == -2 )
		{
			...
			{
				$price = new \IPS\nexus\Money( $defaultPrice->amount + $basePriceAdjustments[ $defaultPrice->currency ], $defaultPrice->currency );
			}
			else
			{
				$price = $defaultPrice;
		...
		foreach ( $items as $quantity )
		{
			...
			{					
				try
				{
					...
					{
						$price += $basePriceAdjustments[ $memberCurrency ];
					}
				}
	...
	public function clientAreaPage( \IPS\nexus\Purchase $purchase )
	{
		...
		foreach ( $customFields as $field )
		{
			...
			{
				$customFieldsForm->addDummy( $field->_title, $value );
			}
		}
		if ( !$editable )
		{
		...
		{
			/* Reactivate */
			if ( !$purchase->renewals and count( json_decode( $this->renew_options, TRUE ) ) and $purchase->can_reactivate )
			{
		...
		/* Associated IP.Downloads Files */
		$associatedFiles = array();
		if ( \IPS\Application::appIsEnabled( 'downloads' ) and \IPS\Settings::i()->idm_nexus_on )
		{
			$associatedFiles = \IPS\downloads\File::getItemsWithPermission( array( array( \IPS\Db::i()->in( 'file_nexus', array( $this->id ) ) ) ) );
		}
	...
	public function acpEditSave( \IPS\nexus\Purchase $purchase, array $values )
	{
		...
		foreach ( new \IPS\Patterns\ActiveRecordIterator( \IPS\Db::i()->select( '*', 'nexus_package_fields', \IPS\Db::i()->findInSet( 'cf_packages', array( $this->id ) ) ), 'IPS\nexus\Package\CustomField' ) as $field )
		{
			$customFields[ $field->id ] = $values[ 'nexus_pfield_' . $field->id ];
		}
	...
	{		
		...
		if ( $this->notify )
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'purchaseNotify', array( $purchase, $invoice ) );
	...
	{
		if ( $this->custom or !$purchase->active or ( !$this->allow_upgrading and !$this->allow_upgrading and !$overrideRestrictions ) or $purchase->grouped_renewals )
		{
			return array();
		}
		else
		...
		foreach ( $this->upgradeDowngradeOptions( $purchase, $overrideRestrictions ) as $package )
		{
			...
			if ( count( $renewalOptions ) === 1 )
			{
				foreach ( $renewalOptions as $k => $v )
				{
					...
					if ( $costToUpgrade->amount === 0 )
					{
						$desciptions[ $package->id ] = $v;
					}
					elseif( $costToUpgrade->amount > 0 )
					{
					...
					{
						$costToUpgrade->amount = abs( $costToUpgrade->amount );
			...
			{
				$renewalOptionsString = \IPS\Member::loggedIn()->language()->formatList( $renewalOptions, \IPS\Member::loggedIn()->language()->addToStack('or_list_format') );
	...
	protected function costToUpgrade( \IPS\nexus\Purchase $purchase, $renewalOption = NULL )
	{
		...
		switch ( $type )
		{
			...
			case 2:
				...
				$option = new \IPS\nexus\Purchase\RenewalTerm( new \IPS\nexus\Money( $option['cost'][$currency]['amount'], $currency ), new \DateInterval( 'P' . $option['term'] . mb_strtoupper( $option['unit'] ) ) );
				$daysLeftUntilExpiry = \IPS\DateTime::create()->diff( $purchase->expire )->days;
				$newCostPerDay = $option->costPerDay()->amount;
				$oldCostPerDay = $purchase->renewals->costPerDay()->amount;				
				return new \IPS\nexus\Money( ( $newCostPerDay * $daysLeftUntilExpiry ) - ( $oldCostPerDay * $daysLeftUntilExpiry ), $currency );
		}
	}
	...
	public function upgradeDowngrade( \IPS\nexus\Purchase $purchase, \IPS\nexus\Package $newPackage, $chosenRenewalOption = NULL, $skipCharge = FALSE )
	{
		...
		if ( !$skipCharge )
		{
			...
			elseif ( $costToUpgrade->amount < 0 )
			{
				...
				$credits[ $costToUpgrade->currency ]->amount += abs( $costToUpgrade->amount );
	...
	public function _removeUsergroups( \IPS\nexus\Purchase $purchase, $basis = array() )
	{
		...
		if ( $basis['return_primary'] )
		{
			...
			if ( $purchase->member->member_group_id == $basis['primary_group'] )
			{
				...
				catch ( \UnderflowException $e )
				{
					...
					/* Don't move them back into validating */
					if ( $purchase->member->member_group_id == \IPS\Settings::i()->auth_group )
					{
		...
		{
			$previous_secondary = explode( ',', $extra['old_secondary_groups'] );
			$current_secondary = $purchase->member->mgroup_others ? explode( ',', $purchase->member->mgroup_others ) : array();
			$added_secondary = explode( ',', $basis['secondary_group'] );
												
			/* We just want to take out the ones that we put in */
			foreach ( $added_secondary as $added )
			{
				if ( !in_array( $added, $previous_secondary ) )
				{
					$k = array_search( $added, $current_secondary );
					unset( $current_secondary[$k] );
				}
			}
			
			$purchase->member->mgroup_others = implode( ',', array_filter( $current_secondary ) );
			$purchase->member->save();
		}
	}
}