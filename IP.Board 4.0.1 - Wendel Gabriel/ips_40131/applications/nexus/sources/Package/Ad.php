...
class _Ad extends \IPS\nexus\Package
{
	...
	public static function acpFormFields( \IPS\nexus\Package $package, $custom=FALSE, $customEdit=FALSE )
	{
		...
		if ( !$customEdit ) // If we're editing a custom package, they can get to this by editing the advertisement
		{
			...
			$return['package_settings']['p_exempt'] = new \IPS\Helpers\Form\Select( 'p_exempt', $package->id ? ( $package->exempt == '*' ? '*' : explode( ',', $package->exempt ) ) : '*', FALSE, array( 'options' => \IPS\Member\Group::groups(), 'parse' => 'normal', 'multiple' => TRUE, 'unlimited' => '*', 'unlimitedLang' => 'everyone' ) );
	...
	public function generateInvoiceForm( \IPS\Helpers\Form $form, $k )
	{
		...
		$field = new \IPS\Helpers\Form\Upload( 'advertisement_image' . $k, NULL, TRUE, array( 'storageExtension' => 'nexus_Ads', 'image' => $maxDims ) );
		$field->label = \IPS\Member::loggedIn()->language()->addToStack('advertisement_image');
		$form->add( $field );
	}
	...
	{
		$item->extra['image'] = (string) $values[ 'advertisement_image' . $k ];
