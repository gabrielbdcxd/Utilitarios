...
class _Product extends \IPS\nexus\Package
{
	...
	public function updatePurchase( \IPS\nexus\Purchase $purchase, $changes )
	{
		if ( array_key_exists( 'lkey', $changes ) )
		{
			...
			$class = 'IPS\nexus\Purchase\LicenseKey\\' . ucfirst( $this->lkey );
	...
	public function addToCart( \IPS\nexus\extensions\nexus\Item\Package $item, array $values, $memberCurrency )
	{
		if ( $this->subscription )
		{
			...
			$purchase = \IPS\Db::i()->select( '*', 'nexus_purchases', array( 'ps_app=? AND ps_type=? AND ps_item_id=? AND ps_cancelled=0 AND ps_member=?', 'nexus', 'package', $this->id, \IPS\Member::loggedIn()->member_id ) );
			if ( count( $purchase ) )
			{
