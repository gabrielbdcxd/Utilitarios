...
class _Coupon extends \IPS\Node\Model
{
	...
	public function form( &$form )
	{
		...
		$id = $this->id ?: NULL;
		$form->add( new \IPS\Helpers\Form\Text( 'c_code', $this->id ? $this->code : mb_strtoupper( mb_substr( md5( uniqid() ), 0, 5 ) ), TRUE, array( 'maxLength' => 25 ), function( $val ) use ( $id )
		{
			try
			{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'c_renewals', $this->renewals, FALSE, array(), NULL, NULL, NULL, 'c_renewals' ) );
		...
		$form->add( new \IPS\Helpers\Form\Date( 'c_start', $this->id ? $this->start : \IPS\DateTime::ts( time() ), FALSE, array( 'time' => TRUE ) ) );
		$form->add( new \IPS\Helpers\Form\Date( 'c_end', $this->id ? $this->end : 0, FALSE, array( 'time' => TRUE, 'unlimited' => 0, 'unlimitedLang' => 'no_end_date' ) ) );
		...
		$form->add( new \IPS\Helpers\Form\Select( 'c_groups', ( $this->id and $this->groups !== '*' ) ? explode( ',', $this->groups ) : '*', FALSE, array( 'options' => $groups, 'multiple' => TRUE, 'unlimited' => '*' ) ) );
	...
	public function useCoupon( \IPS\nexus\Invoice $invoice, \IPS\nexus\Customer $customer )
	{
		...
		{
			if ( isset( $uses[ $customer->member_id ] ) and $uses[ $customer->member_id ] >= $this->member_uses )
			{
		...
		if ( $this->products !== '*' )
		{
			...
			foreach ( $invoice->items as $item )
			{
				if ( $item instanceof \IPS\nexus\extensions\nexus\Item\Package and in_array( $item->id, $allowedProductIds ) )
				{
					...
					$productTotal += $item->price->amount;
				}
		...
		{
			if ( $this->limit_discount )
			{
				$base = $productTotal;
			}
			else
			{
				$base = $invoice->total->amount;
			}
			
			$discount = new \IPS\nexus\Money( -( $base / 100 * $this->discount ), $invoice->currency );
		}
		
		/* Never allow a discount greater than the invoice total */
		if ( $discount->amount < -$invoice->total->amount )
		{
			$discount = new \IPS\nexus\Money( -$invoice->total->amount, $invoice->currency );
		}
				
		/* Save that we've used it */
		if ( isset( $uses[ $customer->member_id ] ) )
		{
			$uses[ $customer->member_id ]++;
		}
		else
		{
			$uses[ $customer->member_id ] = 1;
		}
