...
class _Server extends \IPS\Node\Model
{
	...
	public function monitoringOnline( $version )
	{
		...
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'monitoring_online', array( $this, urlencode( $address ) ) );
			$email->send( $address );
		}
	}
	...
	public function monitoringOffline()
	{
		foreach ( explode( ',', \IPS\Settings::i()->monitoring_alert ) as $address )
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'monitoring_offline', array( $this, urlencode( $address ) ) );
			$email->send( $address );
		}
	}
	...
	public function monitoringPanic()
	{
		foreach ( explode( ',', \IPS\Settings::i()->monitoring_alert ) as $address )
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'monitoring_panic', array( $this, urlencode( $address ) ) );
			$email->send( $address );
		}
	}
	...
	public function monitoringAcknowledged( $by )
	{
		foreach ( explode( ',', \IPS\Settings::i()->monitoring_alert ) as $address )
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'monitoring_acknowledged', array( $this, urlencode( $address ), $by ) );
			$email->send( $address );
		}
	}
	...
	public function monitoringReset( $by )
	{
		foreach ( explode( ',', \IPS\Settings::i()->monitoring_alert ) as $address )
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'monitoring_reset', array( $this, urlencode( $address ), $by ) );
			$email->send( $address );
		}
	}
