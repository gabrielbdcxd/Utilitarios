...
class _Reply extends \IPS\Content\Comment implements \IPS\Content\Hideable
{
	...
	public function sendNotifications()
	{
		...
		foreach ( \IPS\Db::i()->select( 'staff_id', 'nexus_support_notify', array( $type, array( "departments='*' OR " . \IPS\Db::i()->findInSet( 'departments', array( $this->item()->department->id ) ) ) ) ) as $staffId )
		{
			...
			{
				if ( !in_array( $staffId, $sentTo ) and ( $this->item()->department->staff === '*' or in_array( $staffId, explode( ',', $this->item()->department->staff ) ) ) )
				{
					$email = \IPS\Email::buildFromTemplate( 'nexus', $this->type === static::REPLY_HIDDEN ? 'staffNotifyNote' : 'staffNotifyReply', array( $this ) );
					$email->from = $this->item()->department->email;
					switch ( \IPS\Settings::i()->nexus_sout_from )
					{
						case 'staff':
							if ( $this->member )
							{
								$email->fromName = $this->author()->name;
							}
							break;
						case 'dpt':
							$email->fromName = $email->language->get( 'nexus_department_' . $this->item()->department->_id );
							break;
						default:
					...
					$email->send( \IPS\Member::load( $staffId ) );
	...
	public function sendCustomerNotifications( $to, $cc, $bcc )
	{
		...
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'staffReply', array( $this ) );
		}
		else
		{
			$email = \IPS\Email::buildFromTemplate( 'nexus', 'staffReplyNoChrome', array( $this ) );
			$email->useWrapper = FALSE;
		}
		$email->from = $this->item()->department->email ?: \IPS\Settings::i()->email_out;
		switch ( \IPS\Settings::i()->nexus_sout_from )
		{
			case 'staff':
				$email->fromName = $this->author()->name;
