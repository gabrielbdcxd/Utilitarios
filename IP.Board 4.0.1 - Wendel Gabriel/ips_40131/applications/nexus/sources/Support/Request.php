...
class _Request extends \IPS\Content\Item implements \IPS\Content\ReadMarkers
{
	...
	public static function staff()
	{
		...
		foreach ( \IPS\Db::i()->select( '*', 'core_admin_permission_rows' ) as $row )
		{
			...
			{
				foreach ( \IPS\Db::i()->select( '*', 'core_members', array( 'member_group_id=? OR ' . \IPS\Db::i()->findInSet( 'mgroup_others', array( $row['row_id'] ) ), $row['row_id'] ) ) as $member )
				{
					$return[ $member['member_id'] ] = $member['name'];
				}
			}
			else
			{
				$return[ $row['row_id'] ] = \IPS\Member::load( $row['row_id'] )->name;
			}
		}
	 ...
	 */
	public static function table( \IPS\Http\Url $url, $where )
	{
		...
		$table->sortBy = $table->sortBy ? "sev_position asc, {$table->sortBy}" : 'sev_position asc, r_last_new_reply';
		...
		$table->parsers = array(
			...
			'r_member'	=> function( $val, $row )
			{
				...
				{
					return htmlentities( $row['r_email'], \IPS\HTMLENTITIES, 'UTF-8', FALSE );
				}
			},
			...
			{
				/* Use formatInterval to show an elapsed time here rather than a simple date */
				return \IPS\DateTime::formatInterval( \IPS\DateTime::ts( $val )->diff( new \IPS\DateTime ), 2 );
			},
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		else
		{
			...
			{
				foreach ( $contact->purchaseIds() as $id )
				{
		...
		{
			\IPS\Output::i()->error( \IPS\nexus\Customer::loggedIn()->member_id ? 'no_module_permission' : 'no_module_permission_guest', '1X248/1', 403, '' );
		}
		...
		foreach ( $availableDepartments as $department )
		{
			if ( count( $department->availableSeverities() ) > 1 )
			{
				...
				if ( !isset( $severityFields[ $serialized ] ) )
				{
					...
					$field = new \IPS\Helpers\Form\Radio( 'support_severity_' . $serialized, in_array( $defaultSeverityId, array_keys( $options ) ) ? $defaultSeverityId : NULL, FALSE, array( 'options' => $options ), NULL, NULL, NULL, 'support_severity_' . $serialized );
				...
				$departmentToggles[ $department->id ][] = 'support_severity_' . $serialized;
			}
		}
	...
	public function processForm( $values )
	{
		...
		if ( $ppiCost = $department->ppiCost() )
		{
			...
			catch ( \UnderflowException $e )
			{
				...
				$item = new \IPS\nexus\extensions\nexus\Item\SupportCharge( $department->_title, $ppiCost );
	...
	public function processAfterCreate( $comment, $values )
	{
		if ( $this->member )
		{
			...
			if ( $comment->type === Reply::REPLY_STAFF )
			{
				$data['type'] = 'staff';
			}
			elseif ( $comment->type === Reply::REPLY_EMAIL )
			{
				$data['type'] = 'email';
			}
			
			\IPS\nexus\Customer::load( $this->member )->log( 'support', $data );
		}
	...
	public function noteForm()
	{
		...
		$form->add( new \IPS\Helpers\Form\Editor( 'note', NULL, TRUE, array( 'app' => 'nexus', 'key' => 'Support', 'minimize' => 'support__note_placeholder', 'autoSaveKey' => md5( "req{$this->id}-note" ) ) ) );
		...
		$form->add( new \IPS\Helpers\Form\Select( 'assign_to', $this->staff ? $this->staff->member_id : 0, FALSE, array( 'options' => array_merge( array( 0 => 'unassigned' ), static::staff() ) ), NULL, NULL, NULL, 'staffNoteAssign' ) );
		...
		if ( $values = $form->values() )
		{
			...
			\IPS\File::claimAttachments( 'reply-nexus/support-' . $this->id, $this->id, $message->id );
	...
	{	
		...
		$form->add( new \IPS\Helpers\Form\Editor( 'message', NULL, TRUE, array( 'app' => 'nexus', 'key' => 'Support', 'minimize' => 'support__comment_placeholder', 'autoSaveKey' => md5( "req{$this->id}-reply" ) ) ) );
	...
	public function sendNotifications()
	{
		...
		foreach ( \IPS\Db::i()->select( 'staff_id', 'nexus_support_notify', array( array( 'type=?', 'n' ), array( "(departments='*' OR " . \IPS\Db::i()->findInSet( 'departments', array( $this->department->id ) ) . ')' ) ) ) as $staffId )
		{
			if ( in_array( $staffId, $staffIds ) )
			{
				...
				{
					$email = \IPS\Email::buildFromTemplate( 'nexus', 'staffNotifyNew', array( $this ) );
					$email->from = $this->department->email;
					switch ( \IPS\Settings::i()->nexus_sout_from )
					{
						case 'staff':
							$email->fromName = $this->supportAuthor()->name();
							break;
						case 'dpt':
							$email->fromName = $email->language->get( 'nexus_department_' . $this->department->_id );
							break;
						default:
							$email->fromName = \IPS\Settings::i()->nexus_sout_from;
							break;
					}
					$email->unsubscribe = array( 'text' => 'manage_notification_settings', 'link' => \IPS\Http\Url::internal( 'app=nexus&module=support&controller=requests&do=notifications', 'admin' ) );
					$email->send( $member );
				}
			}
			else
