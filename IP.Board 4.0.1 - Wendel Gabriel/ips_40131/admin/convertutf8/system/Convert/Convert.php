...
class Convert
{
			...
			if ( ! in_array( $table['name'], static::$problemTables ) AND ( in_array( $tableCharSet, $this->getMysqlCharSets() ) ) and ( \IPSUtf8\Session::i()->current_charset == 'utf-8' or \IPSUtf8\Session::i()->current_charset == 'iso-8859-1' ) AND ( !isset( $sessionData['force_conversion'] ) OR $sessionData['force_conversion'] !== 1 ) )
			{
				...
				foreach( array_keys( $table['definition']['columns'] ) as $col )
				{
					if ( in_array( $col, $convertCols ) )
					{
						...
						if ( \IPSUtf8\Session::i()->current_charset != 'utf-8' OR ( $tableCharSet != 'utf8' AND ! in_array( $columnCollation, array( 'utf8_unicode_ci', 'utf8mb4_unicode_ci' ) ) ) OR ( $tableCharSet == 'utf8' AND mb_substr( $columnCollation, 0, 6 ) == 'latin1' ) )
						{
							if ( \IPSUtf8\Session::i()->current_charset == 'iso-8859-1' AND $tableCharSet == 'latin1' )
							{
						...
						{
							/**
							 * @note This looks a little weird but bare with me. Semi-old databases (post-2.x but pre-4.x) can have UTF8 Tables and Columns, but contain Latin1 data.
							...
							if ( $tableCharSet == 'utf8mb4' OR $columnCollation == 'utf8mb4_unicode_ci' )
							{
							...
							$select[] = 'CASE WHEN STRCMP(CONVERT(CONVERT(CONVERT(`' . $col . '` USING ' . $source_charset . ') USING BINARY) USING utf8), `' . $col . '` COLLATE ' . $collation . ') = 0 THEN CONVERT(`' . $col . '` USING BINARY) ELSE CONVERT(CONVERT(CONVERT(`' . $col . '` USING ' . $source_charset . ') USING BINARY) USING utf8) END';
						}
					}
					else
					{
	...
	public function renameTables()
	{
		...
		foreach( $tables as $name )
		{
			...
			{
				if( static::$utf->checkForTable( $tableName ) )
				{
					static::$db->query( "DROP TABLE IF EXISTS `" . $origPrefix . $tableName . '`' );
					static::log( "DROP TABLE IF EXISTS `" . $origPrefix . $tableName . '`' );
				}

				static::$utf->query( "RENAME TABLE `" . $tableNameNoPrefix . '` TO `' . $origPrefix . $tableName . '`' );
				static::log( "RENAME TABLE `" . $tableNameNoPrefix . '` TO `' . $origPrefix . $tableName . '`' );
			}
		}
		...
		foreach( \IPSUtf8\Session::i()->tables as $name => $data )
		{
			if ( in_array( $name, $this->getNonUtf8CollationTables() ) )
			{
				...
				if ( count( $cols ) )
				{
					...
					/* Changing collation can cause issues for duplicate enries in unique keys */
				...
				if ( $hasUnique === TRUE )
				{
					...
					static::$utf->query( "INSERT IGNORE INTO `" . static::$db->prefix . $name . "` SELECT * FROM `" . static::$db->prefix . $name . "_temp`" );
	...
	public function getTable()
	{
		...
		{
			/* Table selected and convertion in progress */
			$table = \IPSUtf8\Session::i()->tables[ \IPSUtf8\Session::i()->current_table ];
	...
	public function getNonUtf8CollationTables()
	{
		if ( static::$nonUtf8Collations === null )
		{
			...
			foreach( \IPSUtf8\Session::i()->tables as $table => $definition )
			{
				...
				if ( $definition['collation'] AND !in_array( $definition['collation'], array( 'utf8_unicode_ci', 'utf8mb4_unicode_ci' ) ) )
				{
